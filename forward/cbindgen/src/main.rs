//! pie-forward-cbindgen — decoupled generator for the committed C header.
//!
//! Emits `forward/include/pie_forward.h` from the `#[repr(C)]` surface in
//! `pie-forward::ffi`. The build graph consumes the committed header;
...
fn main() {
    ...
    let forward_crate = manifest.join("..");
    ...
    cbindgen::Builder::new()
        ...
        .with_config(config)
        .generate()
        .expect("generate pie_forward.h from pie-forward::ffi")
        .write_to_file(&out);
