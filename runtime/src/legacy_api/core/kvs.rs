...
impl inferlet::core::kvs::Host for InstanceState {
    ...
    async fn store_list_keys(&mut self) -> anyhow::Result<Vec<String>> {
        let (tx, rx) = oneshot::channel();
        legacy_kvs::Command::ListKeys { response: tx }.dispatch();
        let res = rx.await?;
        Ok(res)
    }
}
