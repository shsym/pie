...
pub fn spawn() {
    SERVICE
        .spawn(|| MessageQueue::default())
        .expect("messaging already spawned");
}
...
pub async fn subscribe(topic: String, sender: mpsc::Sender<String>) -> anyhow::Result<ListenerId> {
    let (tx, rx) = oneshot::channel();
    SERVICE.send(Message::Subscribe {
        topic,
        sender,
        response: tx,
    })?;
...
impl ServiceHandler for MessageQueue {
    ...
    async fn handle(&mut self, msg: Message) {
        match msg {
            Message::Push { topic, message } => match self.queues.get_mut(&topic) {
                ...
                None => {
                    self.queues
                        .insert(topic, Queue::Messages(VecDeque::from([message])));
                }
            },

            Message::Pull { topic, response } => match self.queues.get_mut(&topic) {
                Some(Queue::Messages(q)) => {
                ...
                None => {
                    self.queues
                        .insert(topic, Queue::Waiters(VecDeque::from([response])));
                }
            },
