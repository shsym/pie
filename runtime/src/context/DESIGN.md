...
│                                                                │
│  ┌────────────┐  ┌────────────┐  ┌───────────┐  ┌───────────┐ │
│  │ PageStore   │  │  Arbiter   │  │ alloc_queue │  │restore_queue│ │
...
├── contention.rs       # Eviction, suspension, alloc_queue queue
└── restore.rs          # Restoration, replay planning, restore_queue queue
```
...
4. EVICTION LOOP — with deferred page tracking
   ...
   loop:
     ...
     b. SUSPEND VICTIM
        ...
        Enqueue victim in restore_queue
   ...
   If deferred_pages > 0:
     → enqueue in alloc_queue (FIFO), return (pages will free on clear_pinned)

5. NO VICTIM, REQUESTER LOSES
   Suspend ALL of requester's contexts (Active immediately, Pinned deferred)
   Set requester state = Pending
   Enqueue in restore_queue (with deferred_op, pending_pinned_count = number of Pinned contexts)
   Return
```
...
if ctx.pending_suspend:
    ...
    decrement owning process's PendingRestore.pending_pinned_count
    drain_queues()
else:
    ctx.state = Active
```
...
drain_queues():
    ...
    while let Some(head) = alloc_queue.front():
        if free_pages[head.device] >= head.num_pages:
            pop head → allocate pages → send response
        else:
            return   // exit drain_queues

    // Phase 2: restore_queue (priority heap, checks ALL devices)
    while let Some(top) = restore_queue.peek():
...
### restore_queue Queue (Priority Heap + Aging)
...
// BinaryHeap<PendingRestore> — global (checks all devices on drain)
```
...
async fn wit_reserve_pages(pid: ProcessId, ctx_id: ContextId, n: u32) -> Result<()> {
    // Actor holds the response channel if enqueued (alloc_queue or restore_queue).
    // Resolves when: direct alloc, drain serves alloc_queue, or process restored.
...
(from alloc_queue or after restoration from restore_queue), it sends Ok on the
...
| Wait queues | Single queue (Allocate + Restore) | Two queues: alloc_queue (FIFO) + restore_queue (heap) |
...
| Queue admission | Per-context committed bypass | Priority gate (requester vs restore_queue head) + FIFO gate (alloc_queue non-empty) |
