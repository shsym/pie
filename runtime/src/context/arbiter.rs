//! Arbiter — DAG-Aware Invested-Importance Scheduling.
//!
...
impl Arbiter {
    ...
    pub fn remove_entry(&mut self, pid: &ProcessId) {
        self.entries.remove(pid);
    }

    pub fn entry_is_empty(&self, pid: &ProcessId) -> bool {
        self.entries.get(pid).map_or(true, |e| e.total_pages() == 0)
    }

    // ==================== DAG Weights ====================

    pub fn set_weight(&mut self, pid: ProcessId, weight: f64) {
    ...
    pub fn zero_device(&mut self, pid: ProcessId, device: DeviceId) {
    ...
    pub fn set_device(&mut self, pid: ProcessId, device: DeviceId, committed: usize, working: usize) {
        let e = self.entry_mut(pid);
        let d = e.device_mut(device);
    ...
    pub fn priority(&self, pid: &ProcessId, device: DeviceId) -> f64 {
        self.entries.get(pid).map(|e| e.priority_on(device)).unwrap_or(0.0)
    }

    /// Pages held by a process on a specific device.
    pub fn pages_on(&self, pid: &ProcessId, device: DeviceId) -> usize {
        self.entries.get(pid).map(|e| e.pages_on(device)).unwrap_or(0)
    }

    /// Invested importance at an arbitrary page count on a device.
    /// For unknown processes, uses default weight (1.0).
    pub fn priority_at(&self, pid: &ProcessId, pages: usize) -> f64 {
        let weight = self.entries.get(pid).map(|e| e.weight).unwrap_or(1.0);
        weight * pages as f64
    }

    /// Birth timestamp of a process (for FCFS tiebreaking).
    pub fn created_at(&self, pid: &ProcessId) -> Option<Instant> {
        self.entries.get(pid).map(|e| e.created_at)
    }
    ...
    pub fn get(&self, pid: &ProcessId) -> Option<&ProcessEntry> {
        self.entries.get(pid)
    }

    // ==================== Internal Helpers ====================

    fn entry_mut(&mut self, pid: ProcessId) -> &mut ProcessEntry {
        self.entries.entry(pid).or_insert_with(ProcessEntry::new)
    }
}
...
mod tests {
    ...
    fn per_device_isolation() {
        ...
        assert_eq!(arb.pages_on(&pid(1), DEV0), 50);
        assert_eq!(arb.pages_on(&pid(1), DEV1), 10);
    }
    ...
    fn higher_weight_means_higher_priority() {
        ...
        arb.set_weight(pid(1), 5.0);
        arb.set_weight(pid(2), 1.0);
    ...
    fn commit_moves_working_to_committed() {
        let mut arb = Arbiter::new();
        arb.add_working(pid(1), DEV0, 10);
        assert_eq!(arb.pages_on(&pid(1), DEV0), 10);

        arb.commit(pid(1), DEV0, 5);
        assert_eq!(arb.pages_on(&pid(1), DEV0), 10); // total unchanged
    }
    ...
    fn zero_device_clears_pages() {
        ...
        arb.add_working(pid(1), DEV1, 7);

        assert_eq!(arb.pages_on(&pid(1), DEV0), 8);
        assert_eq!(arb.pages_on(&pid(1), DEV1), 7);

        arb.zero_device(pid(1), DEV0);
        assert_eq!(arb.pages_on(&pid(1), DEV0), 0);
        assert_eq!(arb.pages_on(&pid(1), DEV1), 7); // unaffected
    }
    ...
    fn post_allocation_floor_prevents_thrashing() {
        let mut arb = Arbiter::new();
        arb.add_working(pid(1), DEV0, 30); // A holds 30 (won eviction)

        let b_current = arb.pages_on(&pid(2), DEV0); // = 0
        let b_floor = arb.priority_at(&pid(2), b_current + 30);
    ...
    fn set_device_round_trip() {
        ...
        let pre_pages = arb.pages_on(&pid(1), DEV0);
        assert_eq!(pre_pages, 8);

        arb.zero_device(pid(1), DEV0);
        assert_eq!(arb.pages_on(&pid(1), DEV0), 0);

        arb.set_device(pid(1), DEV0, 5, 3);
        assert_eq!(arb.pages_on(&pid(1), DEV0), pre_pages);
    }
