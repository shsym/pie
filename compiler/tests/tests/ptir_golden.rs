...
fn step_line_k(
    ...
    b: &BoundTrace,
    inputs: &PassInputs,
    host: &mut dyn pie_eval::interp::KernelHost,
    fed: String,
) -> String {
...
impl pie_eval::interp::KernelHost for QuestKernels {
    fn kernel(
        &mut self,
        name: &str,
        args: &[Value],
...
fn golden_extern_contrastive() {
    use pie_eval::interp::ExternChannel;
    ...
    let mut am =
        pie_eval::interp::Instance::new_with_externs(&am_b, &[], &[(0, ring.clone())]).unwrap();
    let mut ex = pie_eval::interp::Instance::new_with_externs(&ex_b, &[], &[(0, ring)]).unwrap();
