...
/// named classes in `pie_codegen::fault`.
///
...
fn every_fault_code_is_readable_in_its_kernel_vocabulary() {
    let declared: Vec<u32> = pie_codegen::fault::CLASSES
        .iter()
        .flat_map(|class| {
    ...
    for (trace, backend, kernel) in every_emitted_kernel() {
        ...
        for code in literals {
            if speaks_tags {
                ...
                let is_a_recorded_alias = pie_codegen::fault::TAG_ALIASES
                    .iter()
                ...
                assert!(
                    ...
                    "{where_}: reports op tags as faults, and also writes the \
                     ...
                     do, or declare it in pie_codegen::fault."
                );
            } else {
                saw_declared += 1;
                assert!(
                    ...
                    "{where_}: writes fault {code:#x}, which \
                     pie_codegen::fault::CLASSES does not name, and dispatches no \
