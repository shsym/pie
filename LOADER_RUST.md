...
- `storage_program_executor.{hpp,cpp}`: interpreter over flat program view
- `byte_source.{hpp,cpp}`: mmap/pread/GDS sources
- `pie_cuda_abi.{hpp,cpp}`: declarative ABI construction
- `transforms.{hpp,cpp}`: CUDA/FlashInfer/CUTLASS transform dispatch

Model binders consume a finalized `WeightStore`; they should not know whether
the store came from the C++ planner or Rust planner during migration.
...
- Make Rust the default planner behind a config gate.
- Keep the C++ planner as a fallback for one release window.
- Compare dumps and final tensors in CI for overlapping paths.
- Delete C++ planning once coverage is complete.

Acceptance: C++ execution remains; C++ schema/planning/lowering code is removed.
...
- Wired the CUDA loader to compile/dump Rust storage programs with Rust as the
  default planner (`PIE_CUDA_LOADER_PLANNER={rust,dual,cpp}`; `cpp` is
...
- Made Rust the single canonical layout optimizer. The former CUDA C++
  `layout_optimizer` rewrite path has been removed from the production and
  test builds; CUDA `PIE_CUDA_LOADER_PLANNER` now defaults to `rust`, with
  `dual` and deprecated `cpp` left only as compatibility/debug modes.
...
- Extended `benches/run_loader_evidence.py` with `--loader-planner` and
  `--pie-driver` so CUDA and portable Rust storage-program dumps can be
  regenerated by the same evidence harness; the harness now defaults Pie
  evidence runs to the canonical Rust planner.
