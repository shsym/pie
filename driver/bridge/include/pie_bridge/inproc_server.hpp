// pie_bridge/inproc_server.hpp — header-only in-process driver server.
//
...
public:
    ...
    inline void serve_forever(const RequestHandler& handler) {
        while (!stop_.load(std::memory_order_relaxed)) {
            ...
            if (rc != 0) {
                if (rc != -1) {
                    std::cerr << "[pie-driver-bridge] inproc recv returned "
                              << rc << "; stopping\n";
                }
                break;
            }
            ...
            try {
                ...
                std::cerr << "[pie-driver-bridge] inproc handler failed for req_id="
                          << req_id << ": " << e.what() << "\n";
