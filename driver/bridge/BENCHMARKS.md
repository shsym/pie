# pie-bridge benchmarks

Measured on Linux x86_64, single thread, release profile. Run:

```
cargo bench -p pie-bridge --features cabi --bench wire
cargo bench -p pie-bridge --features cabi --bench direct_ffi
cargo bench -p pie-bridge --features "cabi ipc" --bench shmem
...
- The fake subprocess driver uses a real `pie_bridge::ipc::ShmemServer`, copies
...
- IPC primitive roundtrip remains useful as a transport-only bench (`pie-bridge --bench shmem`), but production-shaped runtime IPC should be measured with `cargo bench -p pie --bench driver_channel`.
