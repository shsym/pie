...
use pie_ipc::SCHEMA_HASH;
use pie_ipc::ipc::{ShmemClient, ShmemServer};
...
fn unique_name(tag: &str) -> String {
    ...
    format!("/pie_ipc_test_{tag}_{}_{n}", std::process::id())
}
...
fn forward_request_full_roundtrip_through_shmem() {
                ...
                if let Some(lease) = server.poll() {
                    ...
                    match &frame.payload {
                        pie_ipc::ArchivedRequestPayload::Forward(fr) => {
                            ...
                            assert_eq!(fr.sampler_kinds.len(), 2);
    ...
    match &resp_archived.payload {
        pie_ipc::ArchivedResponsePayload::Forward(fr) => {
