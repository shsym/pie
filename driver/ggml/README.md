...
The binary built here (`pie_driver_ggml`) is intended to be spawned and
managed by the Python wrapper module `pie_driver_ggml` (forthcoming).
...
The binary lands at `build/bin/pie_driver_ggml`.

## Run (scaffold)

```bash
./build/bin/pie_driver_ggml --config dev.toml
```
...
will be the client; `pie_driver_ggml` decodes incoming batches and runs
the forward pass.
...
- [x] **M12 MoE infrastructure** — the full machinery is implemented (untested due to model size):
  ...
  - Per-arch loaders for **qwen3_5 / qwen3_moe** (Qwen3-MoE), **mixtral** (Mixtral-8x7B w/ `block_sparse_moe.experts.E.{w1,w2,w3}` naming), **gpt_oss** (reuses qwen3_moe layout — sinks via existing `flash_attn_ext_add_sinks`)
...
- [ ] **Wrapper** `pie_driver_ggml` Python module (spawn + shmem handshake)
