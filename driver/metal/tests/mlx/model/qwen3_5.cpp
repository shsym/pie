...
// `<dir>/qwen3_5_layer_<NN>.npy`, mirroring HF `output_hidden_states`:
...
void maybe_dump_hidden(std::int32_t slot, const Tensor& h) {
    ...
    char name[32];
    if (slot < 0) {
        std::snprintf(name, sizeof(name), "qwen3_5_layer_final.npy");
    } else {
        std::snprintf(name, sizeof(name), "qwen3_5_layer_%02d.npy", slot);
    }
...
Qwen3_5Graph::Qwen3_5Graph(ModelConfig cfg, ModelWeights weights)
    : cfg_(std::move(cfg)),
      w_(std::move(weights)),
      spec_(arch_spec_for(cfg_.arch, cfg_)) {}

// ── Full-attention layer: Qwen3 paged attention + output gate + partial RoPE ──
Tensor Qwen3_5Graph::full_attn_layer(std::int32_t il, Tensor hidden,
...
Tensor Qwen3_5Graph::linear_attn_layer(std::int32_t il, std::int32_t lin_ordinal,
...
Tensor Qwen3_5Graph::mlp_block(std::int32_t il, Tensor hidden) {
    const auto& L = w_.layers[il];
    const float eps = cfg_.rms_norm_eps;
...
Tensor Qwen3_5Graph::forward(const ForwardBatch& batch, KvCacheView& kv) {
    const float eps = cfg_.rms_norm_eps;
