...
/// Stable hash of the loader's compiler source, baked at build time (see
/// build.rs `hash_sources`). Folded into the C++ storage-program cache key so a
/// compiler-logic change auto-invalidates the on-disk cache — replacing the
/// manual `cache-vN` bump that was easy to forget.
#[unsafe(no_mangle)]
pub extern "C" fn pie_loader_compiler_version() -> u64 {
