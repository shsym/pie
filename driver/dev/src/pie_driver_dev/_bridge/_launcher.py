...
Each driver's `__main__.py` is a thin shim around `launch()` below:
    ...
    from pie_driver_<flavor>._bridge._launcher import launch
    from . import worker
...
def launch(
    *,
    ...
    model_config_dict: dict[str, Any] = {
        ...
        # Forwarded from pie config's `[model.driver]`. The worker
        # consumes this in `_bridge/worker.py` when constructing
        # `ShmemServer`. Threaded through `model_config` rather than a
        # new positional arg because the worker signature is shared
        # across dev/sglang/vllm and changing it is awkward.
