...
//         (GIT_TAG in driver/cuda/CMakeLists.txt is the authority)
//
// Vendored (rather than patched via CPM) so pie can prune the MoE activation
// matrix from driver/cuda/src/kernels.def; see moeGemmBiasAct below. The only
...
// --- pie: activation set pruned by driver/cuda/src/kernels.def -------------
  ...
  case ActivationType::act:                                                 \
    if constexpr (pie_moe_activation_enabled(ActivationType::act)) {        \
      ...
      TLLM_THROW(                                                           \
          ...
          ") to driver/cuda/src/kernels.def and rebuild.");                 \
    }                                                                       \
    break;
