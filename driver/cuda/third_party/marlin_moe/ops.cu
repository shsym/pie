...
// Detoxified for pie, mirroring `third_party/marlin/marlin.cu`: the
// `moe_wna16_marlin_gemm(torch::stable::Tensor& ...)` entry point and the torch
// includes are dropped, leaving the torch-free
// `marlin_moe_wna16::marlin_mm(void*, void*, ...)` launcher that
// `marlin_moe_wrapper.cpp` calls. `STD_TORCH_CHECK` is redirected to pie's
// check macro by `../marlin/scalar_type.hpp`.
#include "kernel.h"

// CUDA 13 gives __global__ template stubs static linkage, so a cross-TU
// explicit instantiation never binds. Same fix as `../marlin/marlin.cu`:
// include the generated instantiation directly so it and the selector below
