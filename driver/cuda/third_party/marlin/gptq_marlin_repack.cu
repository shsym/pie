#include "marlin.cuh"
// Detoxified for pie: skip the torch op-registration include. The
// torch-typed `gptq_marlin_repack(torch::Tensor&, ...)` wrapper at the
// bottom of this file is gated under `PIE_MARLIN_KEEP_TORCH_WRAPPER`.
// pie's loader calls the kernel through a torch-free wrapper instead.
...
// ── Torch-free entry point for pie ───────────────────────────────────
// Pie's loader calls this directly with raw pointers; it sets up the
// device-attribute queries that the torch wrapper does and invokes the
// templated kernel. v1 supports only num_bits=4, no permutation, no
// 8-bit activation (i.e. standard W4A16 GPTQ).
void pie_gptq_marlin_repack_w4_no_perm(
    const std::uint32_t* b_q_weight,    // [K/8, N] packed int32
    std::uint32_t*       out,           // [K/16, N*16/8] marlin tile-packed
    int                  size_k,
    int                  size_n,
    cudaStream_t         stream)
{
    ...
    cudaFuncSetAttribute(
        gptq_marlin_repack_kernel<repack_threads, NUM_BITS,
                                  HAS_PERM, IS_A_8BIT>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, max_shared_mem);
    gptq_marlin_repack_kernel<repack_threads, NUM_BITS, HAS_PERM, IS_A_8BIT>
        <<<blocks, repack_threads, max_shared_mem, stream>>>(
            b_q_weight, /*perm=*/nullptr, out, size_k, size_n);
}
...
#ifdef PIE_MARLIN_KEEP_TORCH_WRAPPER
...
#endif  // PIE_MARLIN_KEEP_TORCH_WRAPPER
