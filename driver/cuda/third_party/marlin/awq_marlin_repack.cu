#include "marlin.cuh"
// Detoxified for pie: drop torch op-registration include. The torch
// wrapper at the bottom is gated under PIE_MARLIN_KEEP_TORCH_WRAPPER;
// the loader calls the kernel through a torch-free entry instead.
...
// AWQ qweight layout differs from GPTQ: the packing axis is N (not K),
// so input shape is `[size_k, size_n/8]` int32 instead of GPTQ's
// `[size_k/8, size_n]`. Output is the same marlin tile-packed layout.
void pie_awq_marlin_repack_w4(
    const std::uint32_t* b_q_weight,    // [K, N/8] packed int32
    ...
    cudaStream_t         stream)
{
    ...
    cudaFuncSetAttribute(
        awq_marlin_repack_kernel<repack_threads, NUM_BITS, IS_A_8BIT>,
