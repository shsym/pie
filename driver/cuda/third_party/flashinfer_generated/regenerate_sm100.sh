#!/bin/bash
# Regenerate the vendored Blackwell (sm100) TMA warp-specialized MoE grouped-GEMM
# instantiations in gemm_grouped/100/ after bumping the vendored FlashInfer
# revision or changing PIE_MOE_ACTIVATION in driver/cuda/src/kernels.def.
#
#   BUILD_DIR=/path/to/cuda/build ./regenerate_sm100.sh
#
...
"$NVCC" -O3 -DNDEBUG -std=c++20 --generate-code=arch=compute_100a,code=[sm_100a] \
  -Xcompiler=-fPIC --extended-lambda --expt-relaxed-constexpr \
  -DENABLE_BF16=1 -DENABLE_FP8=1 -DUSING_OSS_CUTLASS_MOE_GEMM=1 \
  -DCOMPILE_BLACKWELL_TMA_GROUPED_GEMMS=1 --options-file "$RSP" \
...
for index, chunk in enumerate(chunks):
    body = "\n".join(chunk)
    path = outdir / f"cutlass_kernel_file_gemm_grouped_sm100_group{index}.generated.cu"
    path.write_text(f"{header}{body}\n\n}}  // namespace tensorrt_llm::kernels::cutlass_kernels_oss\n")
print(f"{len(ordered)} instantiations in {len(chunks)} files")
PY

if [[ -d "$HERE/gemm_grouped/100" ]] &&
   diff -qr "$OUT/gen" "$HERE/gemm_grouped/100" >/dev/null; then
  echo "no changes"
  exit 0
fi
rm -rf "$HERE/gemm_grouped/100"
cp -r "$OUT/gen" "$HERE/gemm_grouped/100"
echo "regenerated $HERE/gemm_grouped/100"
