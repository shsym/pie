...
#include "model/gemma4_vision_forward.hpp"
...
void run_gemma4_vision(const VisRawWeights& w,
    ...
    auto MAL=[&](long n){bf* d;VCK(cudaMalloc(&d,n*sizeof(bf)));return d;};
    ...
    float* scr;VCK(cudaMalloc(&scr,(long)N*N*4));
    ...
    float* pf;VCK(cudaMalloc(&pf,(long)OUTL*Hd*4));VCK(cudaMemsetAsync(pf,0,(long)OUTL*Hd*4,S));
    ...
    for(bf* b:{h,hn,xc,q,k,v,attn,gate,up,act,tmp,pooled,pn})cudaFree(b);
    cudaFree(scr);cudaFree(pf);
}
...
void scatter_gemma4_vision(const Gemma4VisionInputs& vin, bf* hidden,
    ...
    for(int im=0; im<vin.num_images; ++im){
        ...
        float* pix_f32_d; VCK(cudaMalloc(&pix_f32_d,(long)n_floats*4));
        VCK(cudaMemcpyAsync(pix_f32_d,pix_h,(long)n_floats*4,cudaMemcpyHostToDevice,S));
        bf* pix_bf_d; VCK(cudaMalloc(&pix_bf_d,(long)n_floats*sizeof(bf)));
        ...
        float* pos_d; VCK(cudaMalloc(&pos_d,(long)n_patch*2*4));
        VCK(cudaMemcpyAsync(pos_d,posf.data(),(long)n_patch*2*4,cudaMemcpyHostToDevice,S));
        int* grp_d; VCK(cudaMalloc(&grp_d,(long)n_patch*4));
        ...
        bf* proj_d; VCK(cudaMalloc(&proj_d,(long)out_len*text_hidden*sizeof(bf)));
        ...
        VCK(cudaStreamSynchronize(S));
        cudaFree(pix_f32_d); cudaFree(pix_bf_d); cudaFree(pos_d); cudaFree(grp_d); cudaFree(proj_d);
        patch_off += n_patch;
    }
}
