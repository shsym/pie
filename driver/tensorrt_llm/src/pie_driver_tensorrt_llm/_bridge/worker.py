...
def _leader_loop(
    ...
    from .batching import Batch, prewarm_batch_helpers
    ...
    if os.environ.get("PIE_DRIVER_BATCH_PREWARM", "1") != "0":
        prewarm_batch_helpers(vocab_size=int(_vocab_size))
    if os.environ.get("PIE_DRIVER_ENGINE_PREWARM", "1") != "0":
        prewarm = getattr(engine, "prewarm", None)
        if callable(prewarm):
            try:
                prewarm()
            except Exception as exc:
                print(f"[pie-driver] engine prewarm skipped after failure: {exc}", flush=True)
