...
def prewarm_batch_helpers(vocab_size: int = 128) -> None:
    """Compile/load Numba helpers before the first real forward batch."""
    small_vocab = max(1, min(int(vocab_size), 128))
    decode_brle_batch(
    ...
    decode_sampling_masks(
        np.asarray([0, small_vocab], dtype=np.int32),
        np.asarray([0, 2], dtype=np.int32),
        1,
        small_vocab,
    )
