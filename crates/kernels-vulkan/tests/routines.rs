...
/// A `Encode` that remembers, and refuses nothing.
#[derive(Default)]
...
fn stand_in(at: usize, ty: Ty) -> ArgValue {
    match ty {
        Ty::Buf | Ty::I32s | Ty::U32s | Ty::U8s | Ty::F32s => ArgValue::Buffer {
            handle: at as u32,
            writes: false,
        },
        Ty::BufMut | Ty::F32sMut => ArgValue::Buffer {
            handle: at as u32,
            writes: true,
        },
        ...
        // uses twelve of it -- measured over all 100 rows, and the same twelve
        // metal and wgpu use. A routine that took an eleventh could not have
        // compiled: `kernels::shader` declares an `Arg` impl for these and no
        // others, so `routine!` would have failed at its own line.
...
const RECIPE: &[(&str, &[(usize, i32)])] = &[
    ...
    ("embed_gather_4bit", &[(6, 32), (7, 4)]),
    ("embed_gather_mb_4bit", &[(7, 32), (8, 4)]),
    ("embed_gather_scaled_4bit", &[(7, 32), (8, 4)]),
    ("embed_gather_scaled_mb_4bit", &[(8, 32), (9, 4)]),
    ...
    ("neox_decode", &[(4, 128), (5, 128), (6, 4096)]),
    ("neox_prop_decode", &[(4, 128), (5, 128), (6, 4096)]),
    ("neox_mb", &[(4, 128), (5, 128), (6, 4096), (7, 3)]),
    ...
    ("sdpa_paged_tiled_strided", &[(20, 256)]),
    ("sdpa_vector_decode_sink", &[(15, 64)]),
];
...
const SWEEP: &[Swept] = &[
    ("embed_gather_4bit", &[6, 7], affine),
    ("embed_gather_mb_4bit", &[7, 8], affine),
    ("embed_gather_scaled_4bit", &[7, 8], affine),
    ("embed_gather_scaled_mb_4bit", &[8, 9], affine),
    ("gdn_core_recurrent_prefill", &[9, 10], tilings),
    ("mxfp4_qmm_t_routed_bias", &[9, 10], tiles),
    ("qmm_t", &[7, 8, 9, 10], routed_qmm),
    ("qmm_t_bias", &[8, 9, 10, 11], routed_qmm),
    ("qmm_t_residual", &[8, 9, 10, 11], routed_qmm),
    ("qmm_t_fp16_precast", &[7, 8], tiles),
    ("qmm_t_bias_fp16_precast", &[8, 9], tiles),
    ("qmm_t_residual_fp16_precast", &[8, 9], tiles),
    ("qmm_t_splitk", &[11, 12, 13], wide_qmm),
    ("qmm_t_splitk_f32", &[11, 12, 13], wide_qmm),
    ("qmm_t_splitk_fp16_precast", &[11], row_tiles),
    ("qmm_t_splitk_fp16_precast_f32", &[11], row_tiles),
    ("qmm_t_strided", &[8, 9, 10], wide_qmm),
    ("qmm_t_strided_residual", &[9, 10, 11], wide_qmm),
    ("qmm_t_strided_fp16_precast", &[8], row_tiles),
    ("qmm_t_strided_fp16_precast_residual", &[9], row_tiles),
    ("qmv_fast", &[7, 8], affine),
    ("qmv_fast_residual", &[8, 9], affine),
    ("qmv_tail", &[7], bit_widths),
    ("qmv_tail_bias", &[8], bit_widths),
    ("qmv_wide_strided", &[9], bit_widths),
    ("qmm_t_routed", &[9, 10, 11, 12], routed_qmm),
...
/// The nine `(LANES, VROWS)` shapes the gdn prefill scan is compiled for, in
/// `scan_point` order.
fn tilings() -> Vec<Vec<i32>> {
    [
        (16, 1),
        (16, 2),
        (16, 4),
...
/// A body passes its dispatch EXACTLY the arguments its signature takes, in
/// order.
///
/// Not "every buffer is bound" -- the whole list, buffers and scalars alike,
/// compared value by value against the arguments the routine was called with.
/// The weaker form was written first and missed a real thing within the hour:
/// giving `neox_freqs_decode` a seventh pushed word turned nothing red,
/// because a pushed float is not a descriptor and no other check in either
/// plane counts them. `kernels/tests/shader_backends_agree.rs` compares
/// `Routine::args` against the row and cannot see it either -- an extra
/// `.v()` in the body is not an extra argument in the signature.
///
...
/// the same bytes and decides that from which operands may write. `BufMut` in
...
///
/// So the invariant is a SUBSEQUENCE, in order: a body may skip an argument,
/// and may not reorder two, invent one, or change a writability flag on the
/// way through. `every_routine_binds_the_buffers_its_module_uses_and_no_others`
/// is what says the skipping was right; this says the keeping was.
#[test]
            ...
            for value in got.iter().filter(|v| matches!(v, ArgValue::Buffer { .. })) {
                assert!(
                    ...
                    "`{}` fires `{entrypoint}` with {got:?}, which is not its \
                     signature's {want:?} in order. A buffer handle here is \
                     its own argument POSITION, so this reads directly as \
                     which arguments reached the device and in what order: \
                     anything out of order, invented, or with its writability \
                     changed stops being a subsequence here.",
                    r.name,
                );
            }
        }
...
fn no_routine_invents_a_kernel() {
    ...
    for r in kernels_vulkan::routines() {
        ...
        assert!(
            r.args.iter().any(|(_, p)| *p == Provenance::Env)
                || r.args.iter().all(|(_, p)| *p == Provenance::Trace),
            "unreachable, and here only so `Provenance` is used"
        );
    }
}
