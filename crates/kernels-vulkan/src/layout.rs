...
pub static KERNELS: &[KernelSig] = &[
    // 6 in layout/embed_gather.comp
    kernel!(embed_gather_4bit "embed_gather_4bit", file = Some("layout/embed_gather.comp"), launch = kernels::LaunchRule::Elementwise,
    ...
    // 6 in layout/embed_gather.comp
    kernel!(embed_gather_mb_4bit "embed_gather_mb_4bit", file = Some("layout/embed_gather.comp"), launch = kernels::LaunchRule::ElementwiseRows,
        operands = kernels::operands![
            w: Buf <- kernels::Source::Weight(0),
            scales: Buf <- kernels::Source::Weight(1),
            biases: Buf <- kernels::Source::Weight(2),
    ...
    kernel!(embed_gather_scaled_4bit "embed_gather_scaled_4bit",
        file = Some("layout/embed_gather.comp"),
    ...
    kernel!(ple_combine "ple_combine", file = Some("layout/ple_combine.comp"),
        launch = kernels::LaunchRule::Elementwise,
        operands = kernels::operands![
    ...
    kernel!(row_gather "row_gather", file = Some("layout/row_gather.comp"),
        launch = kernels::LaunchRule::ElementwiseRows,
        operands = kernels::operands![
            ...
            //
            // Metal read this as "append the count to the scalar run", because
            // there the packed slot IS the buffer. Vulkan binds the struct as a
            // real std430 buffer and sends scalars to a separate push block, so
            // there is nothing to append to: the driver writes the count into
            // buffer 3 while filling the struct, and the shader reads `p.count`.
            // `Binding::Packed` is what `bindings()` answers so that the count
            // cannot land in a push word no shader reads.
            params: Buf <- kernels::Source::Param(0),
