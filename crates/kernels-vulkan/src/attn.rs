...
const FILE: &str = "attn/sdpa_paged.slang";

pub(crate) const SDPA_TILE: u32 = 32;

const SDPA_WIDTHS: [u32; 4] = [64, 128, 256, 512];

pub const DECODE_THREADS: u32 = 256;
...
const SDPA_SPLIT_LSE: [&str; 4] = [
    "sdpa_paged_split_lse_bfloat16_d_64",
    "sdpa_paged_split_lse_bfloat16_d_128",
    "sdpa_paged_split_lse_bfloat16_d_256",
    "sdpa_paged_split_lse_bfloat16_d_512",
];

const SPLIT_FILE: &str = "attn/sdpa_split.slang";
const SPLIT_FOLD: &str = "sdpa_split_fold_bf16";
const SPLIT_FOLD_LSE: &str = "sdpa_split_fold_lse_bf16";

const SPLIT_FOLD_GROUP: u32 = 256;
...
pub fn splits_for(
    ...
    let groups = q_heads.saturating_mul(rows).max(1);
    if ceiling <= 1 || groups >= tuning.sdpa_split_groups {
        return 1;
    }
    let min_keys = tuning.sdpa_split_min_keys.max(1);
    let bound = window.map_or(keys, |w| w.min(keys));
    tuning
        .sdpa_split_groups
        .div_ceil(groups)
        .min(bound.div_ceil(min_keys))
        .clamp(1, ceiling.max(1))
}

#[allow(clippy::too_many_arguments)]
pub(crate) fn vector_split(
    ctx: &Ctx<'_>,
    op: &'static str,
    q: Tensor,
    pool: &KvPool,
    ...
    let slices = split_capacity(op, &split, &shape, head_dim, splits)?;
    ...
    args.push(split.part_out.arg_mut());
    args.push(split.part_lse.arg_mut());
    let walker = match lse {
        None => SDPA_SPLIT[shape.at],
        Some(lse) => {
            lse_plane(op, lse, &shape);
            args.push(lse.arg_mut());
            SDPA_SPLIT_LSE[shape.at]
        }
    };
    args.push(stated(op, slices)?.arg());
    args.push(stated(op, shape.rows)?.arg());
    args.push(stated(op, tuning.sdpa_split_min_keys.max(1))?.arg());
    ctx.fire(
        Fire::at(FILE, walker)
            .groups([shape.q_heads, shape.rows, slices])
    ...
    let mut fold: Vec<ArgValue> = vec![split.part_out.arg(), split.part_lse.arg(), o.arg_mut()];
    if let Some(lse) = lse {
        fold.push(lse.arg_mut());
    }
    fold.push(stated(op, slices)?.arg());
    fold.push(stated(op, shape.rows)?.arg());
    fold.push(stated(op, shape.q_heads)?.arg());
    fold.push(stated(op, head_dim)?.arg());
    ctx.fire(
        Fire::at(SPLIT_FILE, entry)
            .apply(Grid::of([o.width, shape.rows, 1], [SPLIT_FOLD_GROUP, 1, 1])),
        &fold,
    )
}

fn split_ceiling(split: &Split, rows: u32, q_heads: u32, head_dim: u32) -> u32 {
    let out_room = u64::from(split.part_out.rows) * u64::from(split.part_out.width);
    let lse_room = u64::from(split.part_lse.rows) * u64::from(split.part_lse.width);
    let out_slice = u64::from(rows) * u64::from(q_heads) * u64::from(head_dim);
    let lse_slice = u64::from(rows) * u64::from(q_heads);
    if out_slice == 0 || lse_slice == 0 {
        return 0;
    }
    let held = (out_room / out_slice).min(lse_room / lse_slice);
...
fn split_capacity(
    ...
    if split.part_out.dtype != Dtype::F32 || split.part_lse.dtype != Dtype::F32 {
        return Err(refuse(op, "the split arm's partial planes are f32"));
    }
    let rows = nonzero(op, "rows", shape.rows)?;
    let held = split_ceiling(split, rows, shape.q_heads, head_dim);
    if held == 0 {
        return Err(refuse(
            op,
            format!(
                "the partial planes hold no slice of this fire's {rows} rows x {} heads x                  {head_dim}",
                shape.q_heads
            ),
        ));
    }
    Ok(asked.clamp(1, held))
}
...
fn decode_arm(
    ...
    if let Some(planes) = split {
        let tuning = crate::tuning::current();
        let heads = q.width.checked_div(head_dim).unwrap_or(0);
        let ceiling = split_ceiling(&planes, q.rows, heads, head_dim);

        let keys = pool
            .page_indices
            .width
            .saturating_mul(u32::try_from(pool.page_size).unwrap_or(0));
        let slices = splits_for(heads, q.rows, ceiling, window, keys, &tuning);
        if slices > 1 {
            return vector_split(
                ctx, op, q, pool, plan, window, head_dim, sm_scale, o, lse, planes, slices, &tuning,
            );
        }
    }
    vector(ctx, op, q, pool, plan, window, head_dim, sm_scale, o, lse)
}
...
pub fn sink(
    ...
    ctx.fire(
        Fire::at("attn/attn_sink.slang", entry).apply(Grid::of(lanes, [64, 1, 1])),
        &[
            o.arg(),
            o.arg_mut(),
            lse.arg(),
...
pub fn logit_softcap(ctx: &Ctx<'_>, x: Tensor, cap: f32) -> Result<(), Error> {
    ...
    ctx.fire(
        Fire::at("attn/logit_softcap.slang", entry).apply(Grid::of(lanes, [256, 1, 1])),
        &[x.arg(), x.arg_mut(), cap.arg(), stated(OP, lanes[0])?.arg()],
    )
}
...
fn append_paged(
    ...
    ctx.fire(
        Fire::at("attn/kv_write.slang", entry).apply(Grid::of(lanes, [64, 1, 1])),
        &[
            k.arg(),
            v.arg(),
            pool.keys.arg_mut(),
...
pub mod mla {
    ...
    pub fn absorb_q(
        ...
        ctx.fire(
            Fire::at(FILE, entry)
                .apply(Grid::of([kv_lora_rank, heads, rows], [ABSORB_GROUP, 1, 1])),
            &[
                q_nope.arg(),
                kv_b.arg(),
                q_latent.arg_mut(),
    ...
    pub fn absorb_out(
        ...
        ctx.fire(
            Fire::at(FILE, entry).apply(Grid::of([v_head_dim, heads, rows], [ABSORB_GROUP, 1, 1])),
            &[
                latent.arg(),
                kv_b.arg(),
                o.arg_mut(),
    ...
    fn latent_strip(op: &'static str, what: &str, width: u32, max: u32) -> Result<i32, Error> {
        if width == 0 || !width.is_multiple_of(LANES) || width > max {
            return Err(refuse(
                op,
                format!(
                    "the {what} {width} is not one this kernel can lane-split \
                     (a nonzero multiple of {LANES}, at most {max})"
                ),
            ));
        }
        stated(op, width)
...
pub mod pool {
    ...
    const FILE: &str = "attn/pool.slang";
    ...
    pub fn state_write(
        ...
        ctx.fire(
            Fire::at(FILE, entry).apply(Grid::of([width, rows, 1], [LANE_GROUP, 1, 1])),
            &[
                kv.arg(),
                score.arg(),
                state_kv.arg_mut(),
    ...
    pub fn gather(
        ...
        ctx.fire(
            Fire::at(FILE, entry).apply(Grid::of([head_dim, rows, 1], [LANE_GROUP, 1, 1])),
            &[
                state_kv.arg(),
                state_score.arg(),
