...
//! as `Arg::Raised` and are built into views by `lowering::views`. The
...
//! table this driver stages the stream in. `resources::Model::named`
//! consults it FIRST and falls back to the seam stand-in, so seam values
//! keep the answer they always had and a runtime stream binds the pool's
//! own staged table instead of a stand-in of zeros.
...
///
/// Per plan, because value ids are the plan's own numbering: the decode and
/// prefill texts each mint their own.
...
/// [`crate::resources::Pool::stage`] fills every fire.
