...
//! sentences names a Metal type or a `wgpu` type, and the two drivers that had
//! written them independently came out 96% identical -- `frame.rs` at 100%, to
//! the character.
//!
//! So they are written once here, over two traits: [`Plane`], which is what a
//! fire is MADE OF, and [`Fires`], which is what happens when it fires.
//!
...
//! crate is that sentence with the word *dispatch* replaced by *executor*. The
//! four types that actually differ between the shader planes were abstracted on
//! the floor before this crate existed; what is added here is the fifth --
...
//! # What a driver keeps
//!
//! Five modules, and each one is kept because it DIFFERS rather than because it
//! was not reached:
//!
//! * `marks.rs` -- the `Slice` and the five mark constructors. A Metal region is
//!   an address and an extent; a wgpu region names its BUFFER, because a
//!   `wgpu::BufferBinding` is an object and two offsets and there is no address
//!   space to do arithmetic in. This crate takes the divergence as a parameter;
//!   it does not hold a copy of it.
//! * `stage.rs` -- what a fire stages. wgpu stages a decode-split partials plane
//!   and states a KV head count metal's pool does not carry.
//! * `views.rs` -- metal's `Pages` is the pool row; wgpu's is the pool row plus
//!   the five per-fire planes its sdpa arms read off the same object.
...
//!
//! `driver-cuda/src/baker/` is the reference both shader planes were written
//! from and it is not a member of this crate, on a measurement rather than a
//! preference: 527 of its 969 lines overlap (54%), and the 442 that do not are
//! the executor's spine -- a raw-pointer arena with no bindings table at all
//! (41 lines against 123 and 147), a `cudaMemcpyAsync` issued inside the
...
//! neither shader plane can declare. A [`Plane`] wide enough to hold cuda would
//! be a shape invented to fit rather than a shape both planes already had.
