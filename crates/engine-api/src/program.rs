...
//! lineage — owned types adopted directly; keep, purify"). An engine reading a
...
//! because the concept is the engine's — a library op, an emitted kernel's
...
/// written out because they are a wire numbering an engine reads, and a
...
/// An engine-side fast path a region admits: an argmax the backend can answer
...
///
/// Deliberately not PTIR: an engine reading this never sees a container, a wire
...
/// [`ExtentRole`]s; an engine carves each stage's fire-path buffers at BIND
