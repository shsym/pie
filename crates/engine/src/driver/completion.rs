...
//!
//! **THIS CODE IS A LIFT, NOT A REWRITE.** It stood in `driver-api` as
...
//! terminal cells and a per-work-item lease are how the ENGINE decides to run
//! ahead of a device, not part of the description of what a driver is
...
//!
//! * The terminal cell is local. It was `driver_api::local::TerminalCell`, a
//!   `#[repr(C)]` word a C driver published into across the ABI. Nothing
//!   crosses the boundary now — [`crate::driver::Driver::fire`] answers a
//!   `Result<FireTicket>` and the settle happens on this side of it — so the
//!   cell is the engine's private word and its `#[repr(C)]` is gone with the
//!   caller that needed it.
//! * [`settle`] is new: the one place a `Result<FireTicket, DriverError>`
//!   becomes a published outcome. A synchronous shell answers before the
//!   engine ever polls, so the cell is written here rather than by the
//!   driver.
...
///
/// **NEW IN THE LIFT, AND IT IS WHERE THE BOUNDARY MOVED.** A C driver used
/// to write these words itself: the engine handed `*mut TerminalCell` across
/// the ABI inside `StepSubmission` and the device's completion thread stored
/// into them. The contract has no such field any more —
/// [`Driver::fire`](driver_api::Driver::fire) answers a
/// `Result<FireTicket, DriverError>` and the shells in this workspace are
/// synchronous — so the engine writes them, once, from the answer it got.
///
/// Which outcome a caller passes is the whole of the run-ahead policy:
/// [`TERMINAL_OUTCOME_RETRY`] for a
/// [scheduling](driver_api::DriverError::is_scheduling) refusal, because the
