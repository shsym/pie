...
//!
//! It was 415 lines of adapter: a `CudaDriverHandle` per rank, an
//! `impl Driver for CudaDriver` forwarding fourteen verbs to a leader shell,
...
//!
//! * **The `Driver` impl is the shell's.** `driver_cuda::Cuda` implements the
...
//!   answers a typed `Fault` and `driver_cuda::api` maps it to `DriverError`
//!   once.
//! * **The broker is the engine's** (`crate::driver::completion`), so a
...
//!   [`LoadRequest`](driver_api::LoadRequest) is one plan, `Shard::Cut` is in
...
//! door (`crate::driver::load::contract_for`) it cannot state for itself.
...
/// [`Driver::load`](driver_api::Driver::load), not here: `Shell::load` is one
...
pub fn open(config_bytes: &[u8]) -> Result<Cuda> {
    ...
    Ok(Cuda::new(
        ...
        crate::driver::load::contract_for,
    ))
}
...
/// epilogue wrote.
///
/// Re-exported here rather than reached for directly because `driver-cuda` is
/// a private link of this crate — `_driver-cuda` is what gates it — and a
...
/// How much of a fire `[driver] graphs` asks to record.
///
...
fn graphs(doc: &toml::Table) -> Graphs {
    match doc
        .get("driver")
        .and_then(toml::Value::as_table)
        .and_then(|driver| driver.get("graphs"))
        .and_then(toml::Value::as_str)
    {
