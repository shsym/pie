...
//! model-gated ETA intrinsics — is [`eta_ir::registry::ModelProfile`],
//! which is the type `eta-compiler` binds a guest program against. The
...
//! [`eta_ir::registry::PortMask`], in the registry that owns the ports —
...
///
/// The record `load` answers with, and the engine's own account of itself: it
/// stays behind `Engine::load`, where its readers are. (`WorkerInfo` used to
/// ship it to the controller, which dropped it on arrival; the control plane
...
pub struct Capabilities {
    ...
    /// `eta_compiler::validate` takes, carried rather than reconstructed.
