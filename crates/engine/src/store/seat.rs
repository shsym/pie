//! Which pool slot a sequence sits in — the engine's half of
//! [`Lane::slot`](driver_api::fire::Lane).
//!
...
//! `driver_cuda::store::Pools::clear` zeroes when a fresh sequence arrives
//! (`palo` build log 19). `driver::fire`'s header states the same thing from
...
//! * **No two live sequences share one.** `FireSubmission::validate` refuses
//!   a fire where "slot N appears twice", by name, because two lanes seated
//!   together would write one another's cache.
//!
//! The engine's per-sequence identity is the KV working set: it IS the page
...
//!
//! A request is LANES, plural (`driver::fire`'s header): a beam fires B row
...
//! # The ceiling
//!
//! `capacity` is what the deployment's driver advertises as
//! [`PoolFacts::state_slots`](driver_api::caps::PoolFacts) — the same number
//! the contract calls [`Budgets::slots`](driver_api::load::Budgets) ("how
...
//! count because it has not asked one, and a deployment whose driver
...
mod tests {
    ...
    /// `FireSubmission::validate` refuses the absence of.
    #[test]
