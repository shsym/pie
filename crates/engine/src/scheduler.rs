...
fn rs_state_copy_plan(
    ...
    let slot_ranges = src_slots
        ...
        .zip(dst_slots)
        .map(
            |(src_slot_id, dst_slot_id)| driver_abi::PieStateCopyRange {
                src_slot_id,
                dst_slot_id,
                src_token_offset: 0,
                dst_token_offset: 0,
...
pub fn submit_async_with_kv_copy(
    ...
    let prelaunch_copy = (!copy_src.is_empty()).then_some(crate::driver::KvCopyPlan {
        src_domain: driver_abi::PIE_MEMORY_DOMAIN_CUDA_DEVICE,
        src_device_ordinal: 0,
        dst_domain: driver_abi::PIE_MEMORY_DOMAIN_CUDA_DEVICE,
