...
//!
//! Design §3's fallback menu has four entries and `engine::fire::walk` served
...
use engine::fire::{EventId, FireDescriptor, Lane, Sink, compose, fallback, walk};
...
/// count alone.
impl engine::fire::fallback::Serve for MockDispatch {}
...
fn the_grouped_arm_pays_one_launch_where_the_split_arm_pays_r() {
    ...
    // print macro, which `engine` denies in its tests.
    assert!(
        compared > 0,
