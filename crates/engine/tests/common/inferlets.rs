...
use engine::inferlet::process::ProcessId;
...
pub fn read_inferlet_manifest(name: &str) -> engine::inferlet::program::Manifest {
    let path = inferlets_dir().join(name).join("Pie.toml");
    let content = std::fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("Failed to read {}: {}", path.display(), e));
    engine::inferlet::program::Manifest::parse(&content)
...
pub fn wait_for_process(id: ProcessId, timeout: Duration) -> bool {
    ...
    loop {
        if !engine::inferlet::process::list().contains(&id) {
            return true;
        }
