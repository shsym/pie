...
impl Seats {
    ...
    pub fn write_routes(&self, seat_of: &[i32]) -> Result<()> {
        ...
        write_bytes(&self.routes, 0, &bytes)
    }
    ...
    pub fn gather(
        ...
        for &(kind, like, plane) in planes {
            ...
            let at = self.kind_off[kind.at()];

            stage_rows(staging, at, len.next_multiple_of(4), |base| {
                copy_rows(map, plane, unique, base)
            })?;
            let (_, slab) = self
                .slabs
                .iter()
            ...
            record_copy(frame, staging, at, slab, 0, len.next_multiple_of(4))?;
...
impl Pump {
    ...
    pub fn stage(
        ...
        let len = plane.bytes.next_multiple_of(4);
        stage_rows(staging, 0, len, |base| copy_plane(map, plane, base))?;
        let (slab, handle) = &self.slots[at];
        record_copy(frame, staging, 0, slab, 0, len)?;
...
#[cfg(feature = "wgpu")]
fn write_bytes(into: &Buffer, at: u64, bytes: &[u8]) -> Result<()> {
    into.slab().write(at, bytes)
}

#[cfg(not(feature = "wgpu"))]
fn write_bytes(_into: &Buffer, _at: u64, _bytes: &[u8]) -> Result<()> {
...
fn stage_rows(
    ...
    at: u64,
    len: u64,
    fill: impl FnOnce(usize) -> Result<()>,
) -> Result<()> {
    if len == 0 {
        return Ok(());
    }
    staging.span(at, len)?;
    let slab = staging.slab();
    let size = std::num::NonZeroU64::new(len).expect("a non-empty stage");
    let mut view = slab
        .core
        .queue
        .write_buffer_with(&slab.buffer, at, size)
        .ok_or(Fault::Device {
            call: "write_buffer_with",
            why: "no staging view".into(),
        })?;
...
fn stage_rows(
    ...
    _at: u64,
    _len: u64,
    _fill: impl FnOnce(usize) -> Result<()>,
) -> Result<()> {
    Err(Fault::Deviceless)
}
...
impl Gathered {
    ...
    pub fn stage(
        ...
        handles: &Handles,
        host: &HostTier,
        bank: kernels_wgpu::Bank,
        ids: Tensor,
    ) -> Result<Option<(kernels_wgpu::Bank, Tensor)>> {
