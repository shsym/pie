...
//! range. Every engine in this repo ignored that table and treated a
...
//! `model_exec::fire::walk` implements: the window becomes a list of spans
...
/// `engine_cuda::Fault::Fragmented` keeps, narrowed to it.
///
...
/// equal is not a hope: `engine_cuda::window::no_schedule_straddles_its_readers`
...
/// kernel no `kernels-*` library publishes an entry for. The walk knows WHEN a
/// copy is owed (P4's table, read by [`copies`]) and nothing about how to pay
/// one, so the two halves meet here.
///
...
///
/// **THE ONE QUESTION `model_exec::fire::walk` ASKS ABOUT A FALLBACK KIND**, and
...
/// the same breath as everything else in `engine_cuda::inputs`.
///
