...
///
/// * `file/` — the reader. Turning a directory into a `Metadata` is precisely
///   the step this test exists to keep separate, so it has to live somewhere,
///   and it lives in one module.
...
fn nothing_below_the_reader_opens_a_file() {
    ...
    for (rel, body) in sources() {
        ...
        if ALLOWED.contains(&rel.as_str()) || rel.starts_with("file/") || rel.ends_with("tests.rs")
        {
...
/// `golden_plans.rs` are all built from synthetic `Metadata`, and this
...
fn the_backend_lowering_reads_its_numbers_off_the_target() {
    ...
    for (line_no, line) in production_lines(tile) {
        ...
        //   one of those, and it is checked from the other end, in the crate
        //   that launches them: `engine-cuda`'s `weights::arena` binds each
        //   string beside the `kernels_cuda::linear::quant` path it names.
