...
fn workspace_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("crates/checkpoint is two levels below the workspace root")
        .to_path_buf()
}
...
/// `crates/checkpoint/src/plan/passes/tile.rs` by suffix.
...
///
/// `crates/checkpoint`'s two trees share one home: a comment in its `src/`
...
///
/// The rot this was written against was not a `checkpoint` habit, and the
...
/// gets disabled.
const TREES: &[(&str, &str)] = &[
    ("crates/checkpoint/src", "crates/checkpoint/"),
    ("crates/checkpoint/tests", "crates/checkpoint/"),
    ("src", "src/"),
    ("tests", "tests/"),
];
...
const DESCRIPTIONS: &[(&str, &str)] = &[
    ("crates/checkpoint/README.md", "crates/checkpoint/"),
    ("crates/checkpoint/Cargo.toml", "crates/checkpoint/"),
];
...
fn every_cited_workspace_crate_exists() {
    ...
    //
    // THIS CRATE'S OWN NAME IS NOT ON THE LIST AND CANNOT BE. It was
    // model-loader, matched the model-* family, and was checked here like
    // any other package; it is `checkpoint` now, which is a common noun this
    // tree writes in prose constantly — a load request carries one, an
    // engine enum is named for one, and a local variable in three crates is
    // spelled one. A rule that read every backticked `checkpoint` as a package
    // reference would report nothing but false positives, so the rename costs
    // this check its coverage of exactly one package. The first rule still
    // covers the crate's FILES, which is where the rot was.
    const FAMILIES: &[&str] = &[
        "client-",
        "controller-",
        "engine-",
