...
fn llama_checkpoint(layers: usize) -> Metadata {
    ...
    let mut push = |name: String, shape: Vec<i64>| {
        ...
        let span_bytes = u64::try_from(elements).unwrap() * DType::Bf16.bytes_ceil();
...
fn histogram(plan: &LoadPlan) -> BTreeMap<String, u64> {
    let stats: serde_json::Value =
        serde_json::from_str(&checkpoint::dump::plan_stats_json(plan)).expect("stats are JSON");
