...
impl Checkpoint {
    ...
    fn push(&mut self, name: &str, shape: &[i64], encoding: Encoding) -> &mut Self {
        ...
        let span_bytes = match &encoding {
            Encoding::Raw(dtype) => u64::try_from(elements).unwrap() * dtype.bytes_ceil(),
            Encoding::Quant(spec) => {
    ...
    fn finish(self, name: &str) -> Metadata {
...
fn qwen3_moe_checkpoint() -> Metadata {
...
fn gpt_oss_checkpoint() -> Metadata {
...
fn gpt_oss_native_checkpoint(intermediate: i64) -> Metadata {
...
fn check_stats_render(name: &str, plan: &LoadPlan) {
    let summary = checkpoint::dump::describe(plan);
    assert!(
    ...
    let rendered = checkpoint::dump::plan_stats_json(plan);
...
fn check(name: &str, metadata: &Metadata, target: StorageTarget) {
...
fn replay(name: &str, plan: &LoadPlan, metadata: &Metadata) {
    if plan.target.tile_map_mask & !HOST_TILE_MAP_MASK != 0 {
        return;
    }
    let snapshot = PathBuf::from(&metadata.files[0].path);
    let storage = checkpoint::executor::Execution::new(plan, snapshot.parent().unwrap())
        .run()
    ...
    for tensor in &plan.tensors {
        ...
        let expected = checkpoint::types::encoding_nbytes(&tensor.shape, &tensor.encoding)
