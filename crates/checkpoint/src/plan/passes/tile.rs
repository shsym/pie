...
/// toolchain.
///
/// What keeps them honest is the other side, and it is in another crate now:
/// `engine-cuda`'s `weights::arena` calls exactly these as the typed
...
/// The device half used to sit in this crate behind a `cuda` feature; moving
/// it is what let this crate stop naming `kernels-cuda` at all, and the check
/// is unchanged because it never depended on the two being co-located — only
/// on someone compiling the call beside the constant.
///
...
/// vocabulary — the word a plan carries from `tile` to the arena's dispatch,
/// and nothing wider.
///
...
/// `engine-cuda`'s `weights::arena` has a test that binds each path beside
/// each string, and is where the two are held together.
///
...
///
/// **`BIAS` IS HERE FOR THE REASON `DECODE` IS**, and it was missing for the
/// reason `DECODE` used to be. A device mask says which transforms a plan for
/// that backend may CARRY, not which its kernels run; `engine-metal` runs
/// none of them on the device, so every `Bias` in a Metal plan folds its
/// constant on the HOST, streaming into the arena, exactly as the `Scale` and
/// `Cast` beside it already do. The host executor has implemented it since
/// the node existed ([`HOST_TILE_MAP_MASK`] carries the bit and
/// `executor::walk::bias_bytes` is the code), so what the omission bought was
/// not safety: it made [`Expr::Bias`](crate::contract::Expr::Bias) — the node
/// whose entire purpose is reconciling a converter that folded a constant
/// into a weight with an engine whose kernel adds it — unreachable from every
/// device plan, which is every plan that matters.
///
...
mod mask_tests {
    ...
    /// statement now.
    ///
    /// What it buys: turning the device path off — an arena the shell built
    /// with device transforms disabled, a build of a shell that has no device
    /// executor at all, or a plan that named no kernel for an instruction —
    /// always lands somewhere that can finish the load. It is
