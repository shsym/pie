...
Typed checkpoint compiler: a checkpoint and a contract in, a verified memory
layout a device can be pointed at out.

It was called model-loader, and the name claimed more than the code does: a
model loader would know model semantics, and nothing here knows a family —
`tests/standalone.rs` is that refusal written as four properties. What it knows
is a checkpoint.

```text
checkpoint  ──materialize──▶  model contract  ──compile──▶  load plan  ──execute──▶  arena + sink
(its own        (what to produce,       (the bytes to move,      (`executor`, into
 metadata)       as expressions over      where, in what           whatever the caller
                 the checkpoint)          order)                   supplies as an
                                                                   `ArenaBacking`)
```
...
`CudaArena` that ran the load transforms on the device; it is `engine-cuda`'s
`weights::arena` now, and nothing that remains names a CUDA crate. The
dependency set is `dtype`/`half`/`ztensor`/`serde`/`thiserror` unconditionally,
so the crate builds and its whole suite runs on a machine with no toolkit —
which is what keeps the host executor the reference the device answer is
checked against. The comparison itself moved with the arena and still reaches
back here for that reference; see the feature's epitaph in `Cargo.toml`.
...
one is gone from `file::meta` for the same reason.
