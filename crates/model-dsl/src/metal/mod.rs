...
/// The GENERATED wrappers — one named `pub fn` per traced `#[routine]` in
/// `crates/kernels-metal/src`, the same contract as [`crate::cuda::generated`]
/// (design-no-ask §10, B4-gen step 6). What remains hand-written in this
/// module is each a decision a signature cannot state: an instantiation
/// point composing the symbol (`qmv`, `qmm`, `sdpa`, the gathers), a
/// guard/region form (`cast_qmm_input_when`), a chooser fanning one call
/// onto several routines (`rope`, `kv_append`, `router_topk`), or a spliced
/// fire extent the rows convention does not claim (`route_sort`,
/// `sample_rows`). `tests/wrappers_are_current.rs` pins both halves: every
/// traced routine has a generated fn, and no hand form here states a symbol
/// a generated fn already states unless it is allowlisted with its reason.
pub mod generated;

// The shared ruled-result evaluation, re-exported so `generated.rs` reaches
// it as `super::ruled_out` — the same path `cuda/generated.rs` uses.
...
///
/// A caller that states this must not also state a norm (`generated::rms_single_row`) or [`rope`] for
...
pub fn rms_rope(
    ...
    let out = with_extents(
        ...
        // and carry the same readings the deleted `rms_norm_gain` explained:
        ...
        vec![
            ...
            scale.to_bits(),
            theta.log2().to_bits(),
            // `rows`, a fire extent the lowering splices -- the arm's last
            // scalar, and a placeholder here for the same reason `rope`'s is.
            0,
        ],
        vec![(9u8, Shape(vec![rows_of(x)]))],
        vec![
            // The operands the marks claim, in order: the tensor rotated in
            // place, then the per-token positions the rotation reads. The
            // stream is the runtime vocabulary's and the driver stages it per
            // fire; the ROPE half of this fusion needs it exactly as `rope`
            // does, and a statement that places nine scalars and one operand
            // against an arm that reads ten and two disagrees about every
            // position after the first.
            x.id,
            runtime::positions(&x.t).id,
        ],
...
pub fn sdpa(
    ...
    let (params, inputs) = if paged {
        ...
        // The decode split policy — the driver's judgement, carried as an
        // operand ONLY on the decode rows, which are the ones that mark it:
        // the tiled/mma prefill rows read five operands, and placing a
        // sixth had every multi-batch metal prefill refused by check_plan
        // at trace time (the operand count is the signature's).
        if !multi_batch {
            inputs.push(split_policy(&q.t).id);
        }
        (
            vec![
                kv_heads,
...
pub fn route_sort(
    ...
    let ids = expert_ids.t.with(expert_ids.layer, |b| {
        b.launch_with_extents(
            ...
            vec![0, n_experts, experts_per_token, block, 0, width, width],
...
pub fn route_gather(
    ...
    with_extents(
        ...
        vec![0, n_experts, experts_per_token, block, 0, width, width, 0],
        vec![
...
/// `ssm/gdn_prep.metal::gdn_prep_prefill_bfloat16` — the prep (`generated::gdn_prep_slotted`) over a
...
/// # The four results
///
/// Three more than the slotted prep's three, in one: `pre_gate` carries the
...
/// `generated::gdn_core_recurrent_slotted`, the slotted form, reads `conv_w` and `conv_b`
