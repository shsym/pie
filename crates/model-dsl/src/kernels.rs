...
pub fn rmsnorm_no_scale(x: &Value, head_dim: u32) -> Value {
    x.stmt("rmsnorm.no_scale").int(head_dim).done()
}

pub fn rmsnorm_gated<W: Dtype>(x: &Value, n: &Norm<W>) -> Value {
    x.stmt("rmsnorm_gated").norm(n).done()
}

pub fn rope(q: &Value, k: &Value, head_dim: u32, theta: f32, pos: &Value) -> (Value, Value) {
    q.stmt("rope").value(k).value(pos).int(head_dim).float(theta).pair()
}
...
pub fn attention_landing<W: Dtype>(a: &Value, o_proj: &Tensor<W>, layer: u32) -> Value {
    a.stmt("attention_landing").weight(o_proj).layer(layer).done()
}

pub fn all_reduce(x: &Value) -> Value {
    x.stmt("all_reduce").done()
}
...
pub fn matmul_select_bias<W: Dtype, B: Dtype>(
    ...
    x.stmt("matmul_select.bias").value(routes).weight(bank).weight(bias).done()
}

pub fn weighted_sum(routed: &Value, routes: &Value) -> Value {
    routed.stmt("weighted_sum").value(routes).done()
}

pub fn sigmoid_gate_add(routed: &Value, shared: &Value, gate: &Value) -> Value {
    routed.stmt("sigmoid_gate_add").value(shared).value(gate).done()
}

pub fn sigmoid_gate_mul(x: &Value, gate: &Value) -> Value {
    x.stmt("sigmoid_gate_mul").value(gate).done()
}
...
pub fn gdn_prep<W: Dtype>(ba: &Value, dt_bias: &Tensor<W>, a_log: &Tensor<W>) -> Value {
    ba.stmt("gdn_prep").weight(dt_bias).weight(a_log).done()
}
...
pub fn mla_latents<W: Dtype>(kv_a: &Value, norm: &Norm<W>, kv_lora_rank: u32) -> (Value, Value) {
    kv_a.stmt("mla_latents").norm(norm).int(kv_lora_rank).pair()
}
...
pub fn split_q_b(q_b: &Value, heads: u32, nope_dim: u32, rope_dim: u32) -> (Value, Value) {
    q_b.stmt("split_q_b").int(heads).int(nope_dim).int(rope_dim).pair()
}

pub fn kv_append_mla(kv_c: &Value, k_pe: &Value, pages: &Pages) {
    kv_c.stmt("kv_append.mla").value(k_pe).cache(&pages.name).effect();
}
...
pub fn mla_absorb_q<W: Dtype>(
    ...
    q_nope.stmt("mla_absorb.q").weight(kv_b).int(heads).int(kv_lora_rank).int(nope_dim).done()
}
...
pub fn mla_absorb_out<W: Dtype>(
    ...
    latent.stmt("mla_absorb.out").weight(kv_b).int(heads).int(kv_lora_rank).int(v_head_dim).done()
}
...
pub fn mla_attention_decode_selected(
    ...
    q.stmt("mla_attention.decode_selected")
        .value(selection)
        .cache(&pages.name)
        .int(heads)
        .int(kv_lora_rank)
...
pub fn mla_attention_prefill_selected(
    ...
    w.data
        .stmt("mla_attention.prefill_selected")
        .value(&w.indptr)
        .value(selection)
        .cache(&pages.name)
        .int(heads)
...
pub fn kv_append_index(k: &Value, keys: &Pages) {
    k.stmt("kv_append.index").cache(&keys.name).effect();
}

pub fn index_rope(q: &Value, heads: u32, head_dim: u32, rope_dim: u32, theta: f32, pos: &Value) -> Value {
    q.stmt("index_rope").value(pos).int(heads).int(head_dim).int(rope_dim).float(theta).done()
}
...
pub fn kv_append_shared(plane: &Value, pages: &Pages) {
    plane.stmt("kv_append.shared").cache(&pages.name).effect();
}

pub fn lse_ln(lse: &Value) -> Value {
    lse.stmt("lse_ln").done()
}

pub fn pool_boundary_decode(pos: &Value, ratio: u32) -> (Value, Value) {
    pos.stmt("pool_boundary.decode").int(ratio).pair()
}

pub fn pool_boundary_prefill(w: &Windows, ratio: u32) -> (Value, Value) {
    w.data.stmt("pool_boundary.prefill").value(&w.indptr).int(ratio).pair()
}

pub fn pool_gather(bpos: &Value, breq: &Value, pages: &Pages, head_dim: u32, ratio: u32) -> Value {
    bpos.stmt("pool_gather").value(breq).cache(&pages.name).int(head_dim).int(ratio).done()
}

pub fn kv_append_pool(pooled: &Value, bpos: &Value, breq: &Value, entries: &Pages) {
    pooled.stmt("kv_append.pool").value(bpos).value(breq).cache(&entries.name).effect();
}
...
pub fn hc_expand(x: &Value, streams: u32) -> Value {
    x.stmt("hc_expand").int(streams).done()
}

pub fn hc_rmsnorm_f32(streams: &Value, eps: f32) -> Value {
    streams.stmt("hc_rmsnorm_f32").float(eps).done()
}
...
pub fn hc_fold(x: &Value, streams: &Value, post_mix: &Value, comb_mix: &Value) -> Value {
    x.stmt("hc_fold").value(streams).value(post_mix).value(comb_mix).done()
}
