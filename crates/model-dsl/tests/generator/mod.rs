...
pub fn generate(kernels_src: &Path) -> String {
    ...
    for file in &files {
        ...
        let rel = file.strip_prefix(kernels_src).expect("file is under the walked tree");
    ...
    for spec in specs {
        ...
        let resolved = match &spec.rule {
            ...
            RuleExpr::Shaped { rows_of, width } => {
                ...
                let w = match width {
                    WidthExpr::Half(a) => {
                        Some(format!("OutWidth::Half {{ of: {} }}", find(&inputs, a, "input")))
                    }
                    WidthExpr::Of(a) => {
                        Some(format!("OutWidth::Of {{ of: {} }}", find(&inputs, a, "input")))
                    }
                    WidthExpr::Weight(w) => {
...
fn parse_spec(attr: &syn::Attribute, file: &Path) -> Spec {
    match &attr.meta {
        ...
        syn::Meta::List(list) => syn::parse2::<Args>(list.tokens.clone())
            .unwrap_or_else(|e| panic!("parsing #[routine(..)] in {}: {e}", file.display()))
            .0,
        syn::Meta::NameValue(_) => {
            panic!("#[routine = ..] in {} is not a form the macro takes", file.display())
        }
    }
}
...
fn emit(out: &mut String, r: &Routine) {
    ...
    for p in &r.params {
        ...
        let slot = match &p.mark {
            Mark::InOut | Mark::Out { .. } => {
                out_slots.iter().find(|s| s.name == p.name)
            }
            _ => None,
        };
    ...
    let ret = match out_slots.len() {
        ...
        1 => format!(
            " -> {}",
            if out_slots[0].optional { "Option<Val>" } else { "Val" }
        ),
        _ => format!(
            " -> ({})",
            out_slots
    ...
    if needs_trace_param {
        ...
        let _ = writeln!(out, "    let t = {}.t.clone();", ident(&required_inputs[0].name));
    }
    ...
    if scalars.is_empty() {
        ...
        let items: Vec<String> = scalars
            .iter()
            .map(|p| {
                let Mark::Scalar(s) = p.mark else { unreachable!() };
    ...
    if out_slots.is_empty() {
        ...
        let items: Vec<String> = out_slots
            ...
            .map(|s| match (&s.rule, &s.shape_param) {
                (Some(rule), _) => format!(
                    "ruled_out(&t, \"{symbol}\", {rule}, &run_inputs, &run_params)"
                ),
    ...
    match out_slots.len() {
        0 => {
            let _ = writeln!(out, "    assert!(made.is_empty(), \"`{symbol}` states no result\");");
        }
