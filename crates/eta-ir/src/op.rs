//! The ETA op set — the closed first-party core plus the
...
//! space; tag `0x80` (`Input`) is *reserved-unused* — ETA stage bodies have no
...
/// An ETA stage-body op. See the module docs for the SSA model and the
...
pub enum Op {
    ...
    /// logits[j] : -inf`; `mask` `[ceil(n/32)]` U32. (The ETA-level
...
///
/// The question is "what DECIDES this value", asked so a partial evaluator can
/// tell whether the host could have computed it. `eta-compiler`'s launch
/// package asks a different question with a similar-sounding name — which
/// trace construct DEFINED an SSA value, a wire numbering an engine reads
/// positionally — and spells its answer `ValueOrigin`. The two share no
/// variant and no conversion; they were both called `ValueSource` while they
/// lived in crates that never met.
...
macro_rules! declare_ops {
    ($($konst:ident = $tag:literal, $name:literal, $family:ident, $operands:expr, $results:expr,
       $rep:expr, $pat:pat, [$($wire:ident),*];)*) => {
        /// The wire tag of every op, by name. **The only place an ETA op tag
        /// is spelled as a number.** Downstream crates (`eta-compiler`,
        /// `eta-compiler`, and engines via the generated header) import these
