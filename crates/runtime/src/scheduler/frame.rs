...
//!   the run-ahead depth. There is no launch-time barrier: the engine's
...
//! engine lane, and drives this through the `on_*` bookkeeping calls plus
...
///
/// **AND SINCE alto F2b IT IS A REAL BOUND** rather than an enqueue horizon.
/// A dispatched frame used to retire the moment `submit` answered, because
/// the shells settled inside it; the CUDA shell answers with the device still
/// running now, and the batch retires on the frame's completion callback. So
/// this number is what actually limits how far ahead of the device the runtime
/// runs, and it is the same number the engine carves its staging ring from
/// (`engine::runahead::Runahead`, `LoadRequest::frames_in_flight`).
///
/// The bounds it has to clear are enforced by
/// `worker::config::RuntimeConfig::validate()`, where both this and
/// `frame_size` are visible at once and where the formula's own module is
/// read rather than a constant transcribed from it.
...
///
/// In the wait-all gate, do not count a lane as missing when the runtime still
...
struct LaneState {
    ...
    served: bool,
    /// When this lane's last frame retired, i.e. when the runtime handed its
    ...
    /// Cleared by any accepted fire and by any debt the runtime owes the lane;
...
impl LaneState {
    ...
    /// `owes` is the runtime's debt to this lane (an unretired dispatch or a
...
pub(super) struct FramePolicy {
    ...
    /// closes while bring-up owns the engine lane); binds do NOT hold the
    ...
    /// which opens the cohort-boundary window. Multi-engine note: consume/release broadcasts reach
    /// every engine's policy, and it is the GLOBAL admission semaphore
    /// (one pool across engines) that bounds outstanding consumes to
    ...
    /// Lanes with fires the runtime has dispatched but not yet retired: the
    /// runtime owes them a result, so the submit deadline's clock must not run
    ...
    /// are behaving, since they cannot submit until the runtime answers.
    ...
    /// lane's clock is reset on every tick forever. The runtime then owes a
...
impl FramePolicy {
    ...
    /// Whether the runtime still records an unretired dispatch for `lane`.
    ...
    /// Override the submit deadline. Tests only — in a live runtime the value
    ...
    /// `runtime_debt_suspends_the_deadline_clock`) and a bare default flip
    ...
    /// A dispatched batch settled: the runtime has paid these lanes back, so
    ...
    /// and a live one is charged for runtime latency. Re-arming from each
    ...
    /// Guarded on `staged`: only a process that bound on THIS engine can
    /// fire here.
    ...
    /// teardown closes so fresh-lane bring-up owns the engine lane.
    ...
    /// window is for is that the runtime is visibly mid-handover, which is the
    ...
    pub fn on_process_leave(&mut self, owner: ProcessId) {
        ...
        for id in owned {
            ...
            // process the runtime genuinely still owes.
    ...
    /// the engine), and the device-side readiness gate (`pass_commit`
...
ready_age_newest={}us",
        ...
        loop {
            ...
            // back so a queued bind reaches the engine while this boundary is
            ...
            //
            // A cohort boundary elsewhere in the runtime used to stop every
            ...
            for (lane_id, lane) in self.lanes.iter_mut() {
                ...
                // silence timeout -- because this is a backstop against a
                // runtime that will never answer, not a bound on how long a
                ...
                // relaxation skips a lane precisely because the runtime owes it
...
mod tests {
    ...
    /// without retiring leaves the runtime permanently owing, so the deadline
    ...
    fn dispatch_depth_defaults_when_bootstrap_installs_nothing() {
        ...
        // hand back a usable one: an engine harness that never boots the worker
    ...
    fn an_abandoned_lane_is_terminated_at_the_silence_timeout() {
        ...
        // silent. (A runtime with nothing at all to do parks and reaps
    ...
    fn a_lane_awaiting_its_own_result_is_never_charged_for_the_wait() {
        ...
        for elapsed in [Duration::ZERO, deadline, deadline * 100] {
            ...
            assert!(
                !policy.leashed(a),
                "a is waiting on the runtime, not the other way round"
            );
        }
    ...
    /// ...but the debt itself has a deadline. A wave the engine never retires
    ...
    /// The runtime's own debts hold the clock: a lane whose rebind the runtime
    /// has not finished cannot submit, so the deadline must not run against
    /// it. Without this the leash would drop guests for runtime latency, and
    /// the silence timeout would eventually kill them for it.
    #[test]
    fn runtime_debt_suspends_the_deadline_clock() {
        let deadline = Duration::from_millis(50);
        ...
        // a is mid-rebind: the runtime owes it, so its silence is not a breach.
        ...
        assert!(
            ...
            "the runtime, not the guest, is why a has not submitted"
        );
    ...
    /// `a` cannot: the runtime owes it the result of the frame still on the
    ...
    /// A debt the runtime will never answer must not hide behind the
    /// exemption. The relaxation skips a lane BECAUSE the runtime owes it a
    ...
    /// runtime having answered it in between.
    #[test]
    ...
    /// against a process the runtime genuinely still owes.
    #[test]
