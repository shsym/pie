...
//! (`batch::planned_prefix_wire_rows`) and crossed to the engine as
//! `planned_hook_free_prefix_rows`, where it feeds the declared Peel's
//! split after a cross-check against the engine's compiled-plan
...
//! [`expert_weights_site`] still say what such a site is, so the day an engine
//! reports one there is a shape for it to land in. What does not survive is a
//! derivation in the past tense.
//!
//! [`EngineSpec`]: crate::engine::EngineSpec
...
pub(crate) enum Lowering {
    ...
    /// attribution CSR and crosses the result to the engine
    ...
    /// This is Stage 4's per-adapter lora correction in the engine. The
    /// engine additionally groups SAME-SHAPE lanes into one grouped-GEMM
    ...
    /// `Prefix::fast_rows` is re-derived engine-side.
    PerLane,
...
/// hooks-last member sort plus the engine's hook-free fast prefix
/// (`StageHooks::hook_free_prefix_rows`); the engine will eventually consume
...
/// operator, per-member weights. The engine applies corrections by span
...
///
/// NOTHING EMITS ONE TODAY. The designed source is the engine, which holds the
/// validated plan and would report its site summary through the capabilities
/// handshake; the module doc measures why the runtime-side walk that used to
...
/// The [`SITE_EXPERT_WEIGHTS`] vocabulary entry, as an engine-reported summary
/// would land in it.
///
...
/// those strategies is the runtime/engine's device-knowledge call, exactly
...
/// the form for callers with no model site source (tests, and any engine
...
pub(crate) fn plan_fire_with_model(members: &[MemberFacts], model_sites: &[Site]) -> FirePlan {
    ...
    let qkv_postprocess = if hook_members == 0 {
        ...
        // exactly what the engine derives for such a step today.
...
mod tests {
    ...
    fn device_geometry_members_are_forced_last() {
        ...
        // mirrors today's engine derivation exactly (min row start over
