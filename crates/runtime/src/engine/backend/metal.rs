...
//!
//! It was a door onto a shell that did not exist: a `MetalEngine` this crate
//! defined itself, one field (`[model] id`, carried so the refusals could
//! name it), and every verb of the contract answering
//! `Error::Unsupported`. Its own header said what was missing —
//! *"a device to bind, a checkpoint to land, pools to reserve, and a command
//! buffer to encode onto"* — and named `engine-cuda/serve.rs` as the shape
//! it would take.
//!
//! It took that shape. `engine-metal` is the whole shell now: `device/`,
...
//! **the `Engine` impl is the shell's**, in the crate that owns the device,
...
//! shell the load door (`crate::engine::load::contract_for`) it cannot state
...
use engine_metal::{DeviceBoot, Metal};
...
/// happens at [`Engine::load`](engine_api::Engine::load), not here:
...
pub fn open(config_bytes: &[u8]) -> Result<Metal> {
    ...
    Ok(Metal::new(DeviceBoot::default(), crate::engine::load::contract_for))
}
