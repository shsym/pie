...
//! the engine lowering (`rs_slot_ids`, `rs_slot_flags`, pre-launch d2d copy)
...
impl PreparedRs {
    ...
    /// a parallel arm of `LaunchPlan` and travelled to the engine; the palo
    ...
    /// submission. So the runtime still computes it (its own store is built on
    /// it, and `copy_state` moves what it names) and `crate::engine::RsPlan`
    /// is where it stands.
    pub fn apply_to(&self, request: &mut crate::engine::FireRequest) {
        request.rs = crate::engine::RsPlan {
...
fn prepare_many_impl(
    ...
    for (index, &ws) in working_sets.iter().enumerate() {
        ...
        // selects the engine's replay mode, and it must be uniform across
