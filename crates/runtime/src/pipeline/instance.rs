...
//! allocated engine-side from the declared shapes; the host instance carries the
...
/// "no instance" sentinel). Each `instantiate` mints a fresh id: the engine
...
pub struct Instance {
    ...
    /// This instance's identity — the engine's channel-arena cache key (stable
...
impl Instance {
    ...
    /// and the engine fills those descriptor ports itself. Sufficient to
    ...
    /// value isn't host-known — the signal that the geometry needs engine / ws /
...
pub struct ForwardBindings {
    ...
    /// ON DEVICE and only the engine ever resolves it. The host then plans
...
/// copy — which the engine then checked and refused on disagreement. Registry
...
/// hash-deduped registered program. The engine's persistent channel arena is keyed by this pass's
/// `instance_id`; a channel MAY bind to several passes (multi-pass channels,
/// W3.2) — the engine's global channel registry (W0.1) resolves one shared
...
pub struct BoundForwardPass {
    ...
    /// per-token submits never re-enter the global engine-handle registry.
    ...
    /// fire's [`crate::engine::LaunchPlan`].
    pub max_layers: Option<u32>,
    ...
    /// double-closing the engine instance or double-detaching a cell.
...
impl BoundForwardPass {
    ...
    /// Idempotent ordered native teardown: enqueues the bound engine's close,
    ...
    /// at least one op is still awaiting its engine completion and hasn't
    ...
    /// race a still-live engine write against page reuse (a use-after-free).
...
mod tests {
    ...
    fn fire_geometry_prefills_request_from_seed() {
        ...
        // positions/readout) prefills a LaunchPlan — no engine/ws needed.
        let prog = register(
            greedy().encode(),
            &ModelProfile {
                vocab: VOCAB,
    ...
    /// interpreter (the mock engine) → golden token. Proves the runtime
    ...
    fn register_instantiate_run_on_mock_interp() {
        ...
        // Run the bound trace on the mock engine (echo's interp) with the same
