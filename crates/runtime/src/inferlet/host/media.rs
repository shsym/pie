...
//! inferlet-side `from_pixels`/`from_mel` produced, so the bridge + engine are
//! unchanged — only the *source* of the bytes moved host-side. See MULTIMODAL.md.
//!
//! The wire-payload fields (`pixels`/`positions`/`patch_grid`/`mel`/`n_frames`/
//! `uses_mrope`) are populated here but read by the not-yet-landed host->engine
...
pub struct Image {
    ...
    /// Pre-merge `(t, h, w)` patch-unit grid for the engine's vision encoder
