...
pub(crate) fn defer_resource_teardown(
    ...
    let task = async move {
        if capped_execution {
            ...
            // every engine's scheduler has purged the pid's queued work and
        ...
        // engine instead of one mailbox item per channel (a teardown herd
        ...
        // then keeps the engine's instance-before-channel close order.
