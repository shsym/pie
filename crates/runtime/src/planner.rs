...
pub enum StarveCause {
    /// The engine advertises no KV swap transport, or the host swap pool has
...
struct Proc {
    ...
    /// execution gate: "everything that creates per-instance engine state or
...
impl ResidencyPlanner {
    ...
    pub(crate) fn acquire_or_enqueue(
        ...
        match parked {
            ...
            Parked::Entry(key, notify) => {
                ...
                // next accepted fire — for a deferred frame, the runtime
    ...
    fn plan_eviction(self: &Arc<Self>) {
        ...
        // out: a swap-incapable engine or an exhausted host pool leaves the
