...
//! # What this is for
//!
//! `engine-cuda/tests/program_parity.rs` proves the guest-program plane
//! computes the right thing: both halves of `engine::program` — the host
//! interpreter and the CUDA one — run the same golden traces over the same
//! inputs and must agree byte for byte. What it drives is an
//! [`engine_cuda::program::Plane`] with a hand-staged f32 buffer standing in
...
//!   Engine::load(request)         -> a real checkpoint on the device
//!   Engine::register_program      -> the corpus golden, compiled by NVRTC
//!   Engine::bind_instance         -> rings carved, seeds planted, extents stated
//!   Engine::publish_channel       -> the host half of the channel join
//!   Engine::fire(.. attachments)  -> forward, then the guest pass on its logits
//!   Engine::take_channel          -> what the guest published
//! ```
//!
...
//!   cargo test -p runtime --features engine-cuda-13 --test cuda_program_epilogue -- --nocapture
//! ```
...
use engine::engine_api::program::{LaunchChannel, LaunchPackage};
...
use engine_api::{
    Attachment, BindExtents, Boundary, Budgets, Error, FrameSubmission, InstanceBinding, Lane,
    Readout, Step,
};
...
/// `cuda_boot_smoke` and `engine-cuda/tests/serve_smoke.rs` pin. Three paths
...
fn program(vocab: u32) -> TraceContainer {
    TraceContainer {
        ...
        channels: vec![
            ...
            // engine asks before it launches anything.
            ChannelDecl {
...
/// The gate's program, all the way to what `Engine::register_program` takes.
///
/// The emitted kernels are attached HERE rather than by
/// `pipeline::program::with_host_codegen`, which is the scheduler lane's
/// splice: this test holds the engine directly, so it plays that part too.
fn registration(
    profile: &engine::tensor_ir::registry::ModelProfile,
...
fn the_gates_program_binds_at_a_real_vocabulary() {
    let profile = engine::tensor_ir::registry::ModelProfile {
        vocab: 151_936,
        ..engine::tensor_ir::registry::ModelProfile::dummy()
    };
...
fn a_guest_program_fires_at_the_boundary_of_a_real_model_fire() {
    ...
    // 1. THE LOAD, through the door the runtime opens. `Cuda` and not
    //    `Box<dyn Engine>` for one reason: the graph-replay claim needs
    ...
    // The degenerate one-step frame: `fire` is what `submit` does to a frame
    // of one, and this gate's whole claim is that an attachment changes
    // nothing about the forward — so it must go through the same door.
    let prefill = FrameSubmission::of(prefill);
    let mut ticket = engine.submit(&prefill).expect("the prefill fires");
    engine
        .settle_frame(&mut ticket)
        .expect("the numbers door hands back what the fire computed");
    let first = argmax(&ticket.steps[0].readouts[0].values);
    assert_eq!(
    ...
    let bound = engine
        .bind_instance(&InstanceBinding {
            ...
            seeds: Vec::new(),
            geometry: engine_api::tensor_ir::registry::GeometryClass::Host,
    ...
    assert_eq!(bound.program, program, "the engine acknowledges the program");
    ...
    for round in 0..DECODES {
        if round == CAPTURE_FROM {
            ...
            engine.shell_mut().expect("loaded").set_mode(Graphs::On);
        }
        let submission = Step {
        ...
        committed += 1;
        // The numbers, asked for by name: `submit` answered with the device
        // still running (alto F2b), and this is the wait.
        engine
            .settle_frame(&mut ticket)
    ...
    // coming across it.
    let stats = engine.shell().expect("loaded").graph_stats();
    eprintln!(
        "guest tokens: {:?}",
        guest_tokens
            .iter()
