...
pub fn rms_strided_row(
    ...
    ctx.fire(
        Fire::at("norm/rms.metal", "rms_strided_row_bfloat16").apply(Grid::of([lanes, 1, 1], [t, 1, 1])),
...
pub fn rms_strided_head_row(
    ...
    let t = rms_threads(*axis)?;
    ctx.fire(
        Fire::at("norm/rms.metal", "rms_strided_head_row_bfloat16").apply(Grid::of(head_row_grid(t, heads, rows)?, [t, 1, 1])),
        &[x.arg(), w.arg(), out.arg(), eps.arg(), axis.arg(), w_stride.arg(), plus_one.arg(), gain.arg(), row_pitch.arg()],
    )
}
...
pub fn gated_rms(
    ...
    let t = head_width(*vd)?;
    ctx.fire(
        Fire::at("norm/gated_rms.metal", "gated_rms_bfloat16").apply(Grid::of(head_row_grid(t, heads, rows)?, [t, 1, 1])),
...
pub fn gated_rms_strided(
    ...
    let t = head_width(*vd)?;
    ctx.fire(
        Fire::at("norm/gated_rms.metal", "gated_rms_strided_bfloat16").apply(Grid::of(head_row_grid(t, heads, rows)?, [t, 1, 1])),
...
pub fn layer_scalar_mul(
    ...
    let rows = *rows;
    ctx.fire(
        Fire::at("norm/layer_scalar.metal", "layer_scalar_mul_bfloat16").apply(Grid::of(elementwise(width, rows)?, [256, 1, 1])),
...
pub fn residual_add(
    ...
    let rows = *rows;
    ctx.fire(
        Fire::at("norm/residual_add.metal", "residual_add_bfloat16").apply(Grid::of(elementwise(width, rows)?, [256, 1, 1])),
...
pub fn residual_add_strided(
    ...
    let rows = *rows;
    ctx.fire(
        Fire::at("norm/residual_add.metal", "residual_add_strided_bfloat16").apply(Grid::of(elementwise_rows(width, rows)?, [256, 1, 1])),
...
pub fn add_bias(
    ...
    ctx.fire(
        Fire::at("norm/add_bias.metal", "add_bias_bfloat16").apply(Grid::of(lanes, [lanes[0].min(256), 1, 1])),
