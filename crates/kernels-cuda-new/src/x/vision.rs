...
//! resolve to. [`crate::x::driver_internal`] is the same shape for the same
//! reason.
//!
...
/// `vision/tower_naive_kernels.cuh` — the six more than one tower launches.
pub mod tower_naive {
    use crate::jit::Root;

    /// The root these routines compile a symbol out of.
    pub static ROOT: Root = Root::new(
        "vision/tower_naive_kernels",
        include_str!("../../csrc/src/vision/tower_naive_kernels.cuh"),
    ...
    /// them: the towers compile no other element type.
    pub mod inst {
        /// `tower_naive_kernels.cuh:136` — RMSNorm, optional weight.
        pub const K_RMS: &str = "::pie_cuda_driver::kernels::vision::device::k_rms\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:169` — `a += b`, flat.
        pub const K_ADD: &str = "::pie_cuda_driver::kernels::vision::device::k_add\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:184` — the f32 feature plane narrowed to the tower's format.
        pub const K_F32_TO_BF16: &str = "::pie_cuda_driver::kernels::vision::device::k_f32_to_bf16\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:206` — the exact erf GELU, `nn.GELU(approximate='none')`.
        pub const K_GELU_ERF: &str = "::pie_cuda_driver::kernels::vision::device::k_gelu_erf\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:223` — LayerNorm, optional scale and bias.
        pub const K_LAYERNORM: &str = "::pie_cuda_driver::kernels::vision::device::k_layernorm\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:117` — the naive `[N, K] x [O, K]^T` matmul.
        pub const K_MATMUL: &str = "::pie_cuda_driver::kernels::vision::device::k_matmul\
             <::pie_cuda_driver::kernels::device::bf16>";
    }
}

/// `vision/gemma4_naive_kernels.cuh` — the one the two gemma-4 towers share.
pub mod gemma4_naive {
    use crate::jit::Root;

    /// The root this routine compiles a symbol out of.
    pub static ROOT: Root = Root::new(
        "vision/gemma4_naive_kernels",
        include_str!("../../csrc/src/vision/gemma4_naive_kernels.cuh"),
        "vision/gemma4_naive_kernels.cuh",
    );

    /// The template-id NVRTC is handed, spelled as it is handed it.
    pub mod inst {
        /// `gemma4_naive_kernels.cuh:45` — the clipped linear's clamp.
        pub const K_CLAMP: &str = "::pie_cuda_driver::kernels::vision::device::k_clamp\
             <::pie_cuda_driver::kernels::device::bf16>";
    }
}

/// `vision/gemma4_vision.cuh` — the gemma-4 vision encoder's own.
pub mod gemma4_vision {
    use crate::jit::Root;

    /// The root these routines compile a symbol out of.
    pub static ROOT: Root = Root::new(
        "vision/gemma4_vision",
        include_str!("../../csrc/src/vision/gemma4_vision.cuh"),
        "vision/gemma4_vision.cuh",
    );

    /// The template-ids NVRTC is handed, spelled as it is handed them.
    pub mod inst {
        /// `gemma4_vision.cuh:68` — `o = 2*(p - 0.5)`.
        pub const K_SCALE: &str = "::pie_cuda_driver::kernels::vision::device::k_scale\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:156` — the per-row score softmax, in f32.
        pub const K_SOFTMAX: &str = "::pie_cuda_driver::kernels::vision::device::k_softmax\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:253` — the float pooling accumulator, scaled and narrowed.
        pub const K_POOL_FINISH: &str = "::pie_cuda_driver::kernels::vision::device::k_pool_finish\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:85` — the two axial position-table rows per patch.
        pub const K_ADDPOS_GRID2D: &str = "::pie_cuda_driver::kernels::vision::device::k_addpos_grid2d\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:106` — 2-D axial RoPE over a 64-wide head.
        pub const K_ROPE_AXIAL2D: &str = "::pie_cuda_driver::kernels::vision::device::k_rope_axial2d\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:131` — one head's `QK^T` into the f32 score matrix.
        pub const K_QK: &str = "::pie_cuda_driver::kernels::vision::device::k_qk\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:194` — one head's `softmax(QK^T) V` back into the activation.
        pub const K_AV: &str = "::pie_cuda_driver::kernels::vision::device::k_av\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:236` — the pooling scatter-accumulate.
        pub const K_POOL: &str = "::pie_cuda_driver::kernels::vision::device::k_pool\
             <::pie_cuda_driver::kernels::device::bf16>";
    }
}

/// `vision/gemma4_audio.cuh` — the USM/Conformer audio encoder's own.
pub mod gemma4_audio {
    ...
    pub static ROOT: Root = Root::new(
        "vision/gemma4_audio",
        include_str!("../../csrc/src/vision/gemma4_audio.cuh"),
        "vision/gemma4_audio.cuh",
    );

    /// The template-ids NVRTC is handed, spelled as it is handed them.
    pub mod inst {
        /// `gemma4_audio.cuh:102` — `o = x * sigmoid(x)`, flat.
        pub const K_SILU: &str = "::pie_cuda_driver::kernels::vision::device::k_silu\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:117` — `a += scale * b`, the macaron residual weight.
        pub const K_AXPY: &str = "::pie_cuda_driver::kernels::vision::device::k_axpy\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:80` — the naive matmul with an optional bias.
        pub const K_MATMUL_BIAS: &str = "::pie_cuda_driver::kernels::vision::device::k_matmul_bias\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:130` — the conv module's gated linear unit.
        pub const K_GLU: &str = "::pie_cuda_driver::kernels::vision::device::k_glu\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:151` — LayerNorm over the channel axis, then ReLU.
        pub const K_LAYERNORM_RELU: &str = "::pie_cuda_driver::kernels::vision::device::k_layernorm_relu\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:248` — `[OC, To, Fo]` flattened to `[To, Fo*OC]`.
        pub const K_SSCP_FLATTEN: &str = "::pie_cuda_driver::kernels::vision::device::k_sscp_flatten\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:267` — the in-place q/k pre-scale.
        pub const K_QKV_SCALE: &str = "::pie_cuda_driver::kernels::vision::device::k_qkv_scale\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:288` — the sinusoidal relative-position encoding.
        pub const K_REL_POS_ENC: &str = "::pie_cuda_driver::kernels::vision::device::k_rel_pos_enc\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:197` — `Conv2d(k3, s2, p1)` over (time, frequency).
        pub const K_CONV2D_S2: &str = "::pie_cuda_driver::kernels::vision::device::k_conv2d_s2\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:223` — channels-first to channels-last.
        pub const K_CHLAST: &str = "::pie_cuda_driver::kernels::vision::device::k_chlast\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:235` — its inverse.
        pub const K_CHFIRST: &str = "::pie_cuda_driver::kernels::vision::device::k_chfirst\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:323` — causal-sliding-window attention with a relative-position
        /// bias.
        pub const K_LOCAL_ATTN: &str = "::pie_cuda_driver::kernels::vision::device::k_local_attn\
             <::pie_cuda_driver::kernels::device::bf16>";
    }
}

/// `vision/qwen3_vl_tower.cuh` — the Qwen3-VL vision encoder's own.
pub mod qwen3_vl_tower {
    use crate::jit::Root;

    /// The root these routines compile a symbol out of.
    pub static ROOT: Root = Root::new(
        "vision/qwen3_vl_tower",
        include_str!("../../csrc/src/vision/qwen3_vl_tower.cuh"),
        "vision/qwen3_vl_tower.cuh",
    );

    /// The template-ids NVRTC is handed, spelled as it is handed them.
    pub mod inst {
        /// `qwen3_vl_tower.cuh:97` — a projection's bias, flat.
        pub const K_BIAS: &str = "::pie_cuda_driver::kernels::vision::device::k_bias\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:134` — the interpolated position embedding, added.
        pub const K_ADD_PE: &str = "::pie_cuda_driver::kernels::vision::device::k_add_pe\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:153` — `gelu_pytorch_tanh`, the block MLP's activation.
        pub const K_GELU_TANH: &str = "::pie_cuda_driver::kernels::vision::device::k_gelu_tanh\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:202` — the same, with fc1's optional bias folded in.
        pub const K_GELU_BIAS: &str = "::pie_cuda_driver::kernels::vision::device::k_gelu_bias\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:343` — the patch merger's 2x2 gather.
        pub const K_MERGE_GATHER: &str = "::pie_cuda_driver::kernels::vision::device::k_merge_gather\
             <::pie_cuda_driver::kernels::device::bf16>";
        /// `:298` — the qkv split, its bias and the ViT rope, fused.
        pub const K_SPLIT_ROPE_QKV: &str = "::pie_cuda_driver::kernels::vision::device::k_split_rope_qkv\
             <::pie_cuda_driver::kernels::device::bf16>";
    }
}
...
pub fn k_f32_to_bf16_bf16(
    ...
    unsafe {
        ctx.launch(
            &tower_naive::ROOT,
            tower_naive::inst::K_F32_TO_BF16,
            launch,
...
pub fn k_pool_finish_bf16(
    ...
    unsafe {
        ctx.launch(
            &gemma4_vision::ROOT,
            gemma4_vision::inst::K_POOL_FINISH,
            launch,
...
pub fn k_addpos_grid2d_bf16(
    ...
    unsafe {
        ctx.launch(
            &gemma4_vision::ROOT,
            gemma4_vision::inst::K_ADDPOS_GRID2D,
            tile16(rows, width),
            &[
                y.cast::<bf16>().arg(),
                tb.cast::<bf16>().arg(),
...
pub fn k_rope_axial2d_bf16(
    ...
    unsafe {
        ctx.launch(
            &gemma4_vision::ROOT,
            gemma4_vision::inst::K_ROPE_AXIAL2D,
...
pub fn k_matmul_bias_bf16(
    ...
    unsafe {
        ctx.launch(
            &gemma4_audio::ROOT,
            gemma4_audio::inst::K_MATMUL_BIAS,
            tile16(rows, width),
            &[
                x.cast::<bf16>().arg(),
                w.cast::<bf16>().arg(),
...
pub fn k_layernorm_relu_bf16(
    ...
    unsafe {
        ctx.launch(
            &gemma4_audio::ROOT,
            gemma4_audio::inst::K_LAYERNORM_RELU,
...
pub fn k_sscp_flatten_bf16(
    ...
    unsafe {
        ctx.launch(
            &gemma4_audio::ROOT,
            gemma4_audio::inst::K_SSCP_FLATTEN,
            tile16(rows, width),
            &[
                input.cast::<bf16>().arg(),
                out.cast::<bf16>().arg(),
...
pub fn k_rel_pos_enc_bf16(ctx: &Ctx, pe: *mut c_void, p: i32, hidden: i32) -> Result<(), Refusal> {
    ...
    unsafe {
        ctx.launch(
            &gemma4_audio::ROOT,
            gemma4_audio::inst::K_REL_POS_ENC,
            tile16(rows, width),
...
pub fn k_local_attn_bf16(
    ...
    unsafe {
        ctx.launch(
            &gemma4_audio::ROOT,
            gemma4_audio::inst::K_LOCAL_ATTN,
...
pub fn k_gelu_tanh_bf16(
    ...
    unsafe {
        ctx.launch(
            &qwen3_vl_tower::ROOT,
            qwen3_vl_tower::inst::K_GELU_TANH,
            launch,
            &[x.cast::<bf16>().arg(), o.cast::<bf16>().arg(), t.arg()],
        )
    }
...
pub fn k_gelu_bias_bf16(
    ...
    unsafe {
        ctx.launch(
            &qwen3_vl_tower::ROOT,
            qwen3_vl_tower::inst::K_GELU_BIAS,
            launch,
...
pub fn k_merge_gather_bf16(
    ...
    unsafe {
        ctx.launch(
            &qwen3_vl_tower::ROOT,
            qwen3_vl_tower::inst::K_MERGE_GATHER,
            tile16(rows, width),
...
pub fn k_split_rope_qkv_bf16(
    ...
    unsafe {
        ctx.launch(
            &qwen3_vl_tower::ROOT,
            qwen3_vl_tower::inst::K_SPLIT_ROPE_QKV,
...
#[cfg(test)]
mod tests {
    use super::{gemma4_audio, gemma4_naive, gemma4_vision, qwen3_vl_tower, tower_naive};

    /// Every instantiation is the string the deleted `vision` unit handed
    /// NVRTC, character for character.
    ///
    /// The unit built it from a `template_path` and an `elem`
    /// (`vision::device::<kernel>` and `device::bf16`, absolutised); a routine
    /// writes it out. This is the one place the two spellings can be compared
    /// now that only one of them exists.
    #[test]
    fn the_instantiations_are_what_the_unit_asked_for() {
        for (instantiation, kernel) in [
            (tower_naive::inst::K_RMS, "k_rms"),
            (tower_naive::inst::K_ADD, "k_add"),
            (tower_naive::inst::K_F32_TO_BF16, "k_f32_to_bf16"),
            (tower_naive::inst::K_GELU_ERF, "k_gelu_erf"),
            (tower_naive::inst::K_LAYERNORM, "k_layernorm"),
            (tower_naive::inst::K_MATMUL, "k_matmul"),
            (gemma4_naive::inst::K_CLAMP, "k_clamp"),
            (gemma4_vision::inst::K_SCALE, "k_scale"),
            (gemma4_vision::inst::K_SOFTMAX, "k_softmax"),
            (gemma4_vision::inst::K_POOL_FINISH, "k_pool_finish"),
            (gemma4_vision::inst::K_ADDPOS_GRID2D, "k_addpos_grid2d"),
            (gemma4_vision::inst::K_ROPE_AXIAL2D, "k_rope_axial2d"),
            (gemma4_vision::inst::K_QK, "k_qk"),
            (gemma4_vision::inst::K_AV, "k_av"),
            (gemma4_vision::inst::K_POOL, "k_pool"),
            (gemma4_audio::inst::K_SILU, "k_silu"),
            (gemma4_audio::inst::K_AXPY, "k_axpy"),
            (gemma4_audio::inst::K_MATMUL_BIAS, "k_matmul_bias"),
            (gemma4_audio::inst::K_GLU, "k_glu"),
            (gemma4_audio::inst::K_LAYERNORM_RELU, "k_layernorm_relu"),
            (gemma4_audio::inst::K_SSCP_FLATTEN, "k_sscp_flatten"),
            (gemma4_audio::inst::K_QKV_SCALE, "k_qkv_scale"),
            (gemma4_audio::inst::K_REL_POS_ENC, "k_rel_pos_enc"),
            (gemma4_audio::inst::K_CONV2D_S2, "k_conv2d_s2"),
            (gemma4_audio::inst::K_CHLAST, "k_chlast"),
            (gemma4_audio::inst::K_CHFIRST, "k_chfirst"),
            (gemma4_audio::inst::K_LOCAL_ATTN, "k_local_attn"),
            (qwen3_vl_tower::inst::K_BIAS, "k_bias"),
            (qwen3_vl_tower::inst::K_ADD_PE, "k_add_pe"),
            (qwen3_vl_tower::inst::K_GELU_TANH, "k_gelu_tanh"),
            (qwen3_vl_tower::inst::K_GELU_BIAS, "k_gelu_bias"),
            (qwen3_vl_tower::inst::K_MERGE_GATHER, "k_merge_gather"),
            (qwen3_vl_tower::inst::K_SPLIT_ROPE_QKV, "k_split_rope_qkv"),
        ] {
            assert_eq!(
                instantiation,
                format!(
                    "::pie_cuda_driver::kernels::vision::device::{kernel}\
                     <::pie_cuda_driver::kernels::device::bf16>"
                ),
                "{kernel}"
            );
    ...
    /// The five roots, as a list, so the two tests below walk the same set.
    const ROOTS: [&crate::jit::Root; 5] = [
        &tower_naive::ROOT,
        &gemma4_naive::ROOT,
        &gemma4_vision::ROOT,
        &gemma4_audio::ROOT,
    ...
    /// here, over the same five roots and the header set each one states.
    #[test]
    ...
    /// The five roots name five different files, so a diagnostic points at the
    /// header the symbol came out of.
    #[test]
