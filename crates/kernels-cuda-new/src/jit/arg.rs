...
//! already has.
//!
//! [`Abi`](crate::x::Abi) was written to be the one place a crossing type's
    ...
    ($($rust:ty),* $(,)?) => {
        $(
            impl ::kernels::routine::Arg<$crate::jit::Cuda> for $rust {
                const TY: ::kernels::Ty = <$rust as $crate::x::Abi>::TY;
                const SPELLING: &'static str = <$rust as $crate::x::Abi>::CPP;

                fn unpack(
                    value: &$crate::jit::ArgValue,
                    at: usize,
                ) -> ::core::result::Result<Self, ::kernels::routine::Refusal> {
                    <$rust as $crate::x::Abi>::unpack(value, at)
                }
            }
        )*
    };
