...
//
// `pie_mma.cuh` is beside this one — the `.cuh` files moved into
// `kernels-cuda-new/csrc/src` — and `kernels-cuda/csrc/CMakeLists.txt`
// reaches this directory with `-Xcompiler=-iquote`, so nvcc CAN resolve
// `#include "pie_mma.cuh"` now. It still must not: `-iquote` answers our own
...
// `__nv_bfloat16` typedef names the prelude's `device::bf16`, which is what
