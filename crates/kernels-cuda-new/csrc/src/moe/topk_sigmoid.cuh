...
// `device::Elem<T>` — which is what `norm/elementwise.cuh` established and
...
// `device::block_sum`, which is what that allowance is for.
//
...
// share one namespace — `moe::device` — where a `constexpr int MAX_EXPERTS`
...
// it always did, and so that `device::i32` inside `kernels::moe` keeps
