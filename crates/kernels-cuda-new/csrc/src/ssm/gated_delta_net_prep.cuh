...
// `kernels-cuda/tests/sources.rs` refuses a second one.
//
...
// element type `device::bf16`, and so the launchers in `gated_delta_net.cu` —
// which sit in `kernels::ssm`, where this namespace shadows `kernels::device`
// — still resolve `device::bf16` and `device::usize` to the prelude's names.
