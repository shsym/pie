...
// `float_to_bf16`, and the launcher instantiates it at `device::bf16`.
//
...
__device__ __forceinline__ float bf16_to_float(const device::bf16 v) {
    return device::bf16_to_f32(v);
}

__device__ __forceinline__ device::bf16 float_to_bf16(float v) {
    return device::f32_to_bf16(v);
}
...
__global__ void mamba_split(
    const device::bf16* __restrict__ projected,
    device::bf16* __restrict__ gate,
    device::bf16* __restrict__ conv_in,
    device::bf16* __restrict__ dt,
    int projection_dim,
    int intermediate,
    int conv_dim,
    int num_heads,
...
__global__ void build_nemotron_moe_ptrs_decode_batched(
    const device::i32* __restrict__ topk_idx,
    const float* __restrict__ topk_w,
    const device::bf16* const* __restrict__ up_weight_ptrs,
    const device::bf16* const* __restrict__ down_weight_ptrs,
    const device::bf16* __restrict__ norm_x,
    device::bf16* __restrict__ expert_up,
    device::bf16* __restrict__ expert_act,
    device::bf16* __restrict__ expert_out,
    const device::bf16** __restrict__ a_up_ptrs,
    const device::bf16** __restrict__ b_up_ptrs,
    device::bf16** __restrict__ c_up_ptrs,
    const device::bf16** __restrict__ a_down_ptrs,
    const device::bf16** __restrict__ b_down_ptrs,
    device::bf16** __restrict__ c_down_ptrs,
...
__global__ void build_nemotron_moe_ptrs_aligned(
    const device::i32* __restrict__ expert_ids,
    const device::bf16* const* __restrict__ up_weight_ptrs,
    const device::bf16* const* __restrict__ down_weight_ptrs,
    const device::bf16* __restrict__ aligned_in,
    device::bf16* __restrict__ aligned_up,
    device::bf16* __restrict__ aligned_act,
    device::bf16* __restrict__ aligned_out,
    const device::bf16** __restrict__ a_up_ptrs,
    const device::bf16** __restrict__ b_up_ptrs,
    device::bf16** __restrict__ c_up_ptrs,
    const device::bf16** __restrict__ a_down_ptrs,
