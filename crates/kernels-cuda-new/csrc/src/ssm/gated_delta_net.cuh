...
// `kernels-cuda/tests/sources.rs` enforces after `norm/altup_aux` shipped two
...
///
/// `DeviceKernel::instantiation()` glues `::pie_cuda_driver::kernels::` to
...
///
/// `state_bf16` is `__nv_bfloat16` and NOT the prelude's `device::bf16`,
/// which is a different type: `state_load`/`state_store` below are
/// template-specialised on `__nv_bfloat16` and a `device::bf16` state would
