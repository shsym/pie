...
// the rows in `kernels_cuda_new::families::quant` fire the templates at run
...
//
// [`Cast`] is why that works at `float` too. `device::Elem` is specialised on
...
///
/// `DeviceKernel::elem` is a path under `::pie_cuda_driver::kernels`, so the
