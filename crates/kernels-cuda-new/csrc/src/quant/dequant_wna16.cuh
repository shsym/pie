...
//     `Narrow2<f16>` below:** `kernels-cuda-new`'s NVRTC shim aliases
//     `__half` to `device::f16`, so under the compiler the JIT actually uses,
...
struct Narrow2<bf16> {
    using pair = bf16x2;
    static __device__ __forceinline__ pair pack(float lo, float hi) {
        return ::pie_cuda_driver::kernels::device::f32_to_bf16x2(lo, hi);
    }
    static __device__ __forceinline__ bf16 narrow(float v) {
        return ::pie_cuda_driver::kernels::device::f32_to_bf16(v);
    }
};
...
/// at `kernels-cuda-new/csrc/src`, where the shim lives: line 84 spells
