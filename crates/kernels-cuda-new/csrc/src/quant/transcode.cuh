...
// `transcode.cu` is the dispatch that binds a (source, target) pair to an
...
// validated model numerics and what lets `tests/test_transcode_fused.cu`
// assert exact equality rather than a tolerance.
//
// # No rows, and exactly why
//
// This file carries no `DeviceKernel` rows and therefore is not a `Unit`,
// for two independent reasons and either would be enough:
//
...
//     `path<elem>` with a single type path, so no row can name it.
//   * its operands are the Decode and Encode functors BY VALUE -- aggregates
//     of pointers and extents. `kernels::Ty` has no kind for a by-value
//     aggregate, and `runtime::args` marshals pointers, `I32`, `U32`, `F32`
//     and `Usize` and refuses everything else. A row would have to state the
//     struct's FIELDS and have the driver assemble it, which is
//     `new-horizon.md` §4.3's struct-layout inversion and is not implemented
//     here.
//
// It is converted anyway. Every `.cuh` under `csrc/src` is carried into
// `DEVICE_HEADERS` by `kernels-cuda-new/build.rs`, so a file that reaches for
// `<cstdint>` is a compile error waiting for whichever unit first includes
// it -- and this one is included by `quant_bf16_to_mxfp4.cuh`'s consumer
// today. Converting it now costs nothing and removes a trap.
//
...
using ::pie_cuda_driver::kernels::quant::device::encode_e8m0;
using ::pie_cuda_driver::kernels::quant::device::encode_fp4_e2m1;
