...
// `kernels-cuda/tests/sources.rs::no_global_is_defined_twice` exists to
...
// in `kernels_cuda_new::families::gemm` are `LaunchRule::Unstated` and the
...
//
// The kernels are in `pie_cuda_driver::kernels::gemm::device` now, where
// `gemv.cu` had them in an anonymous namespace inside
// `pie_cuda_driver::kernels::gemm`. From inside a namespace called `device`,
// the spelling `device::bf16` looks up `device` in the enclosing scope, finds
...
namespace pie_cuda_driver::kernels::gemm::device {
...
// `device::bf16` inside a namespace of that name.
