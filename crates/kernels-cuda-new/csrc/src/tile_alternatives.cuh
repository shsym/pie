...
//
// Every kernel in `csrc/src` classified against the buckets that now have a
...
//
// **The counting rule is a test, not this comment.** The first pass used a
...
// enumeration. `vendor_manifest.rs::reduction_axis_counts` is the
// definition, it runs in CI, and the numbers below are its output:
//
...
// table from 300 to 307 within an hour of it being written, and the gate
// named the drift rather than letting the census quietly go stale. That is
// the intended behaviour and re-deriving them is a one-line fix.
//
// The number that must NOT move quietly is `> 16 tiles`, and it has its own
// assertion with its own message: a new reduction over a vocabulary-sized
// axis is a new ARGMAX row waiting to happen, and it should stop a build
