...
// a `LaunchRule` on a row in `kernels_cuda_new::families::mlp`, and
...
// `norm/altup_aux` shipped exactly that for a release. `kernels-cuda`'s
// `tests/sources.rs::no_global_is_defined_twice` is the check that keeps it
// from happening twice, and it compares namespace-qualified names across
// every `.cu` AND every `.cuh` in the tree.
//
// # Why the kernels are templates now
//
// Every one of them was `_bf16` in its name and `device::bf16` in its
...
// narrow back. `device::Elem<T>` is the widen/narrow pair, specialised on the
...
// type `device::bf16`, and so `swiglu.cu`'s launchers -- which sit in
// `kernels::mlp` and say `device::bf16` on every cast -- resolve the same
...
using ::pie_cuda_driver::kernels::device::bf16x2;
using ::pie_cuda_driver::kernels::device::bf16x2_to_f32;
...
using ::pie_cuda_driver::kernels::device::f32_to_bf16x2;
using ::pie_cuda_driver::kernels::device::f32_to_f16;
