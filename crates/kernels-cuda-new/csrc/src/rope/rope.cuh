...
// header set carried in the Rust binary (`kernels-cuda-new`'s `build.rs`
...
// a STRING -- `::pie_cuda_driver::kernels::rope::device::rotate_partial<...>`
// -- and internal linkage has no such name. So the device text lives in
// `rope::device`, the launchers stay in `rope`, and a launcher's `device::`
// still spells the prelude because the `using` block below re-exports it into
// this namespace. That is deliberate: the `.cu` compiles unchanged apart from
// the kernel names, and the `.hpp` it implements never mentioned `device` at
// a point where `rope::device` existed.
//
...
namespace pie_cuda_driver::kernels::rope::device {
...
// element type `device::bf16`, and so `rope.cu`'s launchers -- which sit in
// the enclosing `rope` and write `device::bf16` meaning the prelude's -- go
...
using ::pie_cuda_driver::kernels::device::false_type;
using ::pie_cuda_driver::kernels::device::i32;
using ::pie_cuda_driver::kernels::device::true_type;
