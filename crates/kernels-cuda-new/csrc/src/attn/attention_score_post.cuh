...
//
// Something asked. `attention_flashinfer.cu` left `crates/kernels-cuda` for
// `crates/driver-cuda/csrc/attn/`, because its two capture dispatches are
...
// `csrc/src`, included only by `attention_flashinfer_common.cuh`, and NVRTC
// has never compiled a byte of it either.
//
// # The rename, stated
//
...
// Pulled in by name rather than by `using namespace`, for the reason
// `attention_flashinfer.cuh` gives: inside `attn::device` the `device::`
// qualifier resolves to THIS namespace, so a prelude name not re-exported
