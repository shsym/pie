...
// and it is gone: the fixed-width names come from `device::`.
//
//===----------------------------------------------------------------------===//
#pragma once
...
// same thing in this header and in `qkv_fused.cu`: inside `attn::device` the
// qualifier resolves to THIS namespace, and a prelude name not re-exported
// here stops resolving in the `.cu` the moment it includes this file.
...
__global__ void qkv_decode_qk_norm_rope_write_kv(
    const device::bf16* __restrict__ packed,
    device::bf16* __restrict__ q_out,
    device::bf16* __restrict__ k_pages,
    device::bf16* __restrict__ v_pages,
    const device::bf16* __restrict__ q_weight,
    const device::bf16* __restrict__ k_weight,
    ...
    float eps)
{
    ...
    device::bf16* dst = nullptr;
    device::bf16* v_dst = nullptr;
    if (is_q) {
    ...
    for (int dim_pair = threadIdx.x; dim_pair < half; dim_pair += BLOCK) {
        const float a = device::bf16_to_f32(src[dim_pair]) *
            inv_rms * device::bf16_to_f32(weight[dim_pair]);
        const float b = device::bf16_to_f32(src[dim_pair + half]) *
            inv_rms * device::bf16_to_f32(weight[dim_pair + half]);
        float cos_v, sin_v;
        if constexpr (USE_ROPE_TABLE) {
...
#pragma unroll
    for (int i = 0; i < ELEMS_PER_THREAD; ++i) {
        ...
        const float v = device::bf16_to_f32(src[dim]);
        vals[i] = v;
        sum += v * v;
    }
#pragma unroll
...
#pragma unroll
    for (int i = 0; i < ELEMS_PER_THREAD; ++i) {
        const int dim = lane * ELEMS_PER_THREAD + i;
        vals[i] *= inv_rms * device::bf16_to_f32(weight[dim]);
    }
...
#pragma unroll
    for (int i = 0; i < ELEMS_PER_THREAD; ++i) {
        const int dim = lane * ELEMS_PER_THREAD + i;
        dst[dim] = device::f32_to_bf16(vals[i]);
    }
    if (!is_q) {
        const device::bf16* v_src =
            src_row + q_dim + kv_dim + local_head * HEAD_DIM;
    ...
    float eps)
{
    ...
    float local = 0.f;
    float local_v = 0.f;
    const device::bf16* v_src = nullptr;
    if (!is_q) {
    ...
    for (int i = threadIdx.x; i < head_dim; i += BLOCK) {
        ...
        local += v * v;
        if (!is_q) {
            const float vv = device::bf16_to_f32(v_src[i]);
            local_v += vv * vv;
        }
    }
    ...
    for (int dim_pair = threadIdx.x; dim_pair < half; dim_pair += BLOCK) {
        const device::bf16 norm_a = device::f32_to_bf16(
            device::bf16_to_f32(src[dim_pair]) *
            inv_rms * device::bf16_to_f32(weight[dim_pair]));
        const device::bf16 norm_b = device::f32_to_bf16(
            device::bf16_to_f32(src[dim_pair + half]) *
            inv_rms * device::bf16_to_f32(weight[dim_pair + half]));
        const float a = device::bf16_to_f32(norm_a);
        const float b = device::bf16_to_f32(norm_b);
        const float freq = powf(
            theta,
            -2.f * static_cast<float>(dim_pair) /
                static_cast<float>(head_dim));
    ...
    if (!is_q) {
        ...
        for (int i = threadIdx.x; i < head_dim; i += BLOCK) {
            v_dst[i] = device::f32_to_bf16(device::bf16_to_f32(v_src[i]) * inv_v);
        }
    }
}
