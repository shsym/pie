...
//    `kernels-cuda-new/src/table/attn.rs`'s `attn::attn_score_fold_heads` --
...
// `std::uint32_t` and `std::size_t` are therefore `device::i32`, `device::u32`
// and `device::usize` here -- the same three types under nvcc, spelled so
...
//
//     multiple definition of `pie_cuda_driver::kernels::attn::device::
...
// and in `attention_flashinfer.cu` means the same thing: inside `attn::device`
// the qualifier resolves to THIS namespace, and a prelude name not re-exported
