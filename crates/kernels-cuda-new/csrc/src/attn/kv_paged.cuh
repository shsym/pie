...
// the principle `kernels_cuda_new::source` states and the vendored FlashInfer
...
// `device::bf16` now and every `__float2bfloat16` is `device::f32_to_bf16`,
...
// Pulled in by name rather than by `using namespace`, so that `device::` in
// this file and in `kv_paged.cu` means the same thing: inside `attn::device`
// the qualifier resolves to THIS namespace, and a prelude name that is not
// re-exported here would stop resolving in the `.cu` the moment it includes
// this header.
...
__device__ __forceinline__ int find_request(const device::u32* qo_indptr,
                                            int R, int token_idx) {
    for (int r = 0; r < R; ++r) {
...
__global__ void write_kv(
    const device::bf16* __restrict__ k_curr,
    const device::bf16* __restrict__ v_curr,
    device::bf16* __restrict__ k_pages,
    device::bf16* __restrict__ v_pages,
    ...
    const device::u32* __restrict__ win,
    int R,
    int page_size,
    int h_kv,
    int d,
...
__global__ void write_kv_explicit(
    const device::bf16* __restrict__ k_curr,   // [LANES, h_kv, d]
    const device::bf16* __restrict__ v_curr,
    device::bf16* __restrict__ k_pages,
    device::bf16* __restrict__ v_pages,
    const device::u32* __restrict__ w_page,   // [LANES] PHYSICAL page id per lane
    const device::u32* __restrict__ w_off,    // [LANES] offset-in-page per lane
    const device::u8* __restrict__ row_valid,
    int B,
    int page_size,
    int h_kv,
...
__global__ void copy_kv_cells(
    device::bf16* __restrict__ k_pages,
    device::bf16* __restrict__ v_pages,
    const device::u32* __restrict__ dst_page,  // [N] PHYSICAL page id per cell
    const device::u32* __restrict__ dst_off,   // [N] offset-in-page per cell
    const device::u32* __restrict__ src_page,  // [N] PHYSICAL page id per cell
    const device::u32* __restrict__ src_off,   // [N] offset-in-page per cell
    int N,
    int page_size,
    int h_kv,
    int d)
    ...
    __nv_fp8_interpretation_t fp8_kind)
{
    ...
    for (int i = threadIdx.x; i < row; i += blockDim.x) {
        const float kf = device::bf16_to_f32(k_curr[src + i]);
        const float vf = device::bf16_to_f32(v_curr[src + i]);
    ...
    int d)
{
    ...
    for (int j = tid; j < d; j += blockDim.x) {
        k_abs = fmaxf(k_abs, fabsf(device::bf16_to_f32(k_curr[src_base + j])));
        v_abs = fmaxf(v_abs, fabsf(device::bf16_to_f32(v_curr[src_base + j])));
    }
    ...
    if constexpr (UseFp8) {
        ...
        for (int j = tid; j < d; j += blockDim.x) {
            k_pages[dst_base + j] = __nv_cvt_float_to_fp8(
                device::bf16_to_f32(k_curr[src_base + j]) * k_inv,
                __NV_SATFINITE, __NV_E4M3);
            v_pages[dst_base + j] = __nv_cvt_float_to_fp8(
                device::bf16_to_f32(v_curr[src_base + j]) * v_inv,
                __NV_SATFINITE, __NV_E4M3);
        }
    } else {
        auto* k_pages = static_cast<device::i8*>(k_pages_raw);
        auto* v_pages = static_cast<device::i8*>(v_pages_raw);
        for (int j = tid; j < d; j += blockDim.x) {
            int kq = static_cast<int>(rintf(device::bf16_to_f32(k_curr[src_base + j]) * k_inv));
            int vq = static_cast<int>(rintf(device::bf16_to_f32(v_curr[src_base + j]) * v_inv));
            kq = kq > 127 ? 127 : (kq < -128 ? -128 : kq);
            vq = vq > 127 ? 127 : (vq < -128 ? -128 : vq);
            k_pages[dst_base + j] = static_cast<device::i8>(kq);
            v_pages[dst_base + j] = static_cast<device::i8>(vq);
        }
    }
}

__device__ __forceinline__ float fp4_e2m1_value(device::u8 code) {
...
__device__ __forceinline__ device::u8 quant_fp4_e2m1(float x) {
    ...
    return static_cast<device::u8>((neg ? 0x8 : 0) | best);
}

__global__ void write_kv_fp4_block(
    const device::bf16* __restrict__ k_curr,
    const device::bf16* __restrict__ v_curr,
    device::u8*        __restrict__ k_pages,
    device::u8*        __restrict__ v_pages,
    float*               __restrict__ k_scales,
    float*               __restrict__ v_scales,
    const device::u32* __restrict__ qo_indptr,
    const device::u32* __restrict__ kv_page_indices,
    ...
    int block_size)
{
    ...
    for (int byte_j = start / 2 + tid; byte_j <= (end - 1) / 2; byte_j += blockDim.x) {
        ...
        device::u8 v1 = 0;
        if (j0 < d) {
            k0 = quant_fp4_e2m1(device::bf16_to_f32(k_curr[src_base + j0]) * k_inv);
            v0 = quant_fp4_e2m1(device::bf16_to_f32(v_curr[src_base + j0]) * v_inv);
        }
        if (j1 < d) {
            k1 = quant_fp4_e2m1(device::bf16_to_f32(k_curr[src_base + j1]) * k_inv);
            v1 = quant_fp4_e2m1(device::bf16_to_f32(v_curr[src_base + j1]) * v_inv);
        }
        k_pages[packed_base + byte_j] = static_cast<device::u8>(k0 | (k1 << 4));
        v_pages[packed_base + byte_j] = static_cast<device::u8>(v0 | (v1 << 4));
    }
}
...
__global__ void dequant_fp8_pages_active(
    ...
    device::bf16*           __restrict__ k_out,
    device::bf16*           __restrict__ v_out,
    const device::u32*     __restrict__ page_indices,
    long long n,
    int page_elems,
    __nv_fp8_interpretation_t fp8_kind)
{
    ...
    k_out[elem] = device::f32_to_bf16(__half2float(kh));
    v_out[elem] = device::f32_to_bf16(__half2float(vh));
}
...
__global__ void dequant_int8_per_token_head_pages_active(
    const device::i8* __restrict__ k_pages,
    const device::i8* __restrict__ v_pages,
    const float*       __restrict__ k_scales,
    const float*       __restrict__ v_scales,
    T*     __restrict__ k_out,
    T*     __restrict__ v_out,
...
__global__ void dequant_fp4_pages_active(
    const device::u8* __restrict__ k_pages,
    const device::u8* __restrict__ v_pages,
    const float*        __restrict__ k_scales,
    const float*        __restrict__ v_scales,
    T*      __restrict__ k_out,
    T*      __restrict__ v_out,
    ...
    int block_size)
{
    ...
    const device::u8 kc = (k_pages[packed_i] >> nibble_shift) & 0xf;
    const device::u8 vc = (v_pages[packed_i] >> nibble_shift) & 0xf;
    const long long scale_idx =
...
__global__ void write_kv_explicit_devwin(
    ...
    device::bf16* __restrict__ k_pages,
    device::bf16* __restrict__ v_pages,
    const device::u32* __restrict__ w_page,
    const device::u32* __restrict__ w_off,
    const device::u8* __restrict__ row_valid,
    const device::u32* __restrict__ win,  // {start, len} device word
    int n_max,
    int page_size,
    int h_kv,
    int d)
...
__global__ void build_window_page_view(
    const device::u32* __restrict__ src_indices,
    const device::u32* __restrict__ src_indptr,
    int keep_pages,
    device::u32* __restrict__ dst_indptr,
    device::u32* __restrict__ dst_indices,
    int R)
{
    if (threadIdx.x == 0) {
        device::u32 acc = 0;
        dst_indptr[0] = 0;
        for (int r = 0; r < R; ++r) {
            const device::u32 have = src_indptr[r + 1] - src_indptr[r];
            const device::u32 keep =
                have < static_cast<device::u32>(keep_pages)
                    ? have : static_cast<device::u32>(keep_pages);
            acc += keep;
            dst_indptr[r + 1] = acc;
        }
    ...
    __syncthreads();
    for (int r = 0; r < R; ++r) {
        const device::u32 src_end = src_indptr[r + 1];
        const device::u32 dst_beg = dst_indptr[r];
        const device::u32 keep    = dst_indptr[r + 1] - dst_beg;
        // Trailing, so anchor both walks at the END of the source slice.
        for (device::u32 i = threadIdx.x; i < keep; i += blockDim.x) {
...
__global__ void build_full_split_view(
    const device::u32* __restrict__ src_indptr,
    const device::u32* __restrict__ src_last_page_len,
    int splits,
    int page_size,
    device::u32* __restrict__ dst_indptr,
    device::u32* __restrict__ dst_indices,
    device::u32* __restrict__ dst_last,
    const device::u32* __restrict__ src_indices)
{
    if (threadIdx.x != 0) return;
    const device::u32 base = src_indptr[0];
    const int pages = static_cast<int>(src_indptr[1] - base);
    const device::u32 tail = src_last_page_len[0];
    device::u32 acc = 0;
    dst_indptr[0] = 0;
    for (int i = 0; i < splits; ++i) {
        ...
        if (hi > lo) {
            ...
            acc += static_cast<device::u32>(hi - lo);
            dst_last[i] = (hi == pages)
                ? tail : static_cast<device::u32>(page_size);
        } else {
