...
// that used to surround them is `kernels_cuda_new::fa2` in Rust.
//
// That split is the whole of north-star §5 step 8. `attention_flashinfer_common.cuh`
// beside this file is the shape being replaced: the same includes, plus
...
// `kernels_cuda_new::fa2::PrefillGeometry::ECHO_TEMPLATE` carries the `&`.
//
...
// handed to `nvrtcCreateProgram` as `includeNames[]` from a set generated by
// walking `csrc/`, and NVRTC matches those names against the literal string in
// the directive — so the two angle includes here resolve because
// `csrc/shim/cuda_bf16.h` is carried as `cuda_bf16.h`, and the six quoted ones
// resolve because `csrc/src/attn/flashinfer/attention/decode.cuh` is carried
// as `attn/flashinfer/attention/decode.cuh` — its path relative to `csrc/src`.
// The trees are rooted to make that fall out; `carried.rs`'s header is the
// design.
//
...
#include "attn/flashinfer/attention/default_prefill_params.cuh"
#include "attn/flashinfer/attention/mask.cuh"
#include "attn/flashinfer/attention/prefill.cuh"
