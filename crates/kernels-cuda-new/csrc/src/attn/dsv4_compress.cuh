...
// this file and in `dsv4_compress.cu` means the same thing: inside
// `attn::device` the qualifier resolves to THIS namespace, and a prelude name
// that is not re-exported here would stop resolving in the `.cu` the moment
...
__global__ void compressed_attn(
    const device::bf16* __restrict__ q,       // [total_tokens, num_q_heads, head_dim]
    const device::bf16* __restrict__ comp_kv, // [total_comp, head_dim]
    device::bf16* __restrict__ o,             // [total_tokens, num_q_heads, head_dim]
    ...
    float scale)
{
    ...
    if (num_visible <= 0) {
        ...
        for (int d = tid; d < head_dim; d += ATTN_BLOCK) {
            o_row[d] = device::f32_to_bf16(0.f);
        }
    ...
    for (int c = tid; c < num_visible; c += ATTN_BLOCK) {
        const device::bf16* k_row =
            comp_kv + static_cast<long long>(comp_off + c) * head_dim;
        float dot = 0.f;
        for (int d = 0; d < head_dim; ++d) {
            dot += q_smem[d] * device::bf16_to_f32(k_row[d]);
        }
    ...
    for (int c = 0; c < num_visible; ++c) {
        ...
        for (int i = 0; i < dims_per_thread; ++i) {
            ...
            if (d < head_dim) {
                acc[i] += w * device::bf16_to_f32(k_row[d]);
            }
        }
    }
...
__global__ void dsv4_compress_gather(
    ...
    const device::i32* __restrict__ boundary_tok,
    const device::i32* __restrict__ boundary_pos,
    const device::i32* __restrict__ window_lo,
    T* __restrict__ out,
    int head_dim,
    int ratio,
    int coff) {
...
// not cross.
template <class T = device::i32>
__global__ void dsv4_boundary_meta_decode(
    const device::i32* __restrict__ positions,
    device::i32* __restrict__ out_pos,
    device::i32* __restrict__ out_req,
    device::i32* __restrict__ out_rope,
    int n,
    int ratio,
...
__global__ void dsv4_compress_gather_paged(
    ...
    const float* __restrict__ ape,
    const device::i32* __restrict__ boundary_pos,
    const device::i32* __restrict__ boundary_req,
    const device::u32* __restrict__ kv_page_indices,
    const device::u32* __restrict__ kv_page_indptr,
    T* __restrict__ out,
    int head_dim,
...
__global__ void compressed_attn_paged(
    ...
    const device::bf16* __restrict__ comp_kv_pages,
    device::bf16* __restrict__ o,
    float* __restrict__ lse_out,
    const device::i32* __restrict__ positions,
    const device::u32* __restrict__ kv_page_indices,
    const device::u32* __restrict__ kv_page_indptr,
    const device::i32* __restrict__ req_of_token,
    int num_q_heads,
    int head_dim,
    int ratio,
    int page_size,
    ...
    for (int c = tid; c < num_visible; c += ATTN_BLOCK) {
        ...
        const device::bf16* k_row = comp_kv_pages + slot * head_dim;
        float dot = 0.f;
        for (int d = 0; d < head_dim; ++d) {
            dot += q_smem[d] * device::bf16_to_f32(k_row[d]);
        }
    ...
    for (int c = 0; c < num_visible; ++c) {
        ...
        for (int i = 0; i < dims_per_thread; ++i) {
            const int d = tid + i * ATTN_BLOCK;
            if (d < head_dim) acc[i] += w * device::bf16_to_f32(k_row[d]);
        }
    }
    ...
    for (int i = 0; i < dims_per_thread; ++i) {
        const int d = tid + i * ATTN_BLOCK;
        if (d < head_dim) o_row[d] = device::f32_to_bf16(acc[i] * inv_z);
    }
}
