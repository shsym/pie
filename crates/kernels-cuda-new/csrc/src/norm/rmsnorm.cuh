...
// by rows in `kernels_cuda_new::families::norm`; the rest are reachable only
...
// shared memory for `device::block_sum`'s warp scratch; these kernels reduce
...
// `device::Elem<T>`, so a second numeric format costs a row instead of a
// translation unit. The four vectorised bodies are NOT: they read rows as
// `float4` and unpack them through `device::bf16x2`, and there is no packed
