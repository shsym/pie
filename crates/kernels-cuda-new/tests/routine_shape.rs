...
use kernels_cuda_new::jit::{ArgValue, Ctx, Launch, Root, Routine};
use kernels_cuda_new::routine;
use kernels_cuda_new::x::abi::bf16;

/// `rope/rope.cuh`, as a routine body names it.
static ROPE: Root =
...
fn rope_standard_table(
    ...
    unsafe {
        ctx.launch(
            &ROPE,
            "::pie_cuda_driver::kernels::rope::device::standard_table\
...
fn rope_bf16_stub(
    ...
    unsafe {
        ctx.launch(
            &ROPE,
            "::pie_cuda_driver::kernels::rope::device::rotate<false, false>",
            Launch::per_row(num_tokens.unsigned_abs(), ROTATE_BLOCK),
            &[],
        )
    }
...
fn two_instantiations_of_a_root_are_two_keys() {
    let a = ROPE.key("rope::device::rotate<false>", "sm_90");
    let b = ROPE.key("rope::device::rotate<true>", "sm_90");
    assert_ne!(a, b);
    assert_eq!(a, ROPE.key("rope::device::rotate<false>", "sm_90"));
}
