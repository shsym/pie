...
use kernels_cuda_new::plan::{decode, mla, prefill, sm90, Device, Workspace};
...
/// that path changes with the build, and there may be more than one, so this
/// globs rather than hard-codes. `PIE_FLASHINFER_INCLUDE` overrides it.
...
/// The workspace `target/` directory, walked up from this test's `OUT_DIR`.
...
fn scratch_dir() -> PathBuf {
    PathBuf::from(env!("OUT_DIR")).join(format!("plan-diff-{}", std::process::id()))
}

/// Compile the harness, or say why not.
///
/// The scratch directory is this test's own `OUT_DIR`, which exists because the
/// crate has a build script. Nothing is written outside `target/`.
