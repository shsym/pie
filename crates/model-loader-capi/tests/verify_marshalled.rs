...
fn decl(name: &str) -> TensorDecl {
    TensorDecl {
        ...
        name: name.to_string(),
        shape: vec![1],
        encoding: Encoding::Raw(model_loader::types::DType::U8),
        alignment: 1,
...
fn a_read_that_runs_past_the_end_of_its_file_is_a_violation() {
    ...
    use model_loader::plan::{CheckpointFileDecl, DestExtent, SourceExtent};
    use model_loader::types::{CheckpointFormat, FileId};
    ...
    plan.instrs.push(StorageInstr::ExtentWrite {
        ...
        source: SourceExtent {
            ...
            file_offset: 0,
            span_bytes: 4096,
            stride: model_loader::extent::Extent::byte_run(4096),
            dtype: model_loader::types::DType::U8,
        },
        dest: DestExtent {
            buffer: BufferId(0),
            offset: 0,
...
fn a_plan_that_delivers_the_contract_verifies() {
    let plan = plan_with(vec![decl("w")], vec![finalize(0, "w")]);
    let demanded = contract(&[("w", vec![1], Encoding::Raw(model_loader::types::DType::U8))]);
...
fn a_contract_tensor_the_plan_omits_is_a_violation() {
    ...
    let demanded = contract(&[
        ("w", vec![1], Encoding::Raw(model_loader::types::DType::U8)),
        ("bias", vec![1], Encoding::Raw(model_loader::types::DType::U8)),
    ]);
...
fn a_plan_delivering_the_wrong_shape_is_a_violation() {
    let plan = plan_with(vec![decl("w")], vec![finalize(0, "w")]);
    let demanded = contract(&[("w", vec![2, 3], Encoding::Raw(model_loader::types::DType::U8))]);
    let violations = verify_marshalled(&plan, Some(&demanded)).unwrap_err();
...
fn a_plan_delivering_the_wrong_encoding_is_a_violation() {
    let plan = plan_with(vec![decl("w")], vec![finalize(0, "w")]);
    let demanded = contract(&[("w", vec![1], Encoding::Raw(model_loader::types::DType::BF16))]);
    let violations = verify_marshalled(&plan, Some(&demanded)).unwrap_err();
