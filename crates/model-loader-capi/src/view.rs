...
//!
//! The mirror of [`super::arena`]: that module writes a [`model_loader::plan::LoadPlan`]
...
use model_loader::verify::{Certificate, ContractView, Violation, verify};
...
/// Reconstruct the safe view [`model_loader::verify`] works on from the POD plan.
///
...
pub(super) unsafe fn plan_view(
    plan: &PieLoaderPlan,
) -> Result<model_loader::verify::PlanView<'_>, String> {
    ...
    for file in unsafe { slice_of(plan.files.ptr, plan.files.len) } {
        files.push(model_loader::verify::FileView {
            id: file.id,
    ...
    for source in unsafe { slice_of(plan.sources.ptr, plan.sources.len) } {
        sources.push(model_loader::verify::SourceView {
    ...
    for tensor in unsafe { slice_of(plan.tensors.ptr, plan.tensors.len) } {
        tensors.push(model_loader::verify::TensorView::new(
    ...
    for instr in instrs {
        match &instr.op {
            ...
            | PieLoaderStorageOp::BulkExtentWrite { source, .. } => {
                reads.push(model_loader::verify::ReadView {
                    instr: instr.id,
                    file_id: source.file_id,
    ...
    Ok(model_loader::verify::PlanView {
...
fn join_encoding(tensor: &PieLoaderTensorDeclView) -> model_loader::types::Encoding {
    match tensor.encoding_kind {
        PieLoaderEncodingKind::Quant => {
            model_loader::types::Encoding::Quant(model_loader::types::QuantSpec {
        ...
        _ => model_loader::types::Encoding::Raw(tensor.dtype.into()),
    }
}
