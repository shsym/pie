...
use model_loader::plan::{DestExtent, Extent, LoadPlan, SourceExtent, StorageInstr};
use model_loader::types::{Encoding, QuantScheme, Visibility};
...
impl PlanArena {
    ...
    fn finish(self, plan: &LoadPlan, cache_key: &str) -> *mut PieLoaderPlan {
        ...
        let summary = arena.store_str(&model_loader::dump::describe(plan));
        let stats_json = arena.store_str(&model_loader::dump::plan_stats_json(plan));
