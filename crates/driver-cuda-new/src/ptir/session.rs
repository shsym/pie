...
use super::bridge::{self, HostChannel, StageChannels};
use super::control::Control;
use super::fire::Prepared;
use super::launch_control;
use super::ring::{ChannelShape, Rings};
...
impl Session {
    ...
    pub fn pull(
        ...
        for &c in &sets.need_full {
            ...
            while let Some(wire) = plane.take() {
                let native = bridge::wire_to_native(shape.dtype, shape.numel, &wire)
    ...
    pub fn push(
        ...
        for &c in &sets.put {
            ...
            for step in 0..produced {
                ...
                let wire = bridge::native_to_wire(shape.dtype, shape.numel, &native)
                    .map_err(|why| Error::invalid("ptir::session", why))?;
                let Some(plane) = host.get_mut(index) else {
                    return Err(Error::invalid(
                        "ptir::session",
    ...
    /// `row_of` says which row a lane samples. They are handed in rather
    ...
    pub fn fire(
        ...
        let sets = bridge::stage_channels(plan)
            .map_err(|why| Error::invalid("ptir::session", why))?;
        ...
        let mut prepared = Prepared::build(alloc, plan, &self.rings, extents, stream)?;
