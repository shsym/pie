...
impl CacheLock {
    fn acquire(cache_path: &Path) -> Result<Self, StoreError> {
        ...
        let io = |what: &'static str| StoreError::Io {
            what,
            path: lock_path.clone(),
            source: std::io::Error::last_os_error(),
        };
        let c_path = CString::new(lock_path.as_os_str().as_encoded_bytes())
        ...
        // and the mode argument is required because O_CREAT is set.
        let fd = unsafe {
            libc::open(
                c_path.as_ptr(),
        ...
        if unsafe { libc::flock(fd, libc::LOCK_EX) } != 0 {
            let err = io("lock");
            // SAFETY: same descriptor, not yet handed to a `CacheLock`, so
...
impl Drop for CacheLock {
    fn drop(&mut self) {
        // SAFETY: `fd` was opened by `acquire` and is closed exactly once,
        // here, because `CacheLock` is neither `Clone` nor `Copy`.
        unsafe {
