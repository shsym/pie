...
pub trait StagingOps {
    ...
    fn event_synchronize(&mut self, event: &Self::Event);
    /// `cudaEventRecord` on the given stream.
    fn event_record(&mut self, event: &Self::Event, stream: *mut c_void);
...
/// makes. Raw rather than [`crate::cuda::Allocator`] on purpose: the
/// workspace allocates at boot and frees at teardown, never inside a
/// capture, and the C++ frees unconditionally too.
///
/// The two infallible trait methods (`event_synchronize`, `event_record`)
/// PANIC on a CUDA error, because that is what the C++ does — both sites
/// are `CUDA_CHECK`, and a fence that silently failed would hand a staging
/// slot to the host while the GPU still reads it.
...
impl StagingOps for LiveStagingOps {
    ...
    fn event_synchronize(&mut self, event: &Self::Event) {
        ...
        assert!(code == cudaError::cudaSuccess, "cudaEventSynchronize: {code:?}");
    }
    ...
    fn event_record(&mut self, event: &Self::Event, stream: *mut c_void) {
        ...
        assert!(code == cudaError::cudaSuccess, "cudaEventRecord: {code:?}");
    }
