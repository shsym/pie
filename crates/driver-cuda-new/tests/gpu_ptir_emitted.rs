...
use driver_cuda_new::ptir::{Disk, Runtime, Target};
...
fn target(device: &driver_cuda_new::cuda::Device) -> Target {
    let (major, minor) = device.compute_capability().expect("compute capability");
    Target {
        major,
        minor,
        device: u64::try_from(device.ordinal()).unwrap_or(0),
        nvrtc: driver_cuda_new::ptir::nvrtc::version().expect("NVRTC must be loadable"),
    }
}
