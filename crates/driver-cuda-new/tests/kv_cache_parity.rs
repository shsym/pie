...
use driver_cuda_new::store::kv_cache::{self, KvCacheLayout, LayerSlot, PerLayer};
use driver_cuda_new::store::kv_geometry::{self, LayerShapes};
...
impl Sweep {
    ...
    fn report(&mut self, id: &str, r: Result<KvCacheLayout, driver_cuda_new::Error>) {
        match r {
            Ok(l) => {
                let body = format!(
                    "OK|{}|{}|{}",
...
fn cxx_message(e: &driver_cuda_new::Error) -> String {
    e.to_string()
}
