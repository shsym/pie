...
//! routed around.
//!
//! `abi_shell::checked` makes the dereference and the validation one
...
fn shell_source() -> String {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/abi_shell.rs");
    std::fs::read_to_string(path).expect("the shell's source is beside this test")
}
...
fn no_entry_point_lets_a_panic_out() {
    ...
    assert!(
        ...
        "these entry points let a panic reach the caller: {naked:?}. \
         `abi_shell::guard` turns it into a failed request instead; a \
