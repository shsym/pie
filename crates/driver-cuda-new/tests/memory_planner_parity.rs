//! Differential parity for [`driver_cuda_new::store::memory_planner`] against
...
use driver_cuda_new::store::memory_planner as mp;
use driver_cuda_new::store::plan::CudaMemoryPlan;
use driver_cuda_new::store::profile_key::{ProfileKey, ProfileShape};
...
impl Sweep {
    ...
    fn run(&mut self, id: &str, d: &DeviceCase, m: &ModelCase, cfg: &mp::PlannerConfig) {
        ...
        let budget = driver_cuda_new::store::profile_cache::planner_budget_bytes();
