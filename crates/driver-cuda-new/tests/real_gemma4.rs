...
use driver_cuda_new::cuda::{Allocator, DeviceBuffer, OwnedStream};
...
fn gemma4_matches_transformers_on_real_weights() {
    let _gpu = gpu_guard();
    let Some(_dev) = device_or_skip("gemma-4 E2B A/B") else { return };
    ...
    let pools: Vec<Option<(DeviceBuffer, DeviceBuffer)>> = (0..facts.layers)
        .map(|i| {
            ...
            let plane =
                (PAGE * i32::from(facts.kv_heads as u16) * facts.head_dim_of(i) as i32) as usize
                    * 2;
    ...
    let mut cublas_ops = driver_cuda_new::cuda::cublas::LiveCublas;
    let mut cublas =
        ...
        for (i, launch) in l.launches.iter().enumerate() {
            ...
            if kernel == "layout::transpose_bf16_nld_to_lnd" {
                ...
                for layer in 0..lcount.min(3) {
                    ...
                    let sn: f32 =
                        (0..d).map(|c| bf(src_off + c * 2).powi(2)).sum::<f32>().sqrt();
                    let dn: f32 =
                        (0..d).map(|c| bf(dst_off + c * 2).powi(2)).sum::<f32>().sqrt();
                    eprintln!(
                        "  relay L{layer} r{r}: src n={sn:.3} [{:.3},{:.3}] dst n={dn:.3} [{:.3},{:.3}]",
                        bf(src_off), bf(src_off + 2), bf(dst_off), bf(dst_off + 2)
                    );
                }
            }
            ...
            if let Some(a) = out {
                ...
                let v = |c: usize| {
                    ...
                    f32::from_bits(
                        u32::from(u16::from_le_bytes([host[off], host[off + 1]])) << 16,
                    )
                };
    ...
    if std::env::var("GEMMA4_AB_DEBUG").is_ok() {
        ...
        let norm: f32 =
            (0..hidden).map(|c| h(TOKENS - 1, c).powi(2)).sum::<f32>().sqrt();
        eprintln!(
        ...
        let (src_at, dst_at) = match (
            &l.args[t.args.start as usize],
            &l.args[t.args.start as usize + 1],
        ) {
            (Arg::Arena { at: s, .. }, Arg::Arena { at: d, .. }) => (*s, *d),
            other => panic!("the relay rides the arena, got {other:?}"),
        };
