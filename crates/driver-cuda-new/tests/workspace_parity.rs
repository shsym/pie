...
//!
//! Run `tests/oracle/workspace/run.sh` to regenerate [`GOLDEN_FNV1A64`]. The
...
use driver_cuda_new::model::workspace::{WorkspaceLayout, WorkspaceShape};
...
const CASES: &[Case] = &[
    Case { label: "qwen3_0_6b", hidden: 1024, intermediate: 3072, vocab: 151_936,
           head_dim: 128, head_dim_kernel: 128, q_heads: 16, kv_heads: 8,
           max_tokens: 2048, output_rows: 64, mtp_draft_rows: 0 },
    Case { label: "llama3_8b", hidden: 4096, intermediate: 14336, vocab: 128_256,
           head_dim: 128, head_dim_kernel: 128, q_heads: 32, kv_heads: 8,
           max_tokens: 4096, output_rows: 128, mtp_draft_rows: 0 },
    Case { label: "olmo2_1b", hidden: 2048, intermediate: 8192, vocab: 100_352,
           head_dim: 128, head_dim_kernel: 128, q_heads: 16, kv_heads: 16,
           max_tokens: 1024, output_rows: 32, mtp_draft_rows: 0 },
    Case { label: "qwen3_32b", hidden: 5120, intermediate: 25600, vocab: 151_936,
           head_dim: 128, head_dim_kernel: 128, q_heads: 64, kv_heads: 8,
           max_tokens: 8192, output_rows: 256, mtp_draft_rows: 0 },
    Case { label: "phi3_mini", hidden: 3072, intermediate: 8192, vocab: 32_064,
           head_dim: 96, head_dim_kernel: 128, q_heads: 32, kv_heads: 32,
           max_tokens: 4096, output_rows: 128, mtp_draft_rows: 0 },
    Case { label: "qwen3_6_mtp", hidden: 2048, intermediate: 8192, vocab: 248_320,
           head_dim: 128, head_dim_kernel: 128, q_heads: 16, kv_heads: 8,
           max_tokens: 8192, output_rows: 64, mtp_draft_rows: 192 },
    Case { label: "padded_with_mtp", hidden: 3072, intermediate: 8192, vocab: 32_064,
           head_dim: 96, head_dim_kernel: 128, q_heads: 32, kv_heads: 32,
           max_tokens: 2048, output_rows: 256, mtp_draft_rows: 32 },
];
...
fn the_rust_layout_reproduces_the_cpp_allocation_transcript() {
    ...
    assert_eq!(
        ...
        "transcript differs from the C++ oracle; \
         run tests/oracle/workspace/run.sh with WS_ORACLE_OUT set to diff them"
    );
}
