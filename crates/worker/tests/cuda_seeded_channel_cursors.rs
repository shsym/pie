...
//! different route into the driver and neither cursor came with it.
//!
//! The engine stages a seed with `peek_seed`, keeping its own copy, and hands
...
//! cell therefore never travels through the engine-shared mirror, and nothing
//! on either side moves the head or tail word on its way past.
//!
//! The engine counts it regardless. `pipeline::channel::bind` sets
//! `writer_tail = max(1)` for a seeded writer, because from the guest's side
//! one cell HAS been put. Two things the engine owns are then wrong at once:
//!
...
//!   it cannot stage, driver blocked on a fire never submitted, 0% on the GPU,
//!   nothing in any log.
...
//! so, in the comment above the geometry advance, and it is right; the driver
//! was not.
//!
...
//! a second defect entirely: the driver published the custom mask one byte per
...
fn a_seeded_host_writer_survives_past_its_seed() {
    ...
    rt.block_on(async {
        ...
        // old driver stopped, and every step after it is a `set` the old driver
