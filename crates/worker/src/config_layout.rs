...
/// Keys `[engine]` carries that are common to every engine. Everything else in
/// that section is handed to the engine-specific options struct.
///
...
/// names the engine it was rejected by.
const ENGINE_COMMON: &[&str] = &[
    "type",
    "device",
...
/// rewritten, so a key moved out of its section by the key pass was then swept
/// back into a third place by the section rule for the section it had just
/// left. Mapping each full path once removes the ordering question entirely.
///
/// A key spelled the way some retired config spelled it is not detected here.
/// It reaches serde and comes back as `unknown field ...` from the struct that
/// refused it, which is what `deny_unknown_fields` is for.
...
mod tests {
    ...
    fn a_key_reads_back_as_the_path_the_file_spells_it() {
        ...
        assert_eq!(to_file_path("model.engine.kv_pages"), "engine.kv_pages");
    ...
    // `the_longest_prefix_wins` STOOD HERE, and the four retired-spelling
    // tests below it: `the_retired_sections_say_so_rather_than_being_ignored`,
    // `the_retired_model_array_gets_the_migration_message_too` and
    // `a_bare_model_array_says_what_replaced_it`.
    //
    // The first asserted a tie-break between `runtime` -> `model.scheduler`
    // and `runtime.worker_threads` -> `server.worker_threads`, and both of
    // those rows are gone: `[runtime]` is `runtime` now and `worker_threads`
    // is `[server]`'s. No pair of surviving rows is a prefix of the other, so
    // there is no contest left to assert. The tie-break itself stays, because
    // it is what makes a future single-key move safe to add.
    //
    // The other three asserted the migration messages `reshape` used to bail
    // with. A config written against a retired spelling is not detected any
    // more; it is refused by `deny_unknown_fields`, naming the key.

    #[test]
    fn engine_keys_split_into_common_and_specific() {
        let file: toml::Table = toml::from_str(
            "[engine]\ntype = \"metal\"\ndevice = [\"metal:0\"]\ntotal_pages = 512\n",
        )
        .unwrap();
        let out = reshape(file).unwrap();
        let engine = out["model"]["engine"].as_table().unwrap();
        assert_eq!(engine["type"].as_str(), Some("metal"));
        ...
        assert_eq!(engine["options"]["total_pages"].as_integer(), Some(512));
        assert!(engine.get("total_pages").is_none());
    }
}
