    ...
    fn verdict(mut self, r: &Result<BoundTrace, tensor_ir::validate::ValidateError>) -> Report {
        match r {
            Ok(b) => {
                ...
                for stage in tensor_compiler::plan::compile_bound(b) {
...
fn step_line_k(
    ...
    b: &BoundTrace,
    inputs: &PassInputs,
    host: &mut dyn tensor_compiler::eval::interp::KernelHost,
    fed: String,
) -> String {
...
impl tensor_compiler::eval::interp::KernelHost for QuestKernels {
    fn kernel(
        &mut self,
        name: &str,
        args: &[Value],
