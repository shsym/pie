...
pub fn golden_dir() -> String {
    concat!(env!("CARGO_MANIFEST_DIR"), "/tests/golden").into()
}
...
pub struct CorpusStage {
    ...
    /// (`tensor_compiler::plan::debug_stage_plan`). Pinned instead of an encoding so a
...
pub fn op_tag(stage: &CompiledStage, node: u32) -> u8 {
    tensor_compiler::codegen::metal::OpView::of(&stage.normalized.ops[node as usize]).tag
}

/// Whether the region is a library region — the C++ `region.library` bit.
pub fn is_library(region: &tensor_compiler::plan::Region) -> bool {
    matches!(region.kind, tensor_compiler::plan::RegionKind::Library(_))
}

/// The C++ `region.library_op` byte. Generated regions encode `0`, which
/// happens to collide with `PTIR_LIBRARY_NUCLEUS_SAMPLE`.
pub fn library_op_byte(region: &tensor_compiler::plan::Region) -> u8 {
    match region.kind {
        tensor_compiler::plan::RegionKind::Library(op) => op as u8,
        _ => 0,
    }
}

/// The `region: ...` line the oracle prints for each case.
pub fn region_shape(region: &tensor_compiler::plan::Region) -> String {
    format!(
