...
use tensor_compiler::codegen::cuda::{
...
/// other test noticing. Bump `tensor_compiler::plan::COMPILER_VERSION` when regenerating,
...
fn stage_identity_is_pinned() {
    ...
    for stage in corpus_stages() {
        let _ = writeln!(
            ...
            "{} {:016x}",
            stage.id(),
            tensor_compiler::plan::stage_identity(&stage.plan)
        );
    }
        ...
        provenance::body_to_diff(&path, &rendered, provenance::Regenerate::Foreign)
    {
        assert_eq!(
            rendered, expected,
            "tensor_compiler::plan::stage_identity moved: a published graph-cache key changed, \
             so tensor_compiler::plan::COMPILER_VERSION must move with it"
        );
    }
}
...
fn emit_driver_test_kernel_fixtures() {
    ...
    for path in entries {
        ...
        for region in tensor_compiler::codegen::cuda::region_analysis::analyze_program(&stages) {
