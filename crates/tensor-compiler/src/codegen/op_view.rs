...
//! the typed [`Op`](tensor_ir::op::Op) enum, which is what makes the CUDA and
//! Metal region emitters diffable line by line.
//!
//! The record itself is defined in [`tensor_ir::wire`], next to the op table's
...
//! make it a third transcription of a byte layout `tensor_ir` already encodes and
...
//! `OpWire::to_op` being inverses a test in `tensor-ir` rather than an invariant
//! this crate hopes for.
//!
//! This module is only the name the emitters know the record by.

pub use tensor_ir::wire::{OpWire as OpView, result_bases};
