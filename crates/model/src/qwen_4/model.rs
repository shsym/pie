...
impl Model {
    ...
    fn new(w: Dtype, kv: Dtype, tp: u32, d: Dims) -> Model {
        ...
        // **EVERYTHING LANDS AS STORED.** The 4-bit artifact quantizes
        // almost everything — the mixed-4/8 conversion raises the embedding,
        // the head, every attention and GDN in/out projection, the shared
        // expert, the residual mixers' two GEMMs and the PLE projections to
        // `bits: 8` and leaves the expert banks and the n-gram table at
        // four — and this text declares each rectangle at the width the
        // file holds it, because every width has its own reader: the routed
        // expert banks take the grouped select, the n-gram table the
        // concatenating gather, and the eight-bit projections the affine
        // gemm point a dense `linear.matmul` resolves to when its weight
        // seats as planes. Nothing is dequantized at load; the one weight
        // transform the import states is the norms' `+1` fold, taken back
        // out (`import.rs`).
        //
        // `proj` is that eight-bit width — `qwen_3::model`'s router
        // predicate, grown to the whole dense set, which is what this
        ...
        let residual = |prefix: &str, inject: bool| Residual {
            ...
            inject: inject.then(|| {
                Weight::sym(format!("{prefix}.inject"), [u64::from(d.streams), sh], dense)
            }),
            eps: d.norm_eps,
        };
            ...
            .map(|l| {
                ...
                let mixer = if attn_at(l) {
                    ...
                    Mixer::Gdn(Gdn {
                        ...
                        // conversion leaves DENSE — too narrow for its own
                        // predicate — where the qkv|z pair beside them rides
                        // eight bits.
                        in_ba: Weight::sym(n("in_ba"), [2 * u64::from(v_heads), hidden], dense)
        ...
        Model {
            ...
            lowrank: d.lowrank,
            kv,
            embed: Weight::sym("embed", [u64::from(d.vocab), hidden], proj),
            head: Weight::sym("lm_head", [u64::from(d.vocab), hidden], proj),
            layers,
