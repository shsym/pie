...
//! lifts a snapshot into" is a fact about models. The worker reads it
...
/// lifted by `worker/src/weights.rs` before anything downstream sees it,
/// exactly as it is for the drivers.
///
...
pub struct ModelMetadata {
    ...
    ///
    /// Verbatim is the point. Its predecessor was a `pie.model/1`
    /// document: ~40 fields RESOLVED from a 136-field schema by 845 lines
    /// of normalizer, which is a second answer to "what is this model"
    /// living next to the catalog's. The catalog answers that now, and
    /// what stayed is the unresolved document, read for one field — the
    /// declared quantization, which no row can state because the same
    /// model is published at four bits and at eight.
    pub config: Vec<u8>,
}
