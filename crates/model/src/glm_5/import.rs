...
pub fn import_hf<B: SfBase, W1: Dtype, W2: Dtype, K: KvDtype>(m: &Model<W1, W2, K>) -> Import {
    ...
    for (l, w) in m.layers.iter().enumerate() {
        ...
        i.write(format!("layer.{l}.index_q_proj"), copy(format!("layer.{l}.self_attn.indexer.wq_b")));
        i.write(format!("layer.{l}.index_k_proj"), copy(format!("layer.{l}.self_attn.indexer.wk")));
        i.write(format!("layer.{l}.index_weights"), copy(format!("layer.{l}.self_attn.indexer.weights_proj")));
        i.write(format!("layer.{l}.index_k_norm"), copy(format!("layer.{l}.self_attn.indexer.k_norm")));
        i.write(format!("layer.{l}.index_k_norm_bias"), copy(format!("layer.{l}.self_attn.indexer.k_norm.bias")));
        match &w.mlp {
            Mlp::Dense { .. } => {
            ...
            Mlp::Routed { shared, experts, .. } => {
                ...
                if shared.is_some() {
                    ...
                    i.write(format!("layer.{l}.shared_down"), copy(format!("layer.{l}.mlp.shared_experts.down_proj")));
                }
            }
        }
    }
