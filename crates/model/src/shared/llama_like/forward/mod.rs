...
fn llama_like_metal_text(
    ...
    dsl::trace_metal("llama_like", &shape, class, |m| {
        ...
        let plus_one =
            |w: &dsl::NormW| u32::from(w.variant == model_ir::trace::NormVariant::Gemma);
        ...
        let activate = |gate: &Val, up: &Val, width: u32| match metal.activation {
            Activation::SiluMul => {
                dsl::metal::generated::silu_mul(gate, up, gate.layer(), None)
            }
            ...
            Activation::Geglu => {
                dsl::metal::generated::geglu_tanh(gate, up, gate.layer(), None)
            }
        };
        ...
        for l in 0..f.layers {
            ...
            let (q, k) = if fused_qk_rope {
                ...
                (
                    norm(&q, &w.q_norm, head_dim),
                    norm(&k, &w.k_norm, head_dim),
                )
            };
        ...
        let logits = if metal.logit_softcap > 0.0 {
            dsl::metal::generated::logit_softcap(
                &logits,
                metal.logit_softcap,
                logits.layer(),
                None,
                            ...
                            let q = dsl::regions(
                                ...
                                || {
                                    ...
                                    dsl::guard(
                                        ...
                                        || cuda::write_kv_to_pages(&kt, &vt, &w.kv, f.kv_heads, plan_head_dim),
                                    );
                                },
                            )
