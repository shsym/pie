...
pub fn import_hf<B: SfBase, W1: Dtype, W2: Dtype, K: KvDtype, const TP: usize>(
    ...
    for (l, w) in m.layers.iter().enumerate() {
        ...
        if w.res_blend.is_some() {
            i.write(
                format!("layer.{l}.res_norm"),
                copy(format!("layer.{l}.self_attention_res_norm")),
            );
            i.write(
                format!("layer.{l}.res_proj"),
                copy(format!("layer.{l}.self_attention_res_proj")),
            );
        }
        match &w.mixer {
            Mixer::Mla(a) => {
                ...
                if a.gate.is_some() {
                    i.write(
                        format!("layer.{l}.o_gate"),
                        copy(format!("layer.{l}.self_attn.g_proj")),
                    );
                }
                i.write(
            ...
            Mixer::Kda(_) => {
                ...
                i.write(
                    format!("layer.{l}.kda_f_a"),
                    copy(format!("layer.{l}.self_attn.f_a_proj")),
                );
                i.write(
                    format!("layer.{l}.kda_f_b"),
                    copy(format!("layer.{l}.self_attn.f_b_proj")),
                );
                i.write(
                    format!("layer.{l}.kda_b"),
                    copy(format!("layer.{l}.self_attn.b_proj")),
                );
                i.write(
                    format!("layer.{l}.kda_dt_bias"),
                    copy(format!("layer.{l}.self_attn.dt_bias")),
                );
                i.write(
                    format!("layer.{l}.kda_a_log"),
                    copy(format!("layer.{l}.self_attn.A_log")),
                );
                i.write(
                    format!("layer.{l}.kda_gate"),
                    copy(format!("layer.{l}.self_attn.g_proj")),
                );
                i.write(
                    format!("layer.{l}.kda_o_norm"),
                    copy(format!("layer.{l}.self_attn.o_norm")),
                );
                i.write(
                    format!("layer.{l}.kda_o_proj"),
                    copy(format!("layer.{l}.self_attn.o_proj")),
                );
            }
        }
        match &w.mlp {
            ...
            Mlp::Routed {
                ...
                i.write(
                    ...
                    copy(format!("layer.{l}.block_sparse_moe.gate")),
                );
                i.write(
                ...
                i.write(
                    ...
                    stack(
                        (0..*experts)
                            .map(|e| copy(format!("layer.{l}.block_sparse_moe.experts.{e}.w2"))),
                    ),
                );
                if shared.is_some() {
                    i.write(
                    ...
                    i.write(
                        ...
                        copy(format!(
                            "layer.{l}.block_sparse_moe.shared_expert.down_proj"
                        )),
                    );
                }
            }
