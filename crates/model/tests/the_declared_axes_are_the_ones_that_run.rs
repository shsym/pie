...
use model_dsl::{
    Attention, Operands, Operation, Param, ParamSource, Plan, Plane, ValueId, resolve_classes,
};

/// Every plane a plan can be traced at: a model text may emit a different op
/// per plane, so an axis that survives on one says nothing about the others.
...
fn plan_of(sku: &str, plane: Plane) -> Plan {
...
/// `attn.qv` names two — but the two export seams this file is about name one
/// each, and a second would mean the text changed under the test.
...
fn a_drafting_lane_lands_in_a_class_that_runs_the_draft_head() {
    ...
    for plane in PLANES {
        let plan = plan_of(DRAFTING, plane);
        ...
        let head_cache = plan
            ...
            .unwrap_or_else(|| panic!("{plane:?}: the draft head declares no kv row"))
            as u32;
        let head_pages = plan
            .values
            .iter()
            .position(|v| v.def == model_dsl::Def::Cache(head_cache))
            .unwrap_or_else(|| panic!("{plane:?}: nothing reads the draft head's kv row"))
            as u32;
        ...
        assert_ne!(
            drafting, plain,
            "{plane:?}: a drafting lane and a plain one are the same word, so \
             `drafts` is not a fact this classifier computes"
        );

        let drafting = classes
            .class_of(drafting)
            .unwrap_or_else(|| panic!("{plane:?}: the drafting word names no class"));
        let plain = classes
            .class_of(plain)
            .unwrap_or_else(|| panic!("{plane:?}: the plain word names no class"));

        assert!(
            runs_the_head(drafting),
            "{plane:?}: the class a drafting lane composes as does not run the \
             draft head's attention, so the axis is declared and unreachable"
        );
        assert!(
            !runs_the_head(plain),
            "{plane:?}: the class a NON-drafting lane composes as runs the \
...
fn a_capturing_lane_lands_in_a_class_that_runs_the_lse_attention() {
    ...
    for plane in PLANES {
        let plan = plan_of(CAPTURING, plane);
        ...
        for rows in [1, 8] {
            ...
            let class = classes
                .class_of(word)
                .unwrap_or_else(|| panic!("{plane:?}: the capturing word names no class"));
            assert_eq!(
                kernels(class),
                (true, false, false),
                "{plane:?}: a {rows}-row capturing lane's class must run the \
        ...
        for (rows, want) in [(1, (false, true, false)), (8, (false, false, true))] {
            ...
            assert_eq!(
                kernels(class),
                want,
                "{plane:?}: a {rows}-row lane that captured nothing must run \
...
/// **THE DRAFT READOUT IS NOT CARVED OVER, AND ORDER IS WHAT BUYS THAT.**
///
/// `model_compiler::arena`'s delivery tail — liveness to the end of the node
/// list, read in every class — is given to the `"out"` seam BY NAME. Every
/// other seam is a trace-time attach point whose span ends where its producer
/// does, which is right for `attn.q` and wrong for an export somebody reads
/// after the launch. Until design §9's export OP lands, what keeps the draft
/// column safe is that the model text states it LAST: nothing runs after it,
/// so no rectangle can be placed on top of it.
///
/// Measured, and this is why the test exists rather than the comment alone:
/// with the draft stated BEFORE the trunk's `lm_head`, qwen36-27b carves
/// 4,236,247,040 bytes — the two `[rows, vocab]` columns sharing an address,
/// 151 MiB over the trunk-only carve. Stated last it carves 8,220,835,840, and
/// the 3.98 GiB difference is the second column paid for honestly.
#[test]
fn the_draft_readout_outlives_the_trunk_readout() {
    for plane in PLANES {
        ...
        assert_ne!(out, draft, "{plane:?}: one value cannot be both readouts");

        let baked = compile(&plan, &budgets_for(&plan), &DeviceProfile::default())
            .unwrap_or_else(|why| panic!("{plane:?}: {}", why.say(&plan)));

        assert!(
            !baked.arena.co_tenants(out, draft),
            "{plane:?}: the draft column and the trunk's logits column share \
             bytes, so whichever the graph writes second clobbers the other \
             between the launch and the sampler's read"
        );
    }
}
...
///
/// | SKU | classes | nodes run | arena bytes |
/// |---|---|---|---|
/// | qwen35-a3b | 4 | 1125 | 4102029312 |
/// | qwen35-d3b | 4 | 485 | 2522873856 |
/// | qwen35-d0.8b | 4 | 485 | 4085252096 |
/// | qwen35-a3b-tp2 | 4 | 1205 | 4102029312 |
///
/// **THE ARENA IS THE STRONGEST OF THE THREE, AND IT IS EXACT.** Adding the
/// capture arm adds a `PrefillLse` per attention layer and one schedule, and
/// the carve does not grow by one byte — because the arm is a MERGE ARM, so
/// its `o` aliases the column the merge owns (build log 4), and its `lse` is a
/// per-layer value the busiest-instant carve reuses like every other per-layer
/// value. A window costs bytes only where it costs a column.
///
...
fn the_new_axes_cost_the_old_words_nothing() {
    ...
    const BEFORE: [(&str, usize, usize, u64); 4] = [
        ("qwen35-a3b-bf16-kv-bf16", 4, 1125, 4_102_029_312),
        ("qwen35-d3b-bf16-kv-bf16", 4, 485, 2_522_873_856),
        ("qwen35-d0.8b-bf16-kv-bf16", 4, 485, 4_085_252_096),
        ("qwen35-a3b-bf16-kv-bf16-tp2", 4, 1205, 4_102_029_312),
    ];

    for (sku, was_classes, was_nodes, was_arena) in BEFORE {
        for plane in PLANES {
            let plan = plan_of(sku, plane);
            ...
            assert_eq!(
                old.len(),
                was_classes,
                "`{sku}` as {plane:?}: a word with neither new fact composes as \
            ...
            assert_eq!(
                run.len(),
                was_nodes,
                "`{sku}` as {plane:?}: the classes a lane with neither new fact \
                 composes as run {} nodes and used to run {was_nodes}",
                run.len(),
            );

            let baked = compile(&plan, &budgets_for(&plan), &DeviceProfile::default())
                .unwrap_or_else(|why| panic!("`{sku}` as {plane:?}: {}", why.say(&plan)));
            assert_eq!(
                baked.arena.bytes, was_arena,
                "`{sku}` as {plane:?}: the carve is {} bytes and used to be \
                 {was_arena}; a fire that captures nothing and drafts nothing \
                 now reserves memory for a fire that does",
                baked.arena.bytes,
            );
        }
    }
    ...
    for sku in [DRAFTING, CAPTURING] {
        for plane in PLANES {
            ...
            assert!(
                ...
                "`{sku}` as {plane:?}: {} node(s) are demanded in no class — \
                 first is `{}`",
                classes.dead.len(),
