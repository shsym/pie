...
pub(crate) fn gemv_bf16(
    ...
    if n <= SPLIT_K_MAX_ROWS {
        ...
        return ctx.fire(
            Fire::at("gemm/gemv.cuh", instantiation).apply(Launch::grid(
                [n.unsigned_abs(), 1, 1],
                [WARP_LANES, warps, 1],
            )),
            &values,
    ...
    ctx.fire(
        Fire::at("gemm/gemv.cuh", instantiation)
            .apply(Launch::grid([grid_x, 1, 1], [WARP_LANES, WARPS, 1])),
        &values,
    )
}
