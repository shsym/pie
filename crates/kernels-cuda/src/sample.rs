...
pub fn lm_head_gemv_argmax_int8(
    ...
    if token_ids.width <= 0 {
        return Err(Refusal::Empty {
            what: "the token id column's width",
        });
    }
    ...
    ctx.fire(
        ...
        .apply(Launch::grid([num_blocks_x, rows, 1], [GEMV_BLOCK_DIM, 1, 1]).smem(smem)),
        &[
            states.ptr.arg(),
            lm_head_weight.arg(),
            scale_inv.arg(),
    ...
    ctx.fire(
        ...
        .apply(Launch::flat(rows, SELECT_BLOCK)),
        &[
            partial_pairs.cast_const().arg(),
            token_ids.arg(),
            num_rows.arg(),
            tiles.arg(),
        ],
...
pub fn argmax<T>(
    ...
    ctx.fire(
        Fire::at(
            "sample/argmax.cuh",
            crate::jit::symbol(&format!("::pie::sample::argmax<{}>", T::CPP)),
        )
        .apply(rms(rows)),
        &[src.ptr.arg(), out.arg(), src.width.arg()],
    )
}
    ...
    ctx.fire(
        Fire::at(
            ...
            crate::jit::symbol(&format!(
                "::pie::sample::argmax_compact_scatter<{}>",
                T::CPP
            )),
        )
        .apply(rms(rows)),
        &[src.ptr.arg(), row_indices.arg(), out.arg(), src.width.arg()],
    )
}
