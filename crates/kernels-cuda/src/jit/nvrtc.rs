...
fn device_link(unit: &str, relocatable: &[u8]) -> Result<Vec<u8>, CompileError> {
    ...
    let devrt = cudadevrt()?;
    // `cuLinkCreate` is a CONTEXT call, and this is the only step of a compile
    // that is. NVRTC itself needs a device only to name an architecture, so
    // every other unit in this crate compiles on a thread that has never
    // touched a context; a cooperative one reaches here and answers
    // `CUDA_ERROR_INVALID_CONTEXT` (201) instead. Binding is thread-local and
    // idempotent, so this costs the second call nothing.
