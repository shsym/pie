...
///
/// **Modules are never unloaded.** `jit/launch.rs`'s `GRANTED` — the memo of
/// which entry points have already been granted a raised dynamic shared-memory
/// cap — is keyed by function ADDRESS, and the driver reuses addresses after
/// an unload; evicting a module would make that memo answer for a different
/// kernel. If eviction is ever wanted, that memo has to be re-keyed first.
///
/// The claim is unchanged and the address for it is not: the memo was
/// `runtime::module`'s, that file went with the unit world, and the memo
/// crossed into `jit/launch.rs` intact. A load-bearing argument that points
/// at a deleted file reads as an argument nobody has checked.
///
/// # Errors
///
