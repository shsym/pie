...
pub fn mxfp4_scales_to_marlin_e8m0(
    ...
    selected_rows: Const<i32>) -> Result<(), Refusal> {
...
pub fn mxfp4_moe_gate_up_decode_bf16(
    ...
    glu_limit: Const<f32>,
    glu_alpha: Const<f32>,
    ew: In<Struct<ExpertWeights>>) -> Result<(), Refusal> {
    if ew.ptr.is_null() {
        return Err(Refusal::Null { what: "the expert-weight view this statement names" });
    }
