...
use kernels_cuda::jit::cache;
use kernels_cuda::source;
...
__global__ void widen_all(unsigned int* out) {
    ...
    if (i >= 65536u) return;
    pie::f16 v; v.raw = (unsigned short)i;
    out[i] = __float_as_int(f16_to_f32(v));
}
}
"#;
...
fn every_fp16_pattern_widens_to_the_same_bits() {
    ...
    let cubin = compile(SWEEP, arch, "::pie::probe::widen_all");
...
///
/// `kernels/` carries two spellings of the same arithmetic on purpose —
...
/// `src/source.rs` lists every file in the directory, so both
...
/// its subject had been merged away, and its coverage was strictly subsumed by
...
///
/// Those two have since gone as well, with the whole of `examples/`, so the
/// counts above are a record of what was measured and not a check anything
/// still runs. That does not weaken the argument for deleting the first probe
/// — its subject is still merged away — but it does change what this file is.
/// It is now the only gate on the prelude a machine runs at all, which is why
/// it is a `#[test]` here and not a fourth example.
///
...
__global__ void probe(unsigned char* out, const float* in) {
    ...
    pie::f16 mine; mine.raw = __half_as_ushort(back);
...
__global__ void probe(float* out, const unsigned int* in) {
    ...
    pie::f16 lane; lane.raw = __half_as_ushort(a.x);
