...
use kernels_cuda::jit::{ArgValue, Ctx, cache};
...
fn a_kernel_with_no_launcher_fires() {
    ...
    assert!(kernels_cuda::routine(symbol).is_some(), "{symbol} is declared by no family");
    ...
    // live.
    unsafe { kernels_cuda::call(symbol, &args, std::ptr::null_mut()) }
...
fn the_dynamic_path_and_the_routine_agree() {
    ...
    unsafe { kernels_cuda::call("norm::scalar_mul_bf16", &args, std::ptr::null_mut()) }
...
fn an_undeclared_symbol_is_refused_as_undeclared() {
    ...
    assert!(kernels_cuda::routine(symbol).is_none());

    // SAFETY: the values are empty and the call returns before touching CUDA.
    let refusal = unsafe { kernels_cuda::call(symbol, &[], std::ptr::null_mut()) };
...
fn a_symbol_compiles_once() {
    ...
    use kernels_cuda::jit::Root;

    let root = Root::new("norm/elementwise.cuh");
    let scalar_mul = "::pie::norm::scalar_mul<::pie::bf16>";
    let first = cache::resolve(&root, scalar_mul)
        .expect("compiles");
    let second = cache::resolve(&root, scalar_mul)
        .expect("compiles");
...
fn two_instantiations_of_one_root_are_distinct() {
    ...
    let root = Root::new("norm/elementwise.cuh");
    let scalar_mul = "::pie::norm::scalar_mul<::pie::bf16>";
    let mul = cache::resolve(&root, scalar_mul)
        .expect("compiles");
    let other = cache::resolve(&root, "::pie::norm::residual_add<::pie::bf16>")
        .expect("compiles");
    assert!(
