...
#include "kernel.h"
// Detoxified for pie: drop the torch op-registration include. The
// `marlin_gemm(torch::Tensor& ...)` wrapper at the bottom of this file
// is wrapped under `#ifdef PIE_MARLIN_KEEP_TORCH_WRAPPER`, leaving only
// the torch-free `marlin::marlin_mm(void*, void*, ...)` launcher that
// `marlin_wrapper.cpp` calls into.
...
//
// The shape list is owned by crates/driver-cuda/csrc/src/kernels.def (PIE_MARLIN_SHAPE);
