#pragma once
...
#ifndef _marlin_cuh
  #define _marlin_cuh
  // Detoxified for pie: replace torch/ATen/c10 includes with raw CUDA. The
  // marlin entry-point wrapper that depended on torch::Tensor is replaced
  // by `marlin_wrapper.hpp` (raw-pointer C++ API).
  #include "scalar_type.hpp"  // vendored vllm::ScalarType, throw-based asserts
  #include <cuda.h>
  #include <cuda_fp16.h>
  ...
  // in the never-called `marlin_gemm` torch wrapper, which we exclude from
  // the build (TU never compiled).
