...
//         moe_gemm/moe_gemm_template_dispatch.h
//         (GIT_TAG in crates/driver-cuda/csrc/CMakeLists.txt is the authority)
//
// Vendored (rather than patched via CPM) so pie can prune the MoE activation
// matrix from crates/driver-cuda/csrc/src/kernels.def; see moeGemmBiasAct below. The only
// other change is that the six sibling #includes were rewritten from
// "./..."/"../include/..." to include-path-relative form, since this copy no
// longer sits next to them.
//
// Refresh after bumping the vendored FlashInfer revision by re-copying the
// upstream file and re-applying both changes.
// ─────────────────────────────────────────────────────────────────────────────
...
// --- pie: activation set pruned by crates/driver-cuda/csrc/src/kernels.def -------------
// Upstream instantiates one CUTLASS epilogue per ActivationType, and each one
// drags the entire grouped-GEMM template stack along with it. pie reaches
// exactly one of them (nemotron_h is the only CutlassMoeFCRunner consumer and
// passes Relu2), so the rest are dead code that accounts for most of this
// target's compile time.
//
// `if constexpr` inside a templated entity leaves the discarded branch
// uninstantiated, so a disabled activation costs nothing to compile while
// still failing loudly -- and naming kernels.def -- if it is ever reached.
constexpr bool pie_moe_activation_enabled(ActivationType t) {
  switch (t) {
#define PIE_MOE_ACTIVATION(name) \
  case ActivationType::name:     \
    return true;
#include "kernels.def"
    default:
      return false;
...
#define PIE_MOE_DISPATCH_ACT(act, tag)                                      \
  case ActivationType::act:                                                 \
    if constexpr (pie_moe_activation_enabled(ActivationType::act)) {        \
      runGemm<cutlass_extensions::tag>(inputs, hopper_inputs);              \
    } else {                                                                \
      TLLM_THROW(                                                           \
          "MoE activation " #act                                            \
          " is not compiled into this build; add PIE_MOE_ACTIVATION(" #act  \
          ") to crates/driver-cuda/csrc/src/kernels.def and rebuild.");                 \
    }                                                                       \
    break;
...
void MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType, IsMXFPX>::moeGemmBiasAct(
  ...
  switch (inputs.activation_type) {
    PIE_MOE_DISPATCH_ACT(Relu, EpilogueOpDefaultReLU)
    PIE_MOE_DISPATCH_ACT(Gelu, EpilogueOpDefaultFtGelu)
    PIE_MOE_DISPATCH_ACT(Silu, EpilogueOpDefaultSilu)
    PIE_MOE_DISPATCH_ACT(Identity, EpilogueOpDefault)
    PIE_MOE_DISPATCH_ACT(Swiglu, EpilogueOpDefaultSilu)
    PIE_MOE_DISPATCH_ACT(Geglu, EpilogueOpDefaultFtGelu)
    PIE_MOE_DISPATCH_ACT(Relu2, EpilogueOpDefaultRelu2)
