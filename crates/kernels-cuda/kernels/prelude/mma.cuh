...
// wants ONE instantiation set. Grepping every `.cu` in the archive crate's
// `kernels-cuda/csrc` found exactly two users --
//
...
//
// `pie_device.cuh` already defines `pie::bf16`,
...
//
// Defining a second bf16 here would undo that -- `bf16*` and
...
// compiling proves nothing. `examples/mma_probe.rs` was the gate: it ran this
// shim under NVRTC and the real `nvcuda::wmma` under `nvcc` on the same
// inputs on the same device and compared the two 16x16 results element by
...
// and a `B` with exactly one non-zero column. The probe then flipped its own
// store to `mem_col_major` and required the comparison to CATCH it, because
...
// The measurement stands for the lane maps as they are written BELOW and for
// nothing else: it says they were right when they were checked, not that the
// next edit is. So the recipe is spelled out above rather than pointed at --
// same inputs, both paths, same device, no tolerance, and a deliberate
// transposition that must be caught -- because whoever changes a lane map now
// has to rebuild the harness before trusting it, and an untested lane map is
// still a wrong answer that compiles.
//
//===----------------------------------------------------------------------===//
#pragma once
...
/// `pie`, once, so the code below reads as
...
/// memory read row-major, since B was loaded column-major. The identity case
/// of the parity probe this file's header describes asserted exactly that, and
/// would have failed if this stored `(n, m)`.
