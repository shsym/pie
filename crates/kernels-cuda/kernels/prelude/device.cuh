...
//
// `pie`, not `pie::norm`. This was under `norm` while `norm` was the only
// family that had been migrated, and the second family to widen a bf16 is
// exactly the moment that becomes wrong -- either it reaches into another
// family's namespace for a scalar type, or it restates the type and the two
// are no longer one instantiation.
//
// A name here is therefore a name every family may write UNQUALIFIED: `pie`
// encloses `pie::norm`, so `bf16` inside a family resolves to this file
// without a `using` line. There were 328 such lines, in 60 files, for as long
// as the prelude sat at `…::kernels::device` and each family at
// `…::kernels::<family>::device` -- two namespaces spelled `device`, the
// inner one shadowing the outer, so the qualification a family wrote could
// not reach the prelude at all. They went with the shadowing.
//
// # Why there is a root at all, which is a measurement and not a taste
//
// The families were nearly given the global namespace outright -- a kernel
// would then be spelled `::norm::scalar_mul`, which is what a trace calls it,
// so the C++ name and the trace symbol would be one string. NVRTC answered
// that in one compile: it pre-includes CUDA's math API, which declares
// `__device__ double norm(int dim, double const*)` at global scope, and
// `namespace norm {` is a redeclaration of it. Six roots failed and 120 did
// not, which is the trap -- the collision is invisible until the one family
// whose name a builtin already owns gets compiled.
//
// So the global namespace is CUDA's, and `pie` is the whole of what this
// crate opens there. `tests/device_sources.rs` holds that as a rule.
//
// # What is NOT here
//
...
struct bf16 {
    ...
    ///
    /// `constexpr` since 2026-08-16, and it is the only thing that changed:
    /// a `constexpr` constructor may still be called at run time, so every
    /// existing `bf16{bits}` compiles to the same nothing it did. What it buys
    /// is the one site that needs a bf16 at COMPILE time --
    /// `flashinfer/comm/trtllm_allreduce_fusion.cuh:1212-1216`'s
    /// `static constexpr __nv_bfloat16 value = __nv_bfloat16_raw{0x8000U}`,
    /// the Lamport protocol's negative-zero empty-slot sentinel, read by the
    /// fused all-reduce's one-shot kernel. `shim/cuda_bf16.h`'s
    /// `__nv_bfloat16_raw(unsigned short)` is the other half of that chain and
    /// carries the argument. **`explicit` is untouched**, which is the part
    /// worth saying: `MODIFICATIONS`' "THE EDIT THAT IS NOT A REMOVAL" turns
    /// on these constructors being explicit, and `constexpr` and `explicit`
    /// are orthogonal.
    ...
    /// `b = f;` — assignment from a `float`, which the constructor above
    /// deliberately does not give.
    ///
    /// Added 2026-08-16, and it is the one place in this struct where an
    /// implicit `float` -> `bf16` narrowing is reachable. NVIDIA's
    /// `__nv_bfloat16` declares exactly this member
    /// (`__nv_bfloat16 &operator=(const float f);`), so this makes our type
    /// closer to the one it impersonates rather than further from it.
    ///
    /// # The site, and why the constructor could not answer it
    ///
    /// `flashinfer/comm/trtllm_allreduce_fusion.cuh`'s `vec_add`:
    ///
    /// ```
    ...
    /// ```
    ///
    /// `vec_t<T, N>::operator[]` returns `T&` (`vec_dtypes.cuh:580`), so this
    /// is an ASSIGNMENT to an existing `bf16`, not an initialisation, and no
    /// constructor participates. It is on the live path: `FusedOp::operator()`
    /// calls `vec_add` whenever `HasResidual<Pattern>`, which is true for
    /// `kARResidualRMSNorm` — the one pattern `kernels_cuda::comm` reaches.
    ///
    /// # What it does and does not open
    ///
    /// `bf16 x = 1.0f;` and `bf16 x{1.0f};` stay refused, because those are
    /// initialisation and the constructor is still `explicit`. Passing a
    /// `float` where a `bf16` parameter is declared stays refused for the same
    /// reason. What becomes legal is `x = f;` on an already-constructed
    /// `bf16`, which is the statement upstream writes.
    ///
    /// The rounding is `__float2bfloat16`'s, so this is the constructor with a
    /// different spelling and cannot disagree with it.
...
struct f16 {
    ...
    /// See [`bf16::bf16()`].
    f16() = default;
    /// See [`bf16::bf16(unsigned short)`], `constexpr` included.
    ...
    /// See [`bf16::operator=(float)`]. Symmetric for the same reason, and
    /// with no asking site of its own: `vec_add` instantiates at `T = half`
    /// only for a fused all-reduce this tree never names, and a template that
    /// found the member on one format and not the other would fail at the
    /// second dtype rather than the first.
...
/// there is no rounding to disagree about. It was held to the same gate as
/// everything else here — the deleted `examples/halftype_parity.rs` swept
/// `__bfloat162float` over 1,150,464 inputs against nvcc — and the half of
/// that gate which is still run lives in-tree: `tests/prelude_parity.rs`
/// sweeps the fp16 twin exhaustively.
///
...
// it: `shim/cstdint` picks `long long`, `ptrdiff_t` on this LP64 target
