...
// in 13.3+. `tests/vendor_manifest.rs` gated on it and was deleted at
// `1a08b179a`; the detector survived the sweep as a DIAGNOSIS rather than a
// gate, in `jit/nvrtc.rs`'s `tile_header_mismatch`, which recognises the
// codegen ICE and prints both this `ls` and the `grep` behind it. It fires
// for whoever meets the failure instead of ahead of them.
//
...
// `bf16` is refused — `template constraint not satisfied` — even
// carrying `__tile_builtin__`. A tile kernel must name NVIDIA's
// `__nv_bfloat16`. That collides with `shim/cuda_bf16.h`, which aliases
// that same name to `bf16` so FlashInfer stays byte-identical to
...
// out of the archive crate's `libpie_kernels_cuda.a`), on §20.8's shape — 318
// tokens, hidden
...
// This tree's `shim/cuda_bf16.h` answers the same name with
// `using __nv_bfloat16 = bf16;`. A struct declaration and a type
...
/// tree's own — `bf16`, say — is refused as `template constraint
