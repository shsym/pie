...
// have, and everything else about them is Rust, in `src/norm.rs`'s
// `altup_aux`.
//
...
// cost it `bf16` (replaced by a `unsigned short` wrapper and two
...
// element type `bf16` -- which is where it lives.
template <class T>
