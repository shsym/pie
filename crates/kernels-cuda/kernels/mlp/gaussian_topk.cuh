...
// `mlp::gaussian_topk_bf16` is in `JIT_DISPATCHED`, so the shim held
...
// `tests/device_sources.rs::no_global_is_defined_twice` exists to hold, after
// `norm/altup_aux` shipped two copies of a kernel for a release.
//
...
// launcher -- which sat in `kernels::mlp` and said `bf16` on its
...
// row states `bf16` for the JIT on exactly the same reading.
