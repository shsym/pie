...
//
// One `__global__`, and it is JIT-ONLY. `graph_pad.cu` included this file and
// held `launch_graph_pad_rows`; §43 deleted the file whole, because that
...
// exists in the tree, and it is the one below. Its launcher is
// `driver_internal::graph_pad_rows`, over the root `src/layout.rs` declares
// for this file; between §43 and that function nothing named this text at all.
//
// # Two spellings of the same integers
//
// The kernel was written against `<cstdint>` and now names the prelude's
// `u32`/`u8`/`i32` instead. That was a RENAME and not a conversion:
// `std::uint32_t` IS `unsigned int` on every target this tree builds for, so
// the `std::uint32_t*` arguments the deleted launcher passed to a `u32*`
// parameter went on meaning the same words, and the archive's object code was
// unchanged by it. The rename was forced -- NVRTC ships no standard library at
// all (the `stdlib_probe` measured 0 of 31 standard headers answering), so a
// `.cuh` in the carried header set that said `std::uint32_t` would compile
// under nvcc and nowhere else.
//
// # Why it is not a row
//
...
// invented one.
//
// Nor is there a caller that could state it: this launch is composed by the
// driver while it builds a graph-captured wave, not by a model text, so no
// `Source` names its operands. That is what keeps the Rust launcher in
// `driver_internal` rather than in `layout`'s `ROUTINES`.
//
//===----------------------------------------------------------------------===//
#pragma once
