...
//
// Every launch site names `T` explicitly — `update_appended<bf16>`.
...
// cache `u16*` and only becomes `bf16*` through a `reinterpret_cast`
...
// where the host needs it. `bf16` and the fixed-width integer names
