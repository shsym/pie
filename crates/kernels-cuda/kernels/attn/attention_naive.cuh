...
// `__nv_bfloat16` became the prelude's `bf16` (the same two bytes,
// through a struct that makes `bf16` and `f16` distinguishable in a row),
// `std::uint32_t`/`std::int32_t` became `u32`/`i32`, and
// `-INFINITY` became `neg_inf()` -- NVRTC does not define the
