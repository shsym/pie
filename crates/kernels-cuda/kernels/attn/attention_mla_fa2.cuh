...
//
// `kernels/attn/fa2.cuh` beside it is the pattern and its header is the
...
// `kernels_cuda::x::attn::mla_fa2` in Rust.
//
// The shape it replaced was the archive crate's
// `crates/kernels-cuda/kernels/attn/attention_mla.cu`, whose two `<<<>>>` --
// reached through `flashinfer::mla::BatchMLAPagedAttention` and
// `attention_mla_naive.cuh` -- were the last nvcc-compiled launches in the
// workspace. That file was host program throughout: zero `__global__`, zero
...
// Without it this root is rejected sixteen times: `shim/type_traits:253`,
...
// vendored FlashInfer closure -- and `kernels/unit.rs`'s `DEMANDS` says so
...
// because `kernels/flashinfer/attention/mla.cuh` is carried under
...
/// interior the two spell differently at the same `sizeof`; `shim/cuda/cmath:265-273`
