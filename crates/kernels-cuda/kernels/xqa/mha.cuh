...
// PIE: UPSTREAM CALLS THIS FILE `mha.cu`. It is carried as `mha.cuh`
// PIE: because `kernels-cuda` holds no translation units: a `.cu` is an
...
// PIE: `includeNames[]`; `src/source.rs` names this `attn/xqa/mha.cuh`
// PIE: after its path -- so the only thing that had to change with it is the
// PIE: one includer, `kernels/attn/attention_xqa_mha.cuh`, which is ours.
// PIE: Nothing inside `kernels/xqa/` includes a `.cu` by name (checked
