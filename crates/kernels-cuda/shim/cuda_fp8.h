...
// `kernels-cuda/csrc/src`, the ahead-of-time crate's own device code. That
// crate was the archive, deleted at `85c6c674b`, and every `kernels-cuda/`
// path in this file names its tree rather than this one's:
//
...
// fp8_pipeline_probe.rs` gated it against `nvcc`'s own `<cuda_fp8.h>` --
...
// is no hand-written converter to differ. That probe went with the whole of
// `kernels-cuda/examples/`, which makes the result a record rather than a
// run -- and leaves the property, not the harness, as the reason this file
// is cheap to trust.
//
...
/// vendor's, because the enumerators cross an ABI in one direction -- the
/// host passes one down as a four-byte integer, which is what
/// `src/jit/abi.rs`'s `fp8_kind` is -- and a shim that renumbered them would
/// turn every E4M3 into an E5M2 without a word.
...
struct __nv_fp8_e4m3 {
    ...
    /// domain, and `fp8_pipeline_probe` checked that they did.
