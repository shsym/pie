...
// replaces at gpt-oss's shape (`crates/driver-cuda/csrc/bench/moe_bench.cu`, gate_up ms):
    ...
    const StageHooks* hooks)
{
    ...
    // "attention-state merge is not built for this CUDA architecture" at the
    // first fire.
    //
    // Upstream fixed the cause instead of the symptom: the merge moved to
    // `ops/attention_merge_states.cu`, which is compiled on every
    ...
    // (`crates/driver-cuda/csrc/bench/moe_bench.cu`, gpt-oss shape, H100) says so plainly:
    //
    //   N       1       8      32
    ...
    for (int L = 0; L < cfg.num_hidden_layers; ++L) {
        ...
        // `rope_kind` term the fused path runs unscaled rotations and gpt-oss
        // silently loses the long-context range, which is the bug the
        // `apply_rope` below was introduced to fix.
        ...
        if (fused_rope_kv) {
            ...
            // The cfg's own rope, not a hardcoded plain one. gpt-oss's config
            // asks for YaRN, the shared `apply_rope_config` had already
            // resolved it, and this line used to pass `rope_theta` alone --
            // so every gpt-oss fire ran unscaled rotations and the model's
            // long-context range was never actually available. Plain Mixtral
            // declares no scaling and is unchanged.
