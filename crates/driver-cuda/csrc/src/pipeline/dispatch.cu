...
//
// The host (`tensor-compiler's codegen/src/cuda/`) is the only emitter, so both the
...
// `build_stage_lane_digest` answers "does this stage name X?" for the HOT
// path -- once per stage, in a walk it already makes, instead of a string
// comparison per lane per wave. That is why these two helpers were deleted
// upstream: nothing there still called them.
//
// They are back because six callers here do, and all six are COLD: the
// graph-eligibility veto and the page-mask replay scans run once per plan,
// not once per lane per fire. Paying a string walk there costs nothing and
// keeps the digest to the fields the per-lane loop actually reads.
...
struct StageLaneDigest {
    ...
    // Read in the same per-lane loop as `writes_page_mask`, for the same
    // reason: resolving the lora sink is what BUILDS the launch's lane table,
    // and asking by string once per lane per wave is what the digest exists
    // to avoid.
    bool writes_lora = false;
    ...
    for (const auto& normalized : stage.ops) {
        ...
        switch (op.tag) {
            ...
            case PTIR_OP_SINK_CALL:
                if (op.name_idx < stage.names.size()) {
                    if (stage.names[op.name_idx] == "attn_page_mask") {
                        digest.writes_page_mask = true;
                    } else if (stage.names[op.name_idx] == "lora") {
                        digest.writes_lora = true;
                    }
                }
                break;
...
void execute_declared_phase(
    ...
    for (std::size_t occurrence = 0;
        ...
        for (auto& lane_ptr : launch.lanes) {
            ...
            if (phase == PTIR_STAGE_PROLOGUE && digest.writes_lora) {
                ...
                for (std::size_t occurrence = 0;
                    ...
                    for (std::size_t first_index = 0;
                        ...
                        for (PreparedTask* member : members) {
                            ...
                            // `record_stage_channel_effects` takes the
                            // per-stage digest upstream computes once per
                            // wave through a lambda local to the dispatch
                            // loop, which this function is not inside. The
                            // hook-graph CAPTURE runs once per program set
                            // rather than once per fire, so building it here
                            // is cheaper than threading it through
                            // `PreparedTask`.
                            const StageLaneDigest member_digest =
                                build_stage_lane_digest(*member->plan);
                            record_stage_channel_effects(
                                *member->lane, *member->plan, member_digest);
                        }
