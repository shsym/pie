...
use driver::fire::{EventId, FireDescriptor, Phases, Sink, WindowTable, walk_phases};
...
pub struct Graphs {
    ...
    /// prebind after a launch. The engine's frame scheduler knows this at
...
pub struct FoldStats {
    ...
    /// Second instantiations of a hot bucket's template — lazy, on the first
    /// back-to-back same-bucket fire, so a cold bucket stays single-exec.
    pub twins: u64,
...
impl core::fmt::Display for FoldStats {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        ...
        for (why, count) in &self.refusals {
            write!(f, "\n[fold-stats]   refused by {why}: {count}")?;
        }
        Ok(())
    }
}

/// **THE ENGINE-REACHABLE MIRROR OF THE FOLD'S MOTION** — five counters of
/// [`FoldStats`], process-global, the same argument as
/// `crate::program::ports::resolved` (the envelope counter): a serving
/// engine's shell lives behind `Box<dyn Driver>` on a scheduler lane thread,
/// so an engine-level gate cannot ask the instance for [`Graphs::fold_stats`]
...
impl Armed {
    /// The seat that is NOT `active` — the prebind target, when a twin
    /// exists.
    fn idle(&self) -> Option<usize> {
        (self.seats.len() == 2).then_some(1 - self.active)
    }
}
...
) -> core::result::Result<(Vec<bool>, Vec<Patch>), String> {
...
impl Graphs {
    ...
    /// consumed by the prebind after the next launch. The engine's frame
    ...
    fn refusal(&mut self, why: &str) {
        for (held, count) in &mut self.fstats.refusals {
            if held == why {
                *count += 1;
                return;
            }
        }
        self.fstats.refusals.push((why.to_string(), 1));
    }
    ...
    pub fn fire_folded(&mut self, at: &Fire<'_>, run: &mut Run<'_>, place: &At) -> Result<Mode> {
        ...
        if self.pipeline && self.last_fold == Some(key) && armed.seats.len() == 1 {
            match armed.graph.instantiate(at.stream) {
                ...
                Err(fault) => {
                    self.refusal(&format!("the twin exec refused to instantiate: {fault}"));
                }
            }
        }
        ...
        if armed.bindings.contains_key(&at.key) {
            ...
            let seat = armed.idle().unwrap_or(armed.active);
                ...
                Self::apply_binding(armed, seat, &at.key, at.stream, &mut self.fstats, library, false)
            {
                ...
                self.fold_refuse(key, "a binding refused to apply mid-list");
                let _ = fault;
                walk_capture(at, run, place, Streams::Serial)?;
                return Ok(Mode::Declined);
            }
        ...
        *seen += 1;
        ...
        if let Some(why) = census.fault.take() {
            ...
            self.refusal(&format!("the real capture's census broke: {why}"));
            return Ok(Mode::Declined);
        }
        let walked = nodes::walk(&graph)?;
        drop(graph);
        ...
        let brought = match if forked {
            ...
            Err(why) => {
                self.fold_unaligned.insert(at.key.clone());
                self.refusal(&format!("the real capture's census does not place every node: {why}"));
                return Ok(Mode::Declined);
            }
        };
    ...
    /// engine seals early and posts at run-ahead depth 2
    /// (`engine::scheduler::frame`, `DEFAULT_DISPATCH_DEPTH`), the dev C++
    ...
    fn prebind(&mut self, just_launched: FoldKey, stream: *mut core::ffi::c_void) {
        ...
        // too — that arm is exactly what an engine A/B diffs against.
        {
            use core::sync::atomic::Ordering::Relaxed;
        ...
        match Self::apply_binding(armed, seat, &hsig, stream, &mut self.fstats, library, true) {
            ...
            Err(_fault) => {
                ...
                if armed.seats.len() == 2 {
                    ...
                    armed.active = 0;
                    self.refusal("a prebind refused to apply mid-list; the twin is dropped");
                } else {
                    self.fold.remove(&hkey);
                    self.fold_refuse(hkey, "a binding refused to apply mid-list");
                }
            }
        }
    }
...
mod tests {
    ...
    fn a_segment_the_template_never_captured_refuses_by_name() {
        ...
        assert!(why.contains("region 7"), "{why}");
    }
    ...
    fn a_segment_whose_node_count_moved_refuses_by_name() {
        ...
        assert!(why.contains("2 template nodes") && why.contains("brought 1"), "{why}");
    }
    ...
    fn every_fold_refusal_is_tallied_by_name_and_a_bucket_refusal_sticks() {
        ...
        assert_eq!(
            stats.refusals,
            vec![
                ("arming at 3 classes: nvrtc under capture".to_string(), 2),
                (
