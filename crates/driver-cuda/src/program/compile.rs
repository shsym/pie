...
//! [`driver::cache_identity`] (backend × device × stage signature × four
...
use driver::driver_api::program::{KernelKind, LibraryOp, RegionKind};
...
pub struct Target {
    ...
    /// and [`driver::cache_identity`] has no seat for it (it is shared with
...
impl Disk {
    ...
    fn path(&self, key: &str, region_index: u32) -> Option<PathBuf> {
        ...
        Some(directory.join(format!(
            ...
            driver::tensor_ir::fnv1a64(key.as_bytes())
        )))
    }
}
...
pub fn disk_key(identity: &str, source: &str) -> String {
    let hash = driver::tensor_ir::fnv1a64(source.as_bytes());
