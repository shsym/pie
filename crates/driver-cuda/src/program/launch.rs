...
//! crate — [`driver::LaneHeader`], [`driver::LaneRecord`],
//! [`driver::LaneChannelSlot`], [`driver::ValueDesc`], [`driver::OpParams`] —
...
//! exist is the plan's, where they sit in scratch is [`driver::layout`]'s,
...
//! slot.
//!
//! [`ChannelState`]: driver::ChannelState

use driver::driver_api::program::LaunchStagePlan;
...
///
/// [`driver::OpParams`] is 64 bytes (sixteen `u32`); CUDA's `M1OpParams` is 88
...
impl CudaOpParams {
    ...
    /// of the 64-byte prefix, so a field inserted into [`driver::OpParams`] is
...
pub fn wire_to_native(dtype: DType, numel: usize, wire: &[u8]) -> Result<Vec<u8>> {
    ...
    let want = driver::wire_cell_bytes(dtype, numel);
    if wire.len() != want {
...
pub fn native_to_wire(dtype: DType, numel: usize, native: &[u8]) -> Result<Vec<u8>> {
    ...
    let mut out = vec![0u8; driver::wire_cell_bytes(dtype, numel)];
...
impl ChannelShape {
    ...
    #[must_use]
    pub fn of(declared: &driver::driver_api::program::LaunchChannel) -> ChannelShape {
        ChannelShape {
            numel: declared
                .shape
                .iter()
...
/// One instance's channel cells, resident on the device.
///
/// **CELLS ONLY — THE CURSORS ARE THE HOST'S.** The rewrite kept four device
/// cursor arrays because its readiness gate and its commit bump were device
/// kernels; this half runs both on the host, out of
/// [`driver::program`](driver)'s own arithmetic (see [`session`]), so the only
/// thing the device needs is the bytes a cell holds and the two addresses a
/// lane's [`LaneChannelSlot`] points at. What that buys is not speed: it is
/// that the host interpreter and this shell then share ONE implementation of
/// readiness and commit, and the parity test is left diffing the part that
/// actually differs — the emitted arithmetic.
///
...
impl Rings {
    ...
    pub fn allocate(shapes: &[ChannelShape]) -> Result<Rings> {
...
pub struct Prepared {
    ...
    /// Where the kernel writes its verdict. One `u32`, started at one every
    /// fire, and the kernel CLEARS it to refuse rather than raising anything.
    commit: Buffer,
    channel_count: u32,
    value_count: u32,
    scratch_stride: u32,
...
impl Prepared {
    ...
    /// shared across instances, which is a decision about the engine's
    ...
    /// `extents` or the scratch exceeds what [`driver::layout`] permits, and
    ...
    pub fn build(
        ...
        let commit = Buffer::zeroed(size_of::<u32>())?;
        write_record(
            &mut table_host,
    ...
    /// commit slot set.
    ///
    /// The commit slot starts at ONE, not zero: the kernel reads it first and
    /// returns if it is clear, so a zero start would run every region and do
    /// nothing.
    ///
    /// # Errors
    ///
    ...
    pub fn refresh(
        ...
        self.commit.stage(stream, 0, &1u32.to_le_bytes())?;
        Ok(())
    }
    ...
    ///
    /// The kernel CLEARS this to refuse — a stale ABI, a fault, a readiness
    /// miss it observed — so a cleared slot must not publish whatever the host
    /// thought.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn only_bool_is_packed_and_the_round_trip_is_lossless() {
        for (dtype, numel) in [
            ...
            let wire = driver::wire_cell_bytes(dtype, numel);
            if dtype == DType::Bool {
