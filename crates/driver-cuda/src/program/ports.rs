...
//! page CSR — is pure arithmetic over the KV length, which the engine's host
...
//! token stops travelling through the engine's asynchronous host plane
...
//!
//! [`Port::consumes`]: driver::tensor_ir::registry::Port::consumes
//! [`PortMask::DECODE_ENVELOPE`]: driver::tensor_ir::registry::PortMask::DECODE_ENVELOPE
//! [`PortMask::DEVICE_GEOMETRY`]: driver::tensor_ir::registry::PortMask::DEVICE_GEOMETRY

use std::sync::atomic::{AtomicU64, Ordering};

use driver::tensor_ir::DType;
use driver::tensor_ir::registry::Port;
use driver::{ExecPlan, Value};
...
impl Envelope {
    ...
    /// `crates/engine/tests/cuda_program_epilogue.rs` authors on purpose.
    #[must_use]
    ...
    /// decode-envelope lane arrives carrying placeholder ids — the engine
