...
//!
//! ```text
//! cargo test -p driver-cuda --features cuda-13 --release --test stream_overlap -- --nocapture
//! ```
//!
...
/// (`driver_cuda::EXCLUSIVE`) and P6 orders them.
///
...
mod gemma {
    ...
    fn load(side_streams: u32) -> (Shell, tokenizer::Tokenizer) {
        ...
        let shell = Shell::load(Boot {
            ...
            // (`driver::fire::EagerSink`), so an eager arm would measure
            // nothing and prove nothing.
            graphs: driver_cuda::Graphs::On,
        })
