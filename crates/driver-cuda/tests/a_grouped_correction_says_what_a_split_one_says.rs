...
//!
//! ```text
//! cargo test -p driver-cuda --features cuda-13 \
...
fn arm(what: &str, grouped: bool) -> Option<Arm> {
    ...
    let mut mixed: Vec<driver::fire::Lane> = Vec::with_capacity(8);
    for &(adapted, captures) in &CLASSES {
        mixed.push(driver::fire::Lane::new(word(1, adapted, captures), 1));
    }
    for &(adapted, captures) in &CLASSES {
