...
/// Where the attention dispatch's output lands, read off the op JOIN
/// first and the neighbour launch second.
///
/// The fixtures used to read only the neighbour -- "the launch after the
/// dispatch is the o_proj, so its first operand is the slot the dispatch
/// wrote". That is a fact about where a statement SITS, and it holds
/// under `GuardMode::Resolve`, where the guards have already deleted
/// every arm this fire does not take. It does NOT hold under `Union`,
/// where every arm is present and the neighbour belongs to some other
/// body -- which is exactly what `.wiki/driver/graph.md` §2 says about
/// the trick, and why `serve::launch`'s own `attention_landing` tries the
/// join first. These are fixtures FOR that shell; they should read what
/// it reads.
fn attention_landing(
    l: &model_compiler::lower::Lowered,
    dplan: &driver_cuda::bind::DispatchPlan,
    fi: usize,
) -> Option<usize> {
...
fn a_driver_internal_binding_seeds_the_envelope_tier() {
    ...
    let mut ops = LiveKvCacheOps::new(stream.as_ref(), &alloc);
...
fn the_executor_prefix_runs_the_anchor_decode_on_device() {
    ...
    struct Live {
        embed: driver_cuda::device::DeviceBuffer,
        ones: driver_cuda::device::DeviceBuffer,
        zeros: driver_cuda::device::DeviceBuffer,
        ids: driver_cuda::device::DeviceBuffer,
...
fn zero_weight_decode(leg: Leg) {
    ...
    let mut named_bufs: BTreeMap<ValueId, driver_cuda::device::DeviceBuffer> = named_widths
        .iter()
        .map(|(&v, &w)| {
    ...
    struct Live<'a> {
        ...
        named: &'a mut BTreeMap<ValueId, driver_cuda::device::DeviceBuffer>,
    }
    ...
    //
    // Both halves are `.wiki/driver/graph.md` §5 (1). The window class is
    // a guard now (§4.1), so a DECODE fire's lowering contains the prefill
    // arm -- and under `Union` that arm is RECORDED whether this fire
    // takes it or not. An arm whose plan was never raised refuses, which
    // abandons the recording, and an abandoned recording is the documented
    // host segfault. Prepared state belongs to the bucket, so the fixture
    // raises every plan the geometry permits, exactly as the shell does.
    //
    // Its own workspace because a FlashInfer plan writes its schedule into
    // the workspace it was raised against: two plans in one is one
    // clobbering the other, and the decode plan above is the one that
    // would lose.
    let mut prefill_ws =
    ...
    //
    // Under `GuardMode::Union` EVERY arm is recorded whether this fire
    // takes it or not, so the `_capture` and `_custom` arms are walked
    // here, and an arm whose state the fixture declined to build refuses
    // -- which abandons the recording, and an abandoned recording is the
    // documented host segfault (`serve::launch`: "a run that refused
    // part-way leaves a recording whose nodes the builder has already
    // dropped"). This fixture is FOR that shell; it has to prepare what
    // the shell prepares.
    let kv_indptr_h: Vec<u32> = (0..=ROWS as u32).collect();
    let kv_lens_h = vec![1u32; ROWS];
    let mask_plan = driver_cuda::fire::page_mask::element_mask::plan_causal(
    ...
    mask_buf
        ...
        .expect("mask csr");
    let sink_plan = driver_cuda::fire::attn_score::plan_score_sink(
        &kv_indptr_h,
        &kv_lens_h,
        PAGE,
        Q_HEADS as u32,
    ...
    .expect("a score sink for this geometry");
    let mut sink_buf = alloc.alloc(sink_plan.bytes).expect("sink");
    sink_buf
        .write_at(
    ...
    let attn = AttnCtx {
        ...
        prefill_plan: pplan_cache.as_ptr(),
        workspace: ws.view(),
        prefill_workspace: prefill_ws.view(),
        layers,
        q_out: named_bufs[&q_pin_value].as_ptr(),
        score_out: scores.as_ptr().cast(),
        score_indptr_d: score_indptr.as_ptr().cast(),
        // The fold's own extent. It shared `score_out`'s slot while the
        // sink was always empty; a real one needs two, or the fold
        // overwrites the rows it is folding.
        folded_out: sink_buf
            .as_ptr()
            .wrapping_byte_add(sink_plan.folded_offset)
            .cast(),
        // The resident causal mask. Under `Union` the `_custom` arm is
        // RECORDED whether this fire takes it or not, and an arm whose
        // state the fixture declined to build refuses -- which abandons
        // the recording, and an abandoned recording is the documented host
        // segfault. `.wiki/driver/graph.md` §5 (1) is about exactly this,
        // and this fixture is for the shell that implements it.
        mask_d: mask_buf.as_ptr().cast::<u8>().cast_const(),
        mask_indptr_d: mask_buf
            .as_ptr()
            .wrapping_byte_add(mask_plan.indptr_offset)
            .cast::<i32>()
            .cast_const(),
    ...
    let ran = if matches!(leg, Leg::Captured | Leg::CapturedUnion | Leg::Reused) {
        use driver_cuda::device::{PredicateWord, SupergraphBuilder};
        use driver_cuda::fire::recordings::fire_predicates;
    ...
    ws.release(&mut sops);
    // The prefill plan's workspace, released beside the decode plans'.
    prefill_ws.release(&mut sops);
...
fn the_full_zero_weight_prefill_walks_every_launch() {
    ...
    // A PREFILL: one request contributing every row, so every row is
    ...
    let named_bufs: BTreeMap<ValueId, driver_cuda::device::DeviceBuffer> = named_widths
        .iter()
        .map(|(&v, &w)| {
    ...
    struct Live<'a> {
        ...
        ones: *const std::ffi::c_void,
        zeros: *const std::ffi::c_void,
        named: &'a BTreeMap<ValueId, driver_cuda::device::DeviceBuffer>,
    }
    impl Resolver for Live<'_> {
        fn weight(&mut self, name: &str) -> Option<*const std::ffi::c_void> {
            Some(if name.contains("embed") {
