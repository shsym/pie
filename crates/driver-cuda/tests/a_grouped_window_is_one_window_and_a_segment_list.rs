...
//!   `driver::fire::walk` turns its launch loop once for this region and a
...
fn the_grouped_region_gets_one_window_whose_segments_are_the_splits_own_spans() {
    ...
    for &at in &regions {
        ...
        // ONE WINDOW, NOT `r` — the number `driver::fire::walk` loops on.
...
/// `driver_cuda::serve`'s `PIE_CUDA_GROUPED` is opt-in — it is an improvement,
