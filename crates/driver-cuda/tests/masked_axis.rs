...
fn every_sku_probes_its_caches() {
    ...
    for (sku, _, trace, _) in model::catalog() {
        if let Err(fault) = driver_cuda::store::kv::probe(&trace(Platform::Cuda)) {
...
fn a_mixed_fire_of_all_three_classes_says_what_each_lane_says_alone() {
    ...
    shell.open(0).expect("slot 0 opens");
    let seated = shell
        .fire_seated(&[Seated::of(driver_cuda::Lane {
            slot: 0,
...
fn a_masked_composition_captures_once_and_replays_identically() {
    ...
    let mut run = |mode: driver_cuda::Graphs| {
        shell.set_mode(mode);
    ...
    let eager = run(driver_cuda::Graphs::Off);
    let shaped = run(driver_cuda::Graphs::Shaped);
    let replayed = run(driver_cuda::Graphs::On);
