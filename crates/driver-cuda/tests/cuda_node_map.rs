...
use driver_cuda::device::graph::Event;
use driver_cuda::device::map::{self, Component, Diff, NodeMap, Refused};
use driver_cuda::device::nodes::{self, Walked};
use driver_cuda::device::{Buffer, Graph};
...
fn a_scalar_that_changed_between_captures_is_the_only_thing_the_patch_names() {
    ...
    assert_eq!(
        patches[0].moved,
        vec![Component::Arg { at: 1, word: 0 }],
        "the SECOND parameter — the pointer never moved"
    );
    assert_eq!(
...
fn a_forked_pair_whose_arguments_disagree_refuses_the_alignment_by_name() {
    ...
    assert_eq!(*component, Component::Arg { at: 1, word: 0 });
