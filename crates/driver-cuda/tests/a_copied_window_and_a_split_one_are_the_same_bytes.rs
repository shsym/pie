...
//!
//! ```text
//! cargo test -p driver-cuda --features cuda-13 --release \
...
fn fire_it(
    shell: &mut Shell,
    fed: &[Vec<u32>],
) -> (Vec<Vec<f32>>, Vec<Vec<LayerScores>>, driver_cuda::FireCost) {
    for slot in 0..3 {
...
fn predicted_wide() -> (u32, u32) {
    ...
    let lanes = [
        ...
        driver::fire::Lane::new(wide_word(4, true, true), 4),
...
fn register_loud(shell: &mut Shell, id: u32) {
    let built: Vec<(String, Vec<u8>)> = shell
        ...
        .collect();
    let planes: Vec<driver_cuda::AdapterPlane<'_>> = built
        .iter()
        .map(|(bank, bytes)| driver_cuda::AdapterPlane {
            bank: bank.as_str(),
            bytes,
        })
        .collect();
...
fn what_a_copy_and_a_split_cost_on_this_device() {
    ...
    for (what, fed) in [("3 rows", &short), ("201 rows", &long)] {
        ...
        for copies in [false, true, false, true] {
            ...
            let at = Instant::now();
            let mut cost = driver_cuda::FireCost::default();
            for _ in 0..FIRES {
