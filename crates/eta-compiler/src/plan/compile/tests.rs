...
use eta_ir::registry::{KernelInfo, ModelProfile, Port};
...
/// `eta-dsl` traces — so it cannot drift away from what the matcher will meet
...
fn byte_identical_nucleus_dags_share_signature_and_library_plan() {
    ...
    assert_eq!(
        eta_ir::container::encode(&first.container),
        eta_ir::container::encode(&second.container)
    );
    let first = compile_stage(&first, Stage::Epilogue).unwrap();
    let second = compile_stage(&second, Stage::Epilogue).unwrap();
    assert_eq!(first.signature, second.signature);
