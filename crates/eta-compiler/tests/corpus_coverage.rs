...
use eta_compiler::plan::{LibraryOp, RegionKind, ScheduleTemplate};
use eta_ir::op::{IntrinsicId, OP_TABLE, Op};
...
/// All four stages, including `OnAttn` -- the stage no golden ever entered,
/// reached by the only intrinsics that stand inside the graph, `Query` and
/// `Layer`. `attn_score` is not one of them: it is read whole at the epilogue,
/// so nothing about it says which stage this is.
#[test]
