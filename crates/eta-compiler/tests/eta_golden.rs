...
fn step_line_k(
    ...
    b: &BoundTrace,
    inputs: &PassInputs,
    host: &mut dyn eta_compiler::eval::interp::KernelHost,
    fed: String,
) -> String {
    ...
    let c = TraceContainer {
        ...
        stages: vec![StageProgram {
            ...
            ops: vec![
                ...
                Op::PivotThreshold {
                    input: 9,
                    predicate: eta_ir::Predicate::CummassLe(2),
                },
...
fn golden_pivot_predicates_multistage() {
    ...
    let mask_p = b.p(Op::PivotThreshold {
        input: probs,
        predicate: eta_ir::types::Predicate::CummassLe(p),
    });
    let mask_t = b.p(Op::PivotThreshold {
        input: probs,
        predicate: eta_ir::types::Predicate::ProbGe(thr),
    });
    b.p(Op::ChanPut {
        chan: 2,
        value: mask_p,
...
impl eta_compiler::eval::interp::KernelHost for QuestKernels {
    fn kernel(
        &mut self,
        name: &str,
        args: &[Value],
        result: eta_ir::types::ValueType,
    ) -> Result<Value, String> {
...
fn pentathlon_profile() -> ModelProfile {
    ...
    p.vocab = PV;
    p.kernels.push(eta_ir::registry::KernelInfo {
...
fn quest_tap(budget_ch: u32) -> StageProgram {
    ...
    let pm = b.p(Op::PivotThreshold {
        input: scores,
        predicate: eta_ir::types::Predicate::RankLe(buds),
    });
    b.p(Op::SinkCall {
        name: 0,
        args: vec![pm],
...
fn golden_extern_contrastive() {
    use eta_compiler::eval::interp::ExternChannel;
    ...
    let mut am =
        eta_compiler::eval::interp::Instance::new_with_externs(&am_b, &[], &[(0, ring.clone())])
            .unwrap();
    let mut ex =
        eta_compiler::eval::interp::Instance::new_with_externs(&ex_b, &[], &[(0, ring)])
            .unwrap();
