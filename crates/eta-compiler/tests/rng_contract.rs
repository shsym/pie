...
//!
//! Every backend has to reproduce ETA's noise bit for bit, so each of them
//! needs the same magic numbers. These tests check that the numbers reach the
//! generated sources by projection from `eta_ir::rng::RNG_FORMULA` rather than
...
use eta_compiler::codegen::rng::{cuda_device_functions, generate_msl_preamble};
use eta_ir::rng::{RNG_FORMULA, UNIFORM_MAX, hash_uniform, keyed_seed};
...
fn generated_rng_artifacts_are_uptodate() {
    ...
    check_or_regenerate(
        &root.join("crates/eta-compiler/include/ptir_rng.generated.metal"),
        &msl,
    );
}
...
///
/// `eta-compiler` owns the preamble's text and commits it once, under
...
struct Allowlists {
    ...
    /// splitmix64 input generator and not an ETA stream.
    ///
...
fn allowlists() -> Allowlists {
    Allowlists {
        ...
        owners: &[
            "crates/eta-ir/src/rng.rs",
            // `crates/eta-compiler/include/rng_contract.generated.h` was the
            ...
            // for.
            "crates/eta-compiler/include/ptir_rng.generated.metal",
        ...
        stride: &[
            ...
            // golden-ratio word again, and nothing to do with the ETA stream.
            ...
            // them reproduces the ETA stream, which is what this guard is
        ...
        // finalizer in an engine test is not an ETA stream.
        //
        ...
        // **a splitmix64 input generator, not an ETA stream** — driving the
...
fn rng_magic_is_owned_by_the_contract() {
    ...
    assert!(
        found.is_empty(),
        "ETA RNG magic is independently transcribed in {} place(s):\n  {}",
        found.len(),
        found.join("\n  ")
    );
}
...
fn emitted_cuda_runtime_carries_the_rng_preamble() {
    let spliced = eta_compiler::codegen::cuda::singleton_runtime_source();
