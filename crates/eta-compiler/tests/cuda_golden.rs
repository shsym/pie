...
fn golden_cuda_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/golden-cuda")
}
...
fn runtime_matches_oracle() {
    ...
    dump.field(
        "fnv1a64",
        &format!("0x{:016x}", eta_ir::fnv1a64(runtime.as_bytes())),
    );
    dump.end();
    compare(&dump);
}
...
fn every_live_device_file_is_spliced_into_a_kernel_verbatim() {
    ...
    assert_eq!(
        files.len(),
        6,
        "crates/eta-compiler/runtime/cuda/ holds six device files; a new one needs \
...
/// `eta_compiler::plan::stage_identity` is a cache key, so it must not move by accident.
///
...
/// other test noticing. Bump `eta_compiler::plan::COMPILER_VERSION` when regenerating,
...
fn stage_identity_is_pinned() {
    ...
    for stage in corpus_stages() {
        let _ = writeln!(
            ...
            "{} {:016x}",
            stage.id(),
            eta_compiler::plan::stage_identity(&stage.plan)
        );
    }
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/golden-stage-identity.txt");
    if let Some(expected) =
        provenance::body_to_diff(&path, &rendered, provenance::Regenerate::Foreign)
    {
        assert_eq!(
            rendered, expected,
            "eta_compiler::plan::stage_identity moved: a published graph-cache key changed, \
             so eta_compiler::plan::COMPILER_VERSION must move with it"
        );
    }
}
...
fn emit_engine_test_kernel_fixtures() {
    ...
    use eta_ir::container::decode as decode_container;
    ...
    for path in entries {
        ...
        // decoded ETA. Those tests are gone, `register_program` takes an
        // owned `ProgramRegistration`, and `engine::image` — the encoder
        ...
        // argmax:
        //   `region <stage> <region> <second-party> <generated-valid>
        //           <argmax-count> <skipped...>`
        //   `argmax <node> <source_value> <intrinsic> <requires_single_row>`
        //
        // The two flag columns were one `flags` word over `REGION_*` bits.
        // They are named booleans on the record now, and printed as `0`/`1`
        // here so a fixture stays a column of digits.
        let mut regions = String::new();
        for region in eta_compiler::codegen::cuda::region_analysis::analyze_program(&stages) {
            let mut header = format!(
                "region {} {} {} {} {}",
                region.stage_index,
                region.region_index,
                u8::from(region.second_party_supported),
                u8::from(region.generated_valid),
            ...
            for record in &region.direct_argmax {
                let _ = writeln!(
                    ...
                    record.source_value,
                    record.intrinsic,
                    u8::from(record.requires_single_row)
                );
            }
        }
