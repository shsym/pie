...
/// named classes in `eta_compiler::codegen::fault`.
///
...
fn every_fault_code_is_readable_in_its_kernel_vocabulary() {
    let declared: Vec<u32> = eta_compiler::codegen::fault::classes()
        .into_iter()
        .flat_map(|class| {
    ...
    for (trace, backend, kernel) in every_emitted_kernel() {
        ...
        for code in literals {
            if speaks_tags {
                let is_a_tag = u8::try_from(code).is_ok_and(|tag| eta_ir::op::spec(tag).is_some());
                let is_a_recorded_alias = eta_compiler::codegen::fault::TAG_ALIASES
                    .iter()
                ...
                assert!(
                    ...
                    "{where_}: reports op tags as faults, and also writes the \
                     ...
                     do, or declare it in eta_compiler::codegen::fault."
                );
            } else {
                saw_declared += 1;
                assert!(
                    ...
                    "{where_}: writes fault {code:#x}, which \
                     eta_compiler::codegen::fault::classes() does not name, and dispatches no \
...
///
/// 918 -> 920 when the attention-score door opened: the two
/// `extended_attn_score` regions that read `IntrinsicId::AttnScore` used to
/// answer `UnbindableIntrinsic` on the single-lane Metal path and emit no
/// kernel at all, and they emit one each now. The corpus GREW by exactly the
/// regions whose refusal became a source, which is the shape of growth this
/// number is here to make anyone state.
#[test]
fn the_corpus_these_tests_walk_is_the_size_it_was_measured_at() {
    assert_eq!(
        every_emitted_kernel().len(),
        920,
        "the corpus is {} long, not the 920 these loops were measured \
         against",
        every_emitted_kernel().len()
    );
}
