...
//!   validator takes a native [`eta_compiler::plan::CompiledStage`] whose types make most
...
use eta_compiler::codegen::metal::preamble::grouped_preamble;
...
/// `crates/eta-compiler/runtime/metal/ptir_m1_runtime.metal` now. Everything
...
///
/// **THE COMPARISON IS AGAINST THE ORACLE-ERA TEXT, NOT THE LIVE FILE**, and
/// the day that stopped being a distinction is the day the mechanism
/// `device_text.rs` was written for first fired. The runtime grew an F32 arm
/// for the score rectangle (`.wiki/alto/attn-score.md` §4), so the live file
/// is no longer the bytes the dump was taken with — and holding it to them
/// would be exactly the decay that header warns about: a suite that forbids
/// anyone from touching a kernel. `golden-msl/oracle-inputs/` carries the
/// text the oracle read, `OracleInputs::rewrite` puts it back, and what is
/// still compared byte for byte is everything the EMITTER decides. The other
/// half of the guarantee is
/// `every_live_device_file_is_spliced_into_a_kernel_verbatim`, which is what
/// makes the substitution incapable of hiding an emitter change.
#[test]
fn embedded_runtime_matches_the_oracle() {
    ...
    let runtime = inputs.rewrite(&runtime_prefix());
    assert_eq!(
        (runtime.len(), eta_ir::fnv1a64(runtime.as_bytes())),
        field("# @runtime:"),
        "crates/eta-compiler/runtime/metal/ptir_m1_runtime.metal has drifted from the \
         frozen C++ oracle dump, and {}",
        inputs.hint()
    );
...
fn every_live_device_file_is_spliced_into_a_kernel_verbatim() {
    ...
    assert_eq!(
        files.len(),
        2,
        "crates/eta-compiler/runtime/metal/ holds two device files; a new one needs \
