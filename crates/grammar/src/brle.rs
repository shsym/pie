...
mod tests {
    ...
    fn mask_range_carves_hole_in_trues() {
        let mut b = RunMask::from_slice(&[true; 6]);
    ...
    fn is_masked_preserves_input_order() {
        let b = RunMask::from_slice(&[false, true, true, false]);
        assert_eq!(b.is_masked(&[3, 1, 0, 2]), bv(&[false, true, false, true]));
    }

    #[test]
    fn is_range_all_value_boundary() {
        let b = RunMask::from_slice(&[false, false, true, true, false]);
    ...
    fn extend_merges_matching_boundary_runs() {
        let mut a = RunMask::from_slice(&[false, true]);
        let b = RunMask::from_slice(&[true, false]);
        a.extend(&b);
    ...
    fn remove_range_splices_correctly() {
        let mut b = RunMask::from_slice(&[false, true, true, true, false]);
        b.remove_range(1, 4);
    ...
    fn naive_from_bitmask(bm: &[u32], total: usize) -> RunMask {
        let bools: Vec<bool> = (0..total)
            .map(|i| (bm[i / 32] >> (i % 32)) & 1 != 0)
            .collect();
        RunMask::from_slice(&bools)
    }

    fn assert_bitmask_matches_naive(bm: &[u32], total: usize) {
        let fast = RunMask::from_bitmask(bm, total);
    ...
    fn from_bitmask_roundtrip_simple() {
        let bm = [0b1011u32];
        let b = RunMask::from_bitmask(&bm, 4);
    ...
    fn from_bitmask_roundtrip_all_false() {
        let bm = [0u32; 4];
        let b = RunMask::from_bitmask(&bm, 128);
        assert_eq!(b.len(), 128);
    ...
    fn from_bitmask_partial_last_word() {
        let bm = [0x3FFu32];
        let b = RunMask::from_bitmask(&bm, 10);
        assert_eq!(b.len(), 10);
    ...
    fn fill_from_bitmask_reuses_buffer() {
        let bm1 = [0xFFu32; 4];
        let bm2 = [0u32; 4];
        let mut brle = RunMask::from_bitmask(&bm1, 128);
    ...
    fn page_bits(b: &RunMask, ps: u32, num_pages: u32, total_seq_len: u32) -> Vec<u64> {
    ...
    fn droppable_pages_attention_sink_pattern() {
        let b = RunMask::from_vec(vec![0, 4, 252, 64]);
        assert_eq!(b.len(), 320);
    ...
    fn droppable_pages_window_pattern() {
        let b = RunMask::from_vec(vec![240, 80]);
        assert_eq!(b.len(), 320);
        let bits = page_bits(&b, 16, 20, 320);
    ...
    fn droppable_pages_partial_page_false_run_not_eligible() {
        let b = RunMask::from_vec(vec![5, 22, 5]);
        assert_eq!(b.len(), 32);
    ...
    fn droppable_pages_aligned_false_run_eligible() {
        let b = RunMask::from_vec(vec![0, 16, 16, 16]);
        assert_eq!(b.len(), 48);
        let bits = page_bits(&b, 16, 3, 48);
    ...
    fn droppable_pages_cross_word_boundary() {
        let b = RunMask::from_vec(vec![0, 16, 16 * 98, 16]);
        let total = 16 * 100;
    ...
    fn rebuild(buffer: Vec<u32>) -> RunMask {
        RunMask::from_vec(buffer)
    }
    ...
    fn write_skipping_drops_middle_false_run_collapses_trues() {
        let b = RunMask::from_vec(vec![4, 16, 16, 4]);
        let mut out = Vec::new();
    ...
    fn write_skipping_drops_partial_false_run() {
        let b = RunMask::from_vec(vec![32, 8]);
        let mut out = Vec::new();
    ...
    fn write_skipping_drops_leading_false_run_keeps_zero_prefix() {
        let b = RunMask::from_vec(vec![16, 16]);
        let mut out = Vec::new();
