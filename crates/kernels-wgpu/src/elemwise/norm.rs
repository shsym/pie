...
use crate::encode::{Arg, Ctx, Fire, Grid, dtype_dispatch, elementwise, nonzero, refuse, stated};
...
pub(crate) fn flat_words(lanes: [u32; 3]) -> Grid {
    words(lanes)
}

fn words(lanes: [u32; 3]) -> Grid {
    Grid::of([lanes[0].div_ceil(2), 1, 1], [FLAT_GROUP, 1, 1])
}
...
fn rms_grid(op: &'static str, width: u32, axis: u32, rows: u32) -> Result<Grid, Error> {
    ...
    if !axis.is_multiple_of(2) {
        return Err(refuse(
            op,
            format!("the {axis}-wide normed axis is odd: a bf16 word holds a pair"),
        ));
    }
...
fn rms_row(
    ...
    ctx.fire(
        Fire::at("norm/rms.wgsl", entry).apply(grid),
        &[
            x.arg(),
            weight.arg(),
            y.arg_mut(),
...
pub fn rmsnorm_no_scale(
    ...
    ctx.fire(
        Fire::at("norm/vector.wgsl", entry).apply(grid),
...
fn gated_rms(
    ...
    ctx.fire(
        Fire::at("norm/gated_rms.wgsl", entry).apply(grid),
        &[
            x.arg(),
            gate.arg(),
            weight.arg(),
...
pub fn residual_add(ctx: &Ctx<'_>, x: Tensor, y: Tensor) -> Result<(), Error> {
    ...
    ctx.fire(
        Fire::at("norm/residual_add.wgsl", entry).apply(words(lanes)),
...
pub fn add_bias(ctx: &Ctx<'_>, bias: Tensor, out: Tensor) -> Result<(), Error> {
    ...
    let lanes = elementwise(OP, out.width, out.rows)?;
    ctx.fire(
        Fire::at("norm/add_bias.wgsl", entry).apply(words(lanes)),
        &[
            out.arg_mut(),
            bias.arg(),
...
pub fn layernorm(
    ...
    ctx.fire(
        Fire::at("norm/layernorm.wgsl", entry).apply(grid),
        &[
            x.arg(),
            weight.arg(),
            bias.arg(),
...
pub fn standardize(ctx: &Ctx<'_>, bias: Tensor, scale: Tensor, out: Tensor) -> Result<(), Error> {
    ...
    ctx.fire(
        Fire::at("norm/standardize.wgsl", entry).apply(words(lanes)),
        &[
            out.arg_mut(),
            bias.arg(),
            scale.arg(),
...
pub fn mul_scalar(ctx: &Ctx<'_>, s: f32, x: Tensor) -> Result<(), Error> {
    ...
    ctx.fire(
        Fire::at("norm/layer_scalar.wgsl", entry).apply(words(lanes)),
        &[x.arg_mut(), s.arg(), lanes[0].arg()],
    )
}
...
pub fn res_blend(
    ...
    if !hidden.is_multiple_of(2) {
        return Err(refuse(
            OP,
            format!("a {hidden}-wide row is not a whole number of bf16 words"),
        ));
    }
    ...
    ctx.fire(
        Fire::at("norm/res_blend.wgsl", entry).apply(Grid::of([lanes, 1, 1], [RMS_GROUP, 1, 1])),
        &[
            prefix.arg(),
            stack.arg(),
            weight.arg(),
