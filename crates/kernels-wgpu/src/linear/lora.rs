...
pub fn correct(
    ...
    if !out_width.is_multiple_of(2) {
        return Err(refuse(
            OP,
            format!("the output width {out_width} is odd: a bf16 word holds a pair"),
        ));
    }
    ...
    ctx.fire(
        Fire::at("gemm/lora.wgsl", entry).apply(Grid::of([GROUP, rows, 1], [GROUP, 1, 1])),
        &[
            x.arg(),
            bank_a.arg(),
            bank_b.arg(),
