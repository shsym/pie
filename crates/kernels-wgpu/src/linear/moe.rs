...
const ROUTE_FILE: &str = "moe/route.wgsl";

const QMV_FILE: &str = "moe/qmv_routed.wgsl";
...
fn route_rows(op: &'static str, width: u32, rows: u32) -> Result<Grid, Error> {
    ...
    nonzero(op, "rows", rows)?;
    let elements = u64::from(width) * u64::from(rows);
    let words = u32::try_from(elements.div_ceil(2))
        .map_err(|_| refuse(op, format!("a {rows} x {width} plane will not launch")))?;
    Ok(Grid::of([words, 1, 1], ROUTE_GROUP))
}
...
pub fn group_routes(ctx: &Ctx<'_>, groups: u32, routes: Tensor) -> Result<(), Error> {
    ...
    ctx.fire(
        Fire::at(ROUTE_FILE, "group_routes").apply(Grid::of([groups, routes.rows, 1], ROUTE_GROUP)),
...
fn routed_point(op: &'static str, bank: Bank, biased: bool) -> Result<&'static str, Error> {
    match (bank.affine(), bank.group, bank.bits) {
        ...
        (affine, group, bits) => Err(refuse(
            op,
            format!(
                "the bank is {} at {bits} bits in groups of {group}, and `moe/qmv_routed.wgsl` \
...
pub fn matmul_select_batched(
    ...
    let pairs = y.rows;
    let tile = tile_rows(pairs, experts, tuning);
    if tile <= 1 || !x.width.is_multiple_of(32) || experts > ROUTER_MAX_EXPERTS {
        return Ok(false);
    }

    if !x.width.is_multiple_of(2) || !y.width.is_multiple_of(2) {
        return Ok(false);
    }
    let family = match (bank.affine(), bank.group, bank.bits) {
        (false, 32, 4) => "mxfp4",
        (true, 64, 4) | (true, 64, 2) => "affine",
        _ => return Ok(false),
    };
    let Some(entry) = ROUTED_TILE_POINTS
        .iter()
        .copied()
        .find(|(_, f, g, b, biased, bm)| {
            *f == family
                && *g == bank.group
                && *b == bank.bits
                && *biased == bias.is_some()
    ...
    ctx.fire(
        Fire::at(SORT_FILE, "route_sort").apply(Grid::of(SORT_GROUP, SORT_GROUP)),
        &[
            routes.arg(),
    ...
    ctx.fire(
        Fire::at(ROUTED_TILE_FILE, entry).apply(Grid::of(
            [
                y.width.div_ceil(ROUTED_TILE_COLS) * 32,
                (padded / tile) * routed_tile_rows(tile),
                1,
            ],
            [32, routed_tile_rows(tile), 1],
        )),
        &[
            bank.codes.arg(),
            bank.scales.arg(),
...
const SORT_FILE: &str = "moe/route_sort.wgsl";

fn routed_tile_rows(tile: u32) -> u32 {
    tile / 16
}

const ROUTED_TILE_FILE: &str = "moe/qmm_t_routed.wgsl";

const SORT_GROUP: [u32; 3] = [256, 1, 1];

const ROUTED_TILE_COLS: u32 = 32;

const ROUTED_TILE_POINTS: &[(&str, &str, u32, u32, bool, u32)] = &[
    (
