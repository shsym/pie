...
const REGISTER_STATE_MIN_ROWS: u32 = 64;
...
pub fn causal_conv1d(
    ...
    ctx.fire(
        Fire::at("ssm/causal_conv1d.wgsl", entry).apply(conv_grid(channels, rows)),
        &[
            x.arg(),
            weight.arg(),
            bank.arg_mut(),
...
pub fn causal_conv1d_chunked(
    ...
    ctx.fire(
        Fire::at("ssm/causal_conv1d.wgsl", entry).apply(conv_grid(channels, lanes)),
        &[
            x.data.arg(),
            x.indptr.arg(),
            weight.arg(),
...
pub fn gdn_prep(
    ...
    ctx.fire(
        Fire::at("ssm/gdn_prep.wgsl", entry)
...
pub fn gated_delta(
    ...
    ctx.fire(
        Fire::at("ssm/gated_delta.wgsl", entry).apply(recurrence_grid(shape.v_heads, rows)),
        &[
            qkv.arg(),
            gates.arg(),
            state.state.arg_mut(),
...
pub fn gated_delta_chunked(
    ...
    let entry = if k_dim == 128 && v_dim <= 128 && qkv.data.rows >= REGISTER_STATE_MIN_ROWS {
    ...
    ctx.fire(
        Fire::at("ssm/gated_delta.wgsl", entry).apply(recurrence_grid(shape.v_heads, lanes)),
        &[
            qkv.data.arg(),
            qkv.indptr.arg(),
            gates.arg(),
...
pub fn gated_delta_committed(
    ...
    let entry = if k_dim == 128 && v_dim <= 128 && qkv.rows >= REGISTER_STATE_MIN_ROWS {
