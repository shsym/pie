...
use eta_compiler::codegen::launch::{LaunchChannel, LaunchPackage};
use eta_ir::container::{ChanDType, ExternDir};
...
impl Registry {
    ...
    pub fn fire(&mut self, id: u64, inputs: &crate::PassInputs) -> Result<crate::StepOutcome> {
        let instance = self.instances.get_mut(&id).ok_or_else(|| Error {
    ...
    pub fn close_instance(&mut self, id: u64) -> Result<()> {
        let instance = self.instances.remove(&id).ok_or_else(|| Error {
            message: format!("no instance {id}"),
        })?;
...
fn check_slot(slot: usize, endpoint: &Channel, declared: &LaunchChannel) -> Result<()> {
    let refuse = |what: &str, found: String, want: String| Error {
        message: format!(
