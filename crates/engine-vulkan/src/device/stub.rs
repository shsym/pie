...
impl Frame {
    ...
    pub fn into_kept(self) -> Result<Kept> {
        match self.never {}
    }
}
...
impl Buffer {
    ...
    pub(crate) fn write_shared(&self, _offset: u64, _bytes: &[u8]) -> Result<()> {
        match self.never {}
    }
...
impl Pipelines {
    ...
    pub fn warm(&self, _device: &Context, _fire: kernels_vulkan::Fire) -> Result<()> {
        Err(Fault::Deviceless)
    }
