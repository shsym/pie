...
use kernels_vulkan::attn::ple;
use kernels_vulkan::linear::moe::{self, RoutedScratch};
...
const SPLIT_PLANE_ROWS: u64 = 128;
...
impl Scratch {
    pub fn reserve(
        ...
        let tuning = kernels_vulkan::tuning::current();
        ...
        let split = trace
            ...
            .filter_map(|node| match &node.op {
                ...
                ) => of(*o).map(|rect| (rect.width, *head_dim)),
                _ => None,
            })
            .filter(|(width, head_dim)| *width > 0 && *head_dim > 0 && width % head_dim == 0)
            .map(|(width, head_dim)| (u64::from(width), u64::from(width / head_dim)))
            .reduce(|a, b| (a.0.max(b.0), a.1.max(b.1)))
            .map(|(width, heads)| {
                let mut at = 0u64;
                SplitRooms {
                    part_out: Room::lay(&mut at, SPLIT_PLANE_ROWS, width, Dtype::F32),
                    part_lse: Room::lay(&mut at, SPLIT_PLANE_ROWS, heads, Dtype::F32),
                }
            });
        ...
        let union = precast
            ...
            .max(split.map_or(0, |r| r.part_lse.at + r.part_lse.bytes()));
    ...
    pub fn split(&self, handles: &Handles) -> Option<Result<kernels_vulkan::Split>> {
        let rooms = self.split?;
        Some((|| {
            Ok(kernels_vulkan::Split {
                part_out: rooms.part_out.bind(handles, &self.plane)?,
                part_lse: rooms.part_lse.bind(handles, &self.plane)?,
            })
        })())
    }
