...
struct Free {
    ...
    /// rounding"*.
    by_shape: HashMap<(&'static str, u64), Vec<Handle>>,
...
impl Scratch {
    ...
    pub fn take(&self, context: &Context, len: u64, what: &'static str) -> Result<Lease> {
        ...
        let handle = match existing {
            Some(handle) => handle,
            None => {
                let handle = allocate(context, len, what)?;
...
mod tests {
    use super::Scratch;
    use crate::metal::{Context, allocate};
    use objc2_metal::MTLResidencySet;

    /// The two things a pool buys, measured against the allocation it
    /// replaces.
    ///
    /// **The leak.** `ring::allocate` adds to the residency set and nothing
    /// removes: fifty allocate-and-drop cycles leave fifty allocations and
    /// 52 MB resident, and a serving driver does three per fire.
    ///
    /// **The address.** Fifty pooled takes of one shape produce ONE device
    /// allocation and ONE address, which is what an indirect command buffer
    /// recorded for that shape needs in order to be replayable.
    #[test]
    fn a_pool_allocates_once_and_keeps_the_address() {
        ...
        for _ in 0..8 {
            drop(allocate(&context, 1 << 16, "unpooled").expect("a region"));
        }
        assert_eq!(
            context.residency().allocationCount() - before,
            8,
            "eight raw allocations should leave eight in the residency set -- \
             if this ever fails, `allocate` learned to remove and this pool's \
             leak argument needs restating"
        );
