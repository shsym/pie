...
//!
//! [`Heap`](super::heap::Heap) is a bump allocator: it hands out placements
...
impl Transient {
    ...
    /// [`Slot::contents`](super::heap::Slot::contents): the GPU may be reading
    /// them.
    #[must_use]
...
impl Drop for Transient {
    fn drop(&mut self) {
        ...
        let Some(owner) = self.owner.upgrade() else {
            // The pool is gone; the buffer goes with it.
            return;
        };
...
impl State {
    ...
    fn make_room(&mut self, wanted: u64) {
        while self.stats.resident_bytes + wanted > self.stats.capacity_bytes {
            let Some(class) = self
                .free
                .iter()
                .rev()
...
impl Pool {
    ...
    pub fn drain(&self) {
        let mut state = self.locked();
        let classes: Vec<u64> = state.free.keys().copied().collect();
        for class in classes {
            while let Some(cached) = state.free.get_mut(&class).and_then(Vec::pop) {
                state.stats.cached_buffers -= 1;
                state.stats.cached_bytes -= class;
                state.release(cached);
            }
        }
    }
