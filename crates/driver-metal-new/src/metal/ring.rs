...
//! words.
//!
//! The interpreter's [`ChannelState`](crate::pipeline::ChannelState) keeps a
...
//! is no release call to forget because there is nothing to call it on.
//! `PARITY.md`'s four `missing` standalone-buffer entries are this hole; the
...
//! [`Words`](crate::pipeline::Words), which is a snapshot either can
...
use super::external::{add, remove};
use super::handle::Handle;
use crate::pipeline::{Words, wire_cell_bytes};
...
impl Ring {
    ...
    /// lanes.
    ///
    /// Mirrors [`ChannelState::host`](crate::pipeline::ChannelState::host)
    ...
    /// # Errors
    ///
    /// [`Error::Create`] when the device declines an allocation.
    pub fn new(context: &Context, dtype: DType, numel: usize, capacity: usize) -> Result<Self> {
        ...
        let cells = allocate(context, (cell_bytes * cap1) as u64, "channel ring cells")?;
        let words = allocate(context, WORDS_BYTES, "channel ring words")?;
    ...
    /// The four words at once, for [`check_words`](crate::pipeline::check_words).
    #[must_use]
...
impl Drop for Ring {
    /// Out of the residency set; the buffers release with their handles.
    fn drop(&mut self) {
        remove(&self.residency, self.cells.buffer());
        remove(&self.residency, self.words.buffer());
    }
}
...
/// One shared-storage buffer of `len` bytes, resident until removed.
///
