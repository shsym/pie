...
fn folded_norm(dir: &std::path::Path, dtype: DType) -> CheckpointMetadata {
...
fn run(dir: &std::path::Path, metadata: &CheckpointMetadata, out: TensorContract) -> Vec<u8> {
    ...
    let plan = model_loader::plan::compile(
        metadata,
        &contract,
        StorageTarget {
    ...
    .unwrap();
    model_loader::executor::Execution::new(&plan, dir)
        .run()
        .unwrap()
        .tensors
        .remove(&name)
...
fn an_unset_constant_is_refused_rather_than_run_as_the_identity() {
    ...
    let why = model_loader::plan::compile(
        &metadata,
        &contract,
        StorageTarget {
            tile_map_mask: CONVERT_TILE_MAP_MASK,
