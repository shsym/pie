...
const GOLDEN_BEAM: u64 = 6874506407235578272;
const GOLDEN_MTP_GRAMMAR: u64 = 17296851854215035127;
...
fn s3_traces_and_validates() {
    ...
    assert_eq!(
        ...
        eta_dsl::eta::container::HostRole::Writer,
...
fn s6_2_beam_epilogue_binds() {
    ...
    b.stage(Stage::Epilogue, move || {
        ...
        w_off.put(&off);
        let tok_u = rem(&i, V);
        let tok_i = cast(&tok_u, eta_dsl::Dtype::I32);
        toks.put(&tok_i);
        scores.put(&s);
        out.put(&tok_i);
        out_par.put(&parent);
    ...
    let decoded =
        eta_dsl::eta::container::decode(&traced.encode()).expect("decode beam container");
    assert_eq!(
    ...
    use eta_dsl::eta::container::HostRole;
    assert_eq!(
...
fn s6_1_mtp_grammar_binds() {
    ...
    b.stage(Stage::Epilogue, move || {
        ...
        let nacc = cast(reduce_sum(&run), eta_dsl::Dtype::U32); // accepted-prefix length
