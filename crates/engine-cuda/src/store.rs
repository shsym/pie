...
impl Pools {
    ...
    fn zero_slot(&mut self, stream: Option<*mut core::ffi::c_void>, slot: u32) -> Result<()> {
        ...
        for (slab, shape) in self.slabs.iter_mut().zip(&self.shapes) {
            ...
            let at = u64::from(slot) * bytes;
            let len = usize::try_from(bytes).unwrap_or(0);
            match stream {
                Some(stream) => slab.zero_span_on(stream, at, len)?,
                None => slab.zero_span(at, len)?,
            }
        }
        Ok(())
    }
...
/// **The engine's half of memory, at F1's reservation model** (alto design §8;
/// article 8: the runtime owns policy, the engine owns supply).
///
/// [`Pools`] carves fixed blocks at load — one block per slot, one page size,
/// `resize_pool` deliberately unimplemented, elastic caps published as 0
/// (survey §2, debt 2) — so the honest `commit` is the CEILING this shell
/// already had, and `trim` has nothing to give back.
///
/// **THE REFUSAL IS THE ONE `kv::geometry_with` ALREADY WROTE**, down to the
/// variant and the `what` string, because a frame that fails admission here
/// must fail it identically to the fire that would have reached the page
/// arithmetic a dozen lines later. That is what makes this a seam rather than
/// a new gate: F1 moves the question earlier without changing its answer.
///
/// Wave C is what makes this method mean something new — dev's budgeted
/// `PhysicalPool` under VMM arenas, with an atomic multi-arena commit as the
/// frame admission gate — and when it lands, the 10-second `resize_pool` poll
/// dies with it, because elasticity becomes a side effect of admission.
impl engine::frame::Supply for Pools {
    ...
    /// Nothing to trim: the pools are one reservation and they live as long as
    /// the load does.
