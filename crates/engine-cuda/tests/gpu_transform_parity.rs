...
//! half: that a real [`CudaArena`] on a real device, reaching `kernels-cuda`
//! through NVRTC rather than an ahead-of-time archive, leaves behind the
//! bytes the host executor leaves behind.
//!
//! **IT MOVED HERE WITH THE ARENA IT MEASURES**, and it is the whole reason
//! that arena was worth keeping. It stood in `checkpoint/tests/` — the
//! crate was called model-loader then — under
//! that crate's optional `cuda` feature — the only target in the tree that
//! ever turned the feature on, and therefore the only thing keeping a
//! device edge in a crate whose value is that it compiles a CUDA plan with
//! no CUDA present. The comparison is unchanged: `checkpoint` is a
//! dependency of this crate, so the host executor is still reachable as the
//! reference, and every assertion below is the one it always made.
//!
...
//! `kernels_cuda::Tensor` — and the arena did not follow, so the loader's
...
//! the implementation the loader builds without a toolkit, it is what
//! [`CudaArena::host_transforms_only`] selects, and a device answer nobody
...
use checkpoint::executor::arena::{ArenaBacking, ArenaSpan, TileMapOp};
use checkpoint::executor::sink::MemorySink;
use checkpoint::plan::passes::tile::CUDA_SCALE_ROWS_BF16;
use cudarc::runtime::sys as rt;
use engine_cuda::weights::arena::CudaArena;
use checkpoint::plan::{
...
fn checkpoint() -> &'static Path {
    ...
    DIR.get_or_init(|| {
        ...
        // `cargo test -p engine-cuda --features cuda-13` runs — two agents,
...
///
/// `#![cfg(feature = "_cuda")]` gates this file on a BUILD, and a build
...
/// typed entry points return a `Result` and the arena propagates it, so the
