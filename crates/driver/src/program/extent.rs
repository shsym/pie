use driver_api::program::{Axis, LaunchPlanValue};
use tensor_ir::DType;
use tensor_ir::types::MAX_RANK;

/// Which runtime quantity a symbolic axis resolves against.
///
/// The contract's [`ExtentRole`](driver_api::program::ExtentRole), re-exported
...
/// declares (`SymbolicExtent`) and `driver-api` also carried as a `u8` tag
/// space; the wire tags were kept in step by nothing.
pub use driver_api::program::ExtentRole as Role;
