...
//! — the copy covers every bucket a decode fire lands in. `driver::fire::walk`
...
//! computes none. That is `driver-cuda`'s
//! `a_copied_window_and_a_split_one_are_the_same_bytes.rs`, which diffs a
//! copy against a split on real weights.

use std::collections::HashMap;

use driver::fire::{EventId, FireDescriptor, Lane, Serve, Sink, compose, fallback, walk};
...
fn a_copied_window_costs_one_launch_where_a_split_one_costs_its_runs() {
    ...
    // everything. (`driver` denies both print macros in its tests.)
