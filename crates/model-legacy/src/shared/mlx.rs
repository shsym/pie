...
pub fn int4_requested(b: &Builder<'_>, schema: &str) -> Result<bool, Error> {
    match b.runtime_quant() {
        ...
        other => fail(format!(
            "the MLX {schema} contract: runtime_quant={other:?} has no encoder \
             here; these kernels read MLX affine, so `int4` is the only \
             request they can serve"
        )),
    }
}
...
pub fn push_encoded_affine(
    ...
    if cols % AFFINE_GROUP != 0 {
        return fail(format!(
            "MLX affine: '{output}' has {cols} columns, which these group-64 \
             kernels cannot quantize"
        ));
    }
...
pub fn routed_expert_member(
    ...
    for shared in [
        ...
        if member.starts_with(shared) {
            return fail(format!(
                "the MLX {schema} schema has no shared expert, but '{raw_name}' is \
                 one: \
                 a shader driver would load it and never read it, running \
                 the routed mixture alone"
            ));
        }
    }
    ...
    {
        return fail(format!(
            "the MLX {schema} schema needs the routed experts stacked on axis 0 \
...
pub fn layer_member<'n>(
    ...
    let Some(tail) = rest.strip_prefix(LAYERS) else {
        return fail(format!(
            "the MLX {schema} schema has no declared mapping or skip for '{raw_name}'"
        ));
    };
    let Some(dot) = tail.find('.') else {
        return fail(format!(
            "MLX {schema} layer tensor '{raw_name}' is malformed"
        ));
    };
    let layer = &tail[..dot];
    if layer.is_empty() || !layer.chars().all(|c| c.is_ascii_digit()) {
        return fail(format!(
            "MLX {schema} layer tensor '{raw_name}' has an invalid layer index"
        ));
    }
    Ok((layer, &tail[dot + 1..]))
}
...
pub fn author_mlx_file(
    ...
    if !encode_floats && !b.tensors().iter().any(|t| t.name.ends_with(".scales")) {
        return fail(format!(
            "the MLX {schema} contract needs quantized weights: this \
             checkpoint carries no `.scales` tensors, so it is unquantized \
             (bf16/fp16), and every shader driver -- Metal, Vulkan and wgpu \
             alike -- binds each projection through this contract's affine-U4 \
             path. Either build it with `--quant int4` to encode the weights \
             now, or use a pre-quantized repo -- the `mlx-community/*-4bit` \
             conversions are the ones this path is built for."
        ));
    }
    ...
    for raw in b.tensors().to_vec() {
        ...
        if raw.name.ends_with(".weight") && is_raw(&raw.encoding, DType::U32) {
            ...
            let (Some(scales), Some(biases)) = (
                ...
                return fail(format!(
                    "MLX affine-U4 weight '{}' is missing scales or biases",
                    raw.name
                ));
            };
    ...
    if declared == 0 {
        return fail(format!("the MLX {schema} schema found no decoder tensors"));
    }
    Ok(())
}
