...
/// # Every row here says `NormVariant::Gemma`, and that is not a copy-paste
///
/// It reads like one, because "Gemma" names a norm and none of these models
/// is a Gemma. What the variant actually decides is whether the checkpoint's
/// norm parameter is the gain or a DELTA around the gain: `Plain` folds `w`,
/// `Gemma` folds `1 + w`. Qwen3.5 declares its norm parameter as
/// `nn.Parameter(torch.zeros(dim))` and folds `1 + w` — in the dense text
/// model and, separately spelled but identical, in the MoE one. So every
/// member of this family is `Gemma` whatever the name suggests, and a row
/// that says `Plain` is not a stylistic difference: it multiplies the first
/// activation of every block by roughly the norm weight instead of by one
/// plus it, and the model never recovers.
///
/// The 0.8B row's weights say so directly — its `input_layernorm.weight`
/// averages +0.24, centred on zero and not on one — and so do the 27B's, at
/// -0.02. The other three ship config-only in this cache, but they declare
/// the same `architectures` entry, which is the thing that picks the norm.
///
/// The one `Plain` left in this module is `gate_norm` in `forward/mod.rs`,
/// and that one is right: RMSNormGated is a different class and initialises
/// its weight to ONES.
pub const VARIANTS: &[Qwen35] = &[
    ...
    Qwen35 {
        ...
        shape: Qwen35HybridFacts {
            ...
            tied_embeddings: true,
            norm_variant: NormVariant::Gemma,
            attn: attn(2560, 16, 4, NormVariant::Gemma),
            gdn: gdn(2560, 16, 32, NormVariant::Gemma),
    ...
    Qwen35 {
        ...
        shape: Qwen35HybridFacts {
            ...
            norm_variant: NormVariant::Gemma,
            attn: attn(4096, 16, 4, NormVariant::Gemma),
            gdn: gdn(4096, 16, 32, NormVariant::Gemma),
    ...
    Qwen35 {
        ...
        shape: Qwen35HybridFacts {
            ...
            tied_embeddings: false,
            norm_variant: NormVariant::Gemma,
            attn: attn(2048, 16, 2, NormVariant::Gemma),
            gdn: gdn(2048, 16, 32, NormVariant::Gemma),
    ...
    Qwen35 {
        ...
        shape: Qwen35HybridFacts {
            ...
            tied_embeddings: false,
            norm_variant: NormVariant::Gemma,
            attn: attn(5120, 24, 4, NormVariant::Gemma),
            gdn: gdn(5120, 16, 48, NormVariant::Gemma),
            mlp: Qwen35MlpKind::Dense {
                intermediate: 17_408,
            },
        },
