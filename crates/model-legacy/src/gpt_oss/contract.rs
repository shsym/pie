...
pub fn author_gpt_oss_mlx(b: &mut Builder<'_>) -> Result<(), Error> {
    ...
    for raw in b.tensors().to_vec() {
        ...
        if raw.name.ends_with(".weight") && is_raw(&raw.encoding, DType::U32) {
            ...
            let Some(scales) = scales else {
                return fail(format!(
                    "MLX GptOss: '{}' is a packed weight with no scales, which no \
            ...
            if groups <= 0 || packed_cols % (2 * groups) != 0 {
                return fail(format!(
                    "MLX GptOss: '{}' is not quantized in groups of 64, which is \
            ...
            if bits != 4 && bits != 8 {
                return fail(format!(
                    "MLX GptOss: '{}' is {bits}-bit, and only 4 and 8 are described here",
                    raw.name
                ));
            }
    ...
    if declared == 0 {
        return fail("MLX GptOss schema found no decoder tensors");
    }
    Ok(())
}
...
fn gptoss_mlx_name(raw_name: &str) -> Result<String, Error> {
    ...
    let Some(rest) = raw_name.strip_prefix("model.") else {
        return fail(format!(
            "MLX GptOss schema has no declared mapping or skip for '{raw_name}'"
        ));
    };
...
fn declare_mxfp4_experts_mlx(b: &mut Builder<'_>, declared: &mut usize) -> Result<(), Error> {
    ...
    for blocks in b.tensors().to_vec() {
        ...
        let Some(scales) = b.find(&format!("{base}_scales")) else {
            return fail(format!(
                "MLX GptOss: '{}' is an MXFP4 block tensor with no '_scales' \
                 beside it",
                blocks.name
            ));
        };
        ...
        {
            return fail(format!(
                "MLX GptOss: MXFP4 tensor '{}' is not shaped \
        ...
        if !fused && !base.ends_with("down_proj") {
            return fail(format!(
                "MLX GptOss: MXFP4 tensor '{}' is neither the fused gate/up \
        ...
        for (half, first_row) in halves {
            ...
            if let Some(bias) = &bias {
                if bias.shape.len() != 2 || bias.shape != [experts, stored_rows] {
                    return fail(format!(
                        "MLX GptOss: '{}' does not match the projection it biases",
                        bias.name
                    ));
                }
                let expr = if fused {
