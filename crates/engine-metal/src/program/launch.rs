...
//! `struct M1OpParams` is exactly `engine::OpParams`, 64 bytes, so the
...
//! `engine::Status` — a state, a fault word, two reserved words the fault
//! sites use — so a refusal can be named with `engine::describe_fault`
//! instead of merely reported.

use std::mem::size_of;

use engine::engine_api::program::{LaunchChannel, LaunchStagePlan};
use engine::{Extents, OpParams, OpRuntime, SCRATCH_ALIGN, Status, ValueDesc, describe, layout};
...
impl ChannelShape {
    ...
    pub fn cell_bytes(&self) -> usize {
        engine::wire_cell_bytes(self.dtype, self.numel)
    }
...
impl Rings {
    ...
    /// (`engine::program::channel`) and not this file's.
    ///
    /// # Errors
    ///
...
impl Prepared {
    ...
    /// `extents`, when the scratch exceeds what `engine::layout` permits, or
    ...
    pub fn build(
        ...
        //    emitted `struct M1OpParams` is `engine::OpParams`, 64 bytes,
        ...
        let status = Buffer::zeroed(device, engine::STATUS_BYTES as u64)?;
    ...
    /// side arrays of eight words each — and the engine stages a base, a
    ...
    ///
    /// Where the CUDA twin answers a boolean, this answers `engine::Status`
    /// — the state, the fault word and the two site words — so a refusal can
    /// be NAMED (`engine::describe_fault`) instead of merely counted.
    ///
    /// # Errors
    ///
    /// Whatever the readback said.
    pub fn status(&self) -> Result<Status> {
        let mut bytes = [0u8; engine::STATUS_BYTES];
