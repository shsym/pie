...
fn trace_of(sku: &str, platform: Platform) -> Trace {
    let trace = models::trace_of(sku).unwrap_or_else(|| panic!("this build ships `{sku}`"));
    trace(platform)
}
...
fn a_drafting_lane_lands_in_a_class_that_runs_the_draft_head() {
    for sku in DRAFTING {
        let classify = models::classify_of(sku).expect("and its classifier");
        for platform in PLATFORMS {
            ...
            let head_cache = trace
                ...
                .iter()
                .position(
                    |row| matches!(row, model_dsl::CacheRow::Kv { name, .. } if name == "kv.mtp"),
                )
                .unwrap_or_else(|| {
                    panic!("`{sku}` as {platform:?}: the draft head declares no kv row")
                }) as u32;
            let head_pages = trace
                ...
                .unwrap_or_else(|| {
                    panic!("`{sku}` as {platform:?}: nothing reads the draft head's kv row")
                }) as u32;
            ...
            let drafting = classify(&models::Request::new(1, false).drafting(true)) & classes.mask;
            let plain = classify(&models::Request::new(1, false)) & classes.mask;
            assert_ne!(
                drafting, plain,
...
fn a_capturing_lane_lands_in_a_class_that_runs_the_lse_attention() {
    let classify = models::classify_of(CAPTURING).expect("this build ships the qwen row");
    ...
    for platform in PLATFORMS {
        ...
        for rows in [1, 8] {
            let word =
                classify(&models::Request::new(rows, false).capturing_scores(true)) & classes.mask;
            let class = classes
                .class_of(word)
        ...
        for (rows, want) in [(1, (false, true, false)), (8, (false, false, true))] {
            let word = classify(&models::Request::new(rows, false)) & classes.mask;
            let class = classes
                .class_of(word)
                .unwrap_or_else(|| panic!("{platform:?}: the plain word names no class"));
            assert_eq!(
