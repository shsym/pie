...
fn a_one_token_lane_lands_in_the_decode_class() {
    let trace = models::trace_of(QWEN).expect("this build ships the qwen row");
    let classify = models::classify_of(QWEN).expect("and its classifier");
    ...
    for platform in PLATFORMS {
        ...
        let decode_word = classify(&models::Request::new(1, false)) & classes.mask;
        let prefill_word = classify(&models::Request::new(8, false)) & classes.mask;
        assert_ne!(
...
fn every_sku_classifies_into_its_own_plan() {
    ...
    for (sku, _, trace, classify) in models::catalog() {
        for platform in PLATFORMS {
            let trace = trace(platform);
                    ...
                    for adapted in [false, true] {
                        for drafts in [false, true] {
                            for scores in [false, true] {
                                let r = models::Request::new(rows, masked)
                                    .adapted(adapted)
                                    .drafting(drafts)
