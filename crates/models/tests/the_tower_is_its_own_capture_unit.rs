...
/// and, as `qwen38-27b-vision` proved by being registered without landing
/// here, so that a row that STARTS declaring one fails until it is named.
///
/// **THE `-mlxu4` ROWS BAKE THE SAME PLAN AS THEIR `-bf16` TWINS**, which is
/// the point of sweeping both: a tower's element is the artifact's fact and
/// the PARTITION is the text's, so a 4-bit trunk under a bf16 tower must
/// still carve two units in the same order. If a dtype ever reached the
/// partition, exactly these four extra rows would say so.
const TOWERED: [&str; 10] = [
