...
impl<'ctx> Stepper<'ctx> {
    ...
    /// Block until `value` retires, and report what the step cost.
    ///
    /// The completion half of [`submit`](Self::submit), so a caller that
    /// committed a fire asynchronously can still end with the number
    /// [`run`](Self::run) returns. `encode` is the caller's own measurement
    /// of the work in front of the commit — this type cannot take it,
    /// because between `submit` and here the caller may have queued more.
    ///
    /// `gpu_exec` is measured from entry, which is the honest reading for an
    /// async submission: it is how long the caller waited, and for a fire
    /// that had already finished it is near zero, correctly. The GPU's own
    /// report still arrives in [`Timing::gpu`] when the feedback for this
    /// step has landed.
    ///
    /// # Errors
    ///
    /// A wedged context or a fault reported while waiting.
    pub fn wait_for_timing(&mut self, value: u64, encode: Duration) -> Result<Timing> {
        let waited = Instant::now();
        self.await_value(value)?;
        Ok(self.timing(value, encode, waited.elapsed()))
    }
