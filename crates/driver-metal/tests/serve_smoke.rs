...
//!
//! ```text
//! cargo test -p driver-metal --release --test serve_smoke -- --nocapture
//! ```
//!
...
/// The lane word the model's own `Classify` computes — engine-side work,
/// done here because this test IS the engine for the length of one fire.
...
fn a_mask_this_plane_does_not_stage_is_refused_by_name() {
    ...
    let prompt = tokenizer.encode(PROMPT);
    shell.open(0).expect("slot 0 opens");
    let mask = driver::driver_api::fire::Mask {
