...
//! at — which is also the only exercise the engine's attachment seam gets
...
//! word anywhere on this plane to say otherwise; the CUDA driver's
...
//!   cargo test -p driver-metal --test program_parity -- --nocapture
//! ```
...
use driver::driver_api::program::{LaunchChannel, LaunchPackage, ProgramRegistration, ValueSource};
use driver::tensor_ir::DType;
use driver::tensor_ir::container::HostRole;
use driver::driver_api::program::KernelKind;
...
use driver_metal::device::{Buffer, Context, present};
use driver_metal::program::{Fired, Plane};
...
fn parity(context: &Context, plane: &mut Plane, subject: &Subject) {
    ...
    // in the plan instead, because the engine wants the sentence rather than a
...
fn a_source_metal_refuses_is_a_named_deterministic_refusal_and_nothing_else() {
    ...
    // `driver_metal::Fault::Compile`'s own doc states that the split comes off
    ...
    assert!(
        matches!(
            refusal,
            driver_metal::Fault::Compile(driver::Failure::Deterministic { .. })
        ),
    ...
    // rather than in the two claims. `driver_cuda::Fault::Compile` prints
