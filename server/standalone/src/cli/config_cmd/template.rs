...
//! active build feature (`driver-portable` / `driver-cuda` /
//! `driver-dummy`) picks which `[model.driver]` block to splice in,
//! so the generated config "just works" without a follow-up edit.

use crate::driver_ffi;

/// Render the default `config.toml` body matching the active driver
/// flavor.
pub fn default_config_content() -> String {
    let driver_block = match driver_ffi::FLAVOR {
        "portable" => PORTABLE_DRIVER_BLOCK,
        "cuda" => CUDA_DRIVER_BLOCK,
        "dummy" => DUMMY_DRIVER_BLOCK,
        _ => CUDA_DRIVER_BLOCK,
    };
