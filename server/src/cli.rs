...
pub enum Command {
    ...
    /// (`pie driver <type> install|doctor|set|unset|show|exec`).
    Driver {
        #[command(subcommand)]
        cmd: driver_cmd::DriverCmd,
    },
    ...
    /// Environment readiness check (platform, GPUs, compiled-in
    /// drivers, per-driver venv resolution).
    Doctor,
    ...
    Smoke {
        ...
        /// (`portable` / `cuda` / `dummy`). Defaults to the
        /// highest-priority flavor compiled into this binary.
        #[arg(long)]
