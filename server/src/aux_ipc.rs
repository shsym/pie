...
impl AuxIpcClient {
    ...
    pub fn connect(socket_path: PathBuf) -> Result<Self> {
        let sock = UnixStream::connect(&socket_path).map_err(|e| {
            anyhow!("connect aux-ipc socket {socket_path:?}: {e}")
        })?;
        Ok(Self {
            sock: Mutex::new(sock),
            socket_path,
    ...
    pub fn load_adapter(&self, adapter_id: u64, path: &Path) -> Result<()> {
        ...
        let path_len = u32::try_from(path_bytes.len())
            .map_err(|_| anyhow!("aux-ipc: adapter path too long ({} bytes)", path_bytes.len()))?;

        let req_id = self.next_req_id.fetch_add(1, Ordering::Relaxed);

        let mut buf = Vec::with_capacity(
            HEADER_SIZE + LOAD_ADAPTER_PAYLOAD_SIZE + path_bytes.len(),
        );
...
mod tests {
    ...
    fn fake_server(
        ...
        thread::spawn(move || {
            ...
            let body_len =
                (n_pages as usize) * PAIR_SIZE + body_extra_bytes;
    ...
    fn copy_returns_error_on_nonzero_status() {
        ...
        let client = AuxIpcClient::connect(path).unwrap();
        let err = client.copy(Method::CopyD2H, &[(1, 2)]).unwrap_err().to_string();
    ...
    fn ack_bad_magic_surfaces_clear_error() {
        ...
        let client = AuxIpcClient::connect(path).unwrap();
        let err = client.copy(Method::CopyD2D, &[(0, 1)]).unwrap_err().to_string();
