...
An inferlet exposes three things to your code: the forward pass at token granularity, the KV cache as named pages you can fork and share, and an I/O surface that reaches HTTP, MCP, the filesystem, and embedded language runtimes. Most of what looks like a separate "feature" of Pie is one of these three primitives applied to a problem. This page lists those primitives and points to the guide pages that go deep.

This page is for engineers deciding what they could build on Pie. If you want to write code, start with [Your first inferlet](../guide/first-inferlet).
...
The page-based KV cache is a Pie-native concept, not a vLLM PagedAttention port. The `kv-cache` guide describes the page model. See [Branch and share state](../guide/context/sharing).
...
These three primitives are not independent features that happen to share a binary. Each enables the others:

- Forward pass control lets you stop generation on a tool call boundary; integrated I/O lets you serve the call inline; KV cache control lets the next forward pass reuse the prefix.
- KV cache control lets best-of-N share a prefix; forward pass control lets each branch use a different sampler; integrated I/O lets each branch validate its output against an external checker.

What you write looks like a regular program. What runs is a program that has the GPU, the KV cache, and a network all on the same call stack.

## Next

- [Comparison with other systems](./comparison). What you give up and what you gain over a black-box endpoint.
- [Your first inferlet](../guide/first-inferlet). A minimal Rust inferlet with build and run.
- [Examples](../guide/examples/overview). The catalog of patterns implemented as inferlets.
