...
- **Speculative decoding.** Run a drafter, verify with the target model, accept or reject. Pie supports prompt-lookup and **native MTP drafting** for models with an MTP head. See [Speculative decoding](../guide/forward/speculation), [`cacheback-speculative-decoding`](https://github.com/pie-project/pie/tree/main/inferlets/cacheback-speculative-decoding), and [`mtp-speculative-decoding`](https://github.com/pie-project/pie/tree/main/inferlets/mtp-speculative-decoding).
- **Custom samplers and raw logits.** Score candidate strings, compute entropy, or pull the full distribution back into your inferlet. See [`sampling-primitives`](https://github.com/pie-project/pie/tree/main/inferlets/sampling-primitives).
...
- **Parallel candidates and tree search.** Branch from a shared prefix N times. Each branch's compute and memory scales with its divergent tokens, not with the prompt length. See [`consensus-decoding`](https://github.com/pie-project/pie/tree/main/inferlets/consensus-decoding) and [`prefix-tree-kv-cache`](https://github.com/pie-project/pie/tree/main/inferlets/prefix-tree-kv-cache).
- **Sliding window and attention sink.** Mask positions that fall out of the window while keeping sink positions visible. See [`sliding-window-attention`](https://github.com/pie-project/pie/tree/main/inferlets/sliding-window-attention) and [`attention-sink`](https://github.com/pie-project/pie/tree/main/inferlets/attention-sink).
...
- **WASI HTTP and filesystem.** Inferlets can issue HTTP requests and read files from a sandboxed view. The `wstd` crate exposes `http::Client`, useful for fetching tool outputs, images, or documents inline. See the `image-fetch` inferlet.
