...
async fn main(input: Input) -> Result<String> {
    let model = Model::load(&runtime::models()[0])?;
...
- **JavaScript** — `npm install pie-client`
...
The Pie repo ships ~35 example inferlets under `inferlets/` covering chat, structured generation, speculative decoding, agents, and concurrency benchmarks. The canonical entry point is `text-completion`:

```bash
pie run text-completion --prompt "Once upon a time"
```

Browse the full catalog in [Examples](./examples).
