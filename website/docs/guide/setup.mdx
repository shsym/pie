...
```bash
pie config set model.0.model Qwen/Qwen2.5-7B-Instruct  # update the first [[model]] entry
```
...
```
╭─ Pie Run ───────────────────────────────────────╮
│ Inferlet     chat-completion@0.1.0              │
...
| `chat-completion` | The inferlet to run. Resolved to `chat-completion@<latest>` via the registry. |
...
```bash
pie run chat-completion@0.1.0 -- --prompt "hi"
```
...
pie config set model.0.model Qwen/Qwen2.5-3B-Instruct
```
