...
The default config is generated from the drivers compiled into your `pie` binary. CUDA builds prefer `cuda_native`; portable-only builds use `portable`; dummy is always available for smoke tests. See [Configuration](../reference/configuration) for the full schema.
...
`pie model list` reports compatibility. Architectures supported by the installed drivers get a `✓`. The full list of supported architectures is on the [CUDA](../reference/drivers/cuda#supported-architectures) and [Portable](../reference/drivers/portable#supported-architectures) driver pages.
