...
For the runtime side (chained hash, two-tier GPU/CPU trie, eviction policy), see `runtime/src/context/STORAGE.md` and the SOSP paper.
...
```rust
ctx.system("Long system prompt with lots of context...");
ctx.user("Question");
