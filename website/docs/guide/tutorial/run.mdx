...
The CLI flags after `--manifest` fold into the inferlet's input. `--question "..."` becomes `{"question": "..."}` because that is the parameter declared in `Pie.toml`. Type inference handles `--max-tokens 64` (int) or `--temperature 0.7` (float) when you add such parameters.
...
bakery build . -o research-agent.wasm
pie run --path ./research-agent.wasm --manifest ./Pie.toml --question "..."
```
...
What to do: run `pie model list` to see what is downloaded, and `pie config show` to see what the engine is configured to load. If the configured model is not downloaded, either download it or `pie config set model.default.hf_repo <repo>` to one you do have.
