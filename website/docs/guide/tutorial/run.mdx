...
What to do: run `pie model list` to see what is downloaded, and `pie config show` to see what the engine is configured to load. If the configured model is not downloaded, either download it or `pie config set model.0.model <repo>` to one you do have.
...
- [I/O overview](../io/overview): streaming events, the full session API, and the rest of the inferlet I/O surface.
