...
`bakery create` does not ship a Python template today, so create the project directory manually:
...
Create `Pie.toml` with the manifest shown below and add `python-runtime` to its `[runtime]` block.
...
The Python tab adds `python-runtime = "^0.4.0"` to `[runtime]`. Bakery does not scaffold Python projects yet, so keep that runtime entry in the hand-authored `Pie.toml`.
