...
The Python tab adds `python-runtime = "^0.3.0"` to `[runtime]`. `bakery create --lang python` writes that automatically.
...
- **Constrain the planner with a schema.** Pie supports JSON-schema-constrained decoding through `Schema::Json` (Rust), `Schema.json(...)` (Python), and `jsonSchema(...)` (JavaScript). Attach the schema to the planner's `generate` call and the model cannot emit invalid JSON. See [Structured generation](../forward/constrained).
