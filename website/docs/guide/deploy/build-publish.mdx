...
This installs the standalone `bakery` command from PyPI. Rust inferlets still
need `cargo` and `rustup target add wasm32-wasip2`; TypeScript / JavaScript
inferlets need Node.js.
...
`publish` reads `Pie.toml`, locates an existing `.wasm` artifact, and uploads both the `.wasm` and the manifest to the registry. Run `bakery build . -o <name>.wasm` first. The version in `Pie.toml` becomes the registry version; bumping it is your responsibility.
