...
| Is throughput in the expected range? | `benches/tput.py` and friends. |
...
| How does Pie compare to vLLM / SGLang? | `benches/all_drivers.py`. |
...
The repo's [`benches/`](https://github.com/pie-project/pie/tree/main/benches) directory has scripts that drive the engine through known workloads and report tokens per second, latency, and per-step timing.

```bash
# Pie native driver, throughput sweep
uv run python benches/tput.py

# vLLM driver, same workload shape
uv run python benches/tput_vllm.py

# SGLang driver, same workload shape
uv run python benches/tput_sglang.py

# All three drivers on the same workload
uv run python benches/all_drivers.py

# Phase breakdown (prefill vs decode)
uv run python benches/tput_phase.py

# Per-RPS sweep
uv run python benches/tput_rps.py
```

Each script writes a CSV next to itself. Hardware and model live at the top of the file. Edit them in place to match your environment.
