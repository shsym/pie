...
Replace `dev` with `vllm` or `sglang` for the other two. Each driver gets a Python 3.12 virtual environment with its pinned wheels. See the [driver reference](../reference/drivers/cuda) for what each driver does and when to pick it.
