...
The native driver handles tensor parallelism. From the inferlet's side, the model handle is the same; the engine schedules forward passes across the listed devices. See [Drivers](../../reference/drivers) for the per-driver behavior.
...
- [Supported models](../../reference/models): which architectures the native driver handles.
