...
```typescript
import { Model, runtime } from '@pie-project/inferlet';
...
CUDA-capable drivers handle tensor parallelism. From the inferlet's side, the model handle is the same; the engine schedules forward passes across the listed devices. See the [CUDA driver](../../reference/drivers/cuda) page for the per-driver behavior.
...
- [CUDA driver](../../reference/drivers/cuda#supported-architectures): which architectures the CUDA drivers handle.
