...
Filesystem access is gated by `[server].allow_filesystem` in `~/.pie/config.toml`. It defaults off:
...
When enabled, the engine creates a host-side scratch directory (under the engine's data path) and preopens it at `/scratch` for every inferlet on this server. Restart the engine to apply.

When disabled (the default), the inferlet's filesystem syscalls return errors immediately. There is no way to expose a different host path or split scratch space per inferlet today; every inferlet on the engine shares the same `/scratch`.
...
- [Configuration](../../reference/configuration): the `[server].allow_filesystem` flag and the rest of the schema.
