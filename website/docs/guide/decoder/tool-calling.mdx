...
for await (const step of g) {
    ...
    if (ev.type === 'call') {
        const result = runTool(ev.name, ev.arguments);
...
```typescript
const pending: { name: string; arguments: string }[] = [];

for await (const step of g) {
    const out = await step.execute();
    const ev = dec.feed(out.tokens);
    if (ev.type === 'call') pending.push({ name: ev.name, arguments: ev.arguments });

    if (turnJustEnded(out)) {
        const results = await Promise.all(
            pending.map(p => runToolAsync(p.name, p.arguments)),
        );
        pending.forEach((p, i) => {
...
If you don't need streaming, `tools::parse_call` extracts a single completed call from a finished string.
...
```python
response = await ctx.generate(Sampler.argmax(), max_tokens=512).collect_text()
parsed = tools.parse_call(model, response)
if parsed:
...
```typescript
const response = await ctx.generate(Sampler.argmax(), { maxTokens: 512 }).collectText();
const parsed = tools.parseCall(model, response);
if (parsed) {
    const [name, argsJson] = parsed;
    const result = runTool(name, argsJson);
}
```
...
grammar = tools.native_grammar(model, [json.dumps(calculator_schema)])
if grammar:
    response = await ctx.generate(
        Sampler.argmax(),
        max_tokens=512,
        constrain=GrammarConstraint.from_grammar(grammar, model),
    ).collect_text()
```
...
```typescript
import { Sampler, grammar as gschema, tools } from 'inferlet';

const g = tools.nativeGrammar(model, [JSON.stringify(calculatorSchema)]);
if (g) {
...
`native_grammar` returns `None` (or `null`) for models without a native tool-call template. Fall back to the hand-rolled approach in that case.
