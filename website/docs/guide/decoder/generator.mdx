...
| `.speculator(s)` | `speculator=s` | `speculator: s` | Use a custom speculator for drafting. Rust only. |
...
| `.probe_each_step(idx, probe)` | (Rust only today) | (Rust only today) | Attach a probe at `idx` on every step. Returns a typed handle. |
...
`g.is_done()` (Rust) reports whether the Generator has terminated. Useful when you split the loop across functions.

The Generator does not expose a "tokens generated so far" counter. Track it yourself by summing `out.tokens.len()` each step.
