...
| Python | hand-authored project (no scaffold) | Python 3.14 inferlet runtime (downloaded by `pie config init`) |
...
`bakery` is the inferlet toolchain. It scaffolds projects, compiles to WebAssembly, and publishes to the registry. It ships with `pie-server`:
...
bakery build . -o hello.wasm
pie run --path ./hello.wasm --manifest ./Pie.toml --prompt "..."
```
