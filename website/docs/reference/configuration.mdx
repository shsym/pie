...
`[model.driver].type` is a discriminator looked up in the driver registry. Driver-specific knobs live in the `[model.driver.options]` sub-table; pie validates them against the driver's typed config. See the per-driver pages ([CUDA](./drivers/cuda), [Portable](./drivers/portable), [vLLM](./drivers/vllm), [SGLang](./drivers/sglang)) for what each backend accepts.
...
| Key | Default | Description |
|-----|---------|-------------|
| `type` | required | Driver name (looked up in the registry). One of `native`, `cuda_native`, `portable`, `vllm`, `sglang`, `dummy`. See the per-driver reference pages. |
...
Driver-specific knobs in the driver's own vocabulary. Pie does not validate these centrally; the driver's typed config dataclass parses them at server / worker startup. See the per-driver pages ([CUDA](./drivers/cuda), [Portable](./drivers/portable), [vLLM](./drivers/vllm), [SGLang](./drivers/sglang)) for each driver's option set.
