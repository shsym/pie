...
| `ipc_profile` | `"balanced"` | IPC wait profile. `low_latency` uses the polling in-process channel for embedded CUDA/portable drivers and busy-spins on shmem; `balanced` spins for 1000 us then parks; `low_power` parks immediately. |
