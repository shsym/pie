...
`[model.driver].type` is a discriminator. Embedded drivers (`portable`, `cuda_native`, `dummy`) run inside the `pie` process. Python drivers (`vllm`, `sglang`, `tensorrt_llm`) run as supervised subprocesses. Driver-specific knobs live in `[model.driver.options]`; see the per-driver pages ([CUDA](./drivers/cuda), [Portable](./drivers/portable), [vLLM](./drivers/vllm), [SGLang](./drivers/sglang), [TensorRT-LLM](./drivers/tensorrt-llm)) for what each backend accepts.
...
| Key | Default | Description |
|-----|---------|-------------|
| `type` | required | Driver name. One of `portable`, `cuda_native`, `dummy`, `vllm`, `sglang`, `tensorrt_llm`. See the per-driver reference pages. |
...
| `ipc_profile` | driver-dependent | IPC wait profile. Omitted CUDA uses `latency`; omitted non-CUDA uses `balanced`. `latency` uses the polling in-process channel for embedded CUDA/portable drivers and busy-spins on shmem; `balanced` spins for 1000 us then parks; `power` parks immediately. |
| `spin_budget_us` | unset | Expert override for the profile's busy-spin window in microseconds. Leave unset unless tuning latency/CPU tradeoffs directly. |

### `[model.driver.options]`

Driver-specific knobs in the driver's own vocabulary. Embedded drivers are validated by `pie` before boot. Python drivers accept `venv` or `python` as standalone-only interpreter overrides and pass the remaining options to the driver subprocess. See the per-driver pages ([CUDA](./drivers/cuda), [Portable](./drivers/portable), [vLLM](./drivers/vllm), [SGLang](./drivers/sglang), [TensorRT-LLM](./drivers/tensorrt-llm)) for each driver's option set.
...
- Per-driver options: [CUDA](./drivers/cuda), [Portable](./drivers/portable), [vLLM](./drivers/vllm), [SGLang](./drivers/sglang), [TensorRT-LLM](./drivers/tensorrt-llm).
