...
| `ipc_profile` | driver-dependent | IPC wait profile. Omitted CUDA uses `latency`; omitted non-CUDA uses `balanced`. `latency` uses the polling in-process channel for embedded CUDA/metal drivers and busy-spins on shmem; `balanced` spins for 1000 us then parks; `power` parks immediately. |
