...
`[model.driver].type` is a discriminator. Embedded drivers (`cuda_native`, `metal`, `dummy`) run inside the `pie` process. Driver-specific knobs live in `[model.driver.options]`; see the per-driver pages ([CUDA](./drivers/cuda)) for what each backend accepts.
...
| `model` | required | HuggingFace repo id or local HuggingFace snapshot directory to load. Repo ids use the standard HuggingFace cache and download on cache miss. |
...
| `admission_oversubscription_factor` | `1.0` | How much logical KV memory the market overcommits relative to physical: `Σ endowment ≤ total_pages × factor`. `1.0` = no overbook. Higher values sell more entitlement than physical capacity and are best treated as an explicit workload choice. Must be a finite `> 0` number. |
| `frame_size` | `2` | Token steps the batch quorum admits before it runs (*k*). At `1` the quorum runs once per token, which stops the fleet overlapping batches above ~64 concurrent processes and costs about 29% throughput at concurrency 256. `3` and `4` measure the same as `2` while consuming more driver staging depth. Bounded jointly with `frame_dispatch_depth` — see that key. Inferlets read it as `model.frame-size()`. |
| `frame_submit_depth` | `3` | Frames an inferlet keeps submitted into the engine: one running, the rest queued behind it so the device still has work while the inferlet is on the host deciding what to submit next. Too few and the pipeline runs in lockstep — submit, wait, submit, wait. Must be `≥ 2`. **Not independent of `frame_size`:** a frame is `frame_size` token steps of device time, so the same three frames cover proportionally less real time as `frame_size` shrinks. Re-measure this whenever you change `frame_size`. Inferlets see it only through the derived `model.channel-capacity()`. |
| `frame_dispatch_depth` | `2` | Frames the engine keeps posted to the driver but not yet retired. Where `frame_submit_depth` is the inferlet running ahead of the engine, this is the engine running ahead of the device: at `2`, one frame executes while the next is already uploaded, so the GPU does not idle between frames. **This is a genuine two-sided trade-off.** Dispatching commits KV pages early, and a frame's contents are fixed when it is sealed, so deeper dispatch runs batches packed against a staler queue. With headroom the idle it removes wins, which is where `2` was tuned; on a fully batched, zero-headroom fleet there is no idle left to remove and depth costs throughput — measured 610 ms at depth 1 against 1034 ms at depth 2 on a pure-attention model. `frame_dispatch_depth × frame_size` must be `< 13`: the CUDA driver allocates one pinned upload slot per token step and has 13, so at or above that every submit blocks waiting for a slot. |
...
| Key | Default | Description |
|-----|---------|-------------|
| `type` | required | Driver name. One of `cuda_native`, `metal`, `dummy`. See the per-driver reference pages. |
...
Driver-specific knobs in the driver's own vocabulary. Embedded drivers are validated by `pie` before boot. See the per-driver pages ([CUDA](./drivers/cuda)) for each driver's option set.
...
`pie config set` takes a dot-path under the section the key lives in (e.g. `server.port`, not `port`). Numeric segments index into TOML arrays — `model.0.model` targets the first `[[model]]` block. Comma-separated values become lists (`"cuda:0,cuda:1"` → `["cuda:0", "cuda:1"]`).
...
- Per-driver options: [CUDA](./drivers/cuda).
- Supported architectures are listed on each driver page (e.g. [CUDA `#supported-architectures`](./drivers/cuda#supported-architectures)).
