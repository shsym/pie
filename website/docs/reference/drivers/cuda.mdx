...
| `kv_cache_dtype` | `"auto"` | KV cache quantization fallback format: `auto`, `bf16`, `bfloat16`, `fp8_e4m3`, `fp8_e5m2`, `int8_per_token_head`, `fp8_per_token_head`, `fp4_e2m1`, or `nvfp4`. |
...
The `dev` driver chooses its FlashInfer page size internally and reports it in
`DriverCapabilities.kv_page_size` during startup.
