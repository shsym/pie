...
Pick SGLang when you want strong custom-mask support or its n-gram speculative decoding. Like vLLM, some inferlet features that depend on raw forward-pass control are not available through SGLang; those require the [CUDA `native` driver](./cuda).

## Install

```bash
pip install 'pie-server[sglang]'
```
