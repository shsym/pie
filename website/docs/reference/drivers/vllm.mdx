...
| `max_num_batched_tokens` | unset | Max tokens (across all sequences) in a batch. Unset uses vLLM's default. |
| `block_size` | unset | KV cache block size override. Unset lets vLLM pick based on the attention backend's allowed sizes (FlashInfer: 16/32/64; FlashAttention: 16/32). |
