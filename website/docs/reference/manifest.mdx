...
`Pie.toml` sits at the root of the inferlet project, next to `Cargo.toml` (Rust), `pyproject.toml` (Python), or `package.json` (JavaScript). `bakery create <name>` lays one down for Rust and TypeScript projects; Python projects currently create it manually.
