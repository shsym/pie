...
`pie run` walks the trailing flags and turns each `--flag value` pair into a JSON entry. Hyphens in long-form flag names become underscores in JSON keys (`--max-tokens` → `"max_tokens"`). Types are inferred from the value itself, not from `[parameters]`: `pie run` tries `int`, then `float`, then `bool`, then falls back to `string`. The manifest's declared `type` is documentation; the CLI does not consult it.
