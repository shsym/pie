...
Multimodal media input (image/audio/video) and audio output (text-to-speech). See [Multimodal generation](../guide/model/multimodal) for the full guide and examples.

| Method | Description |
|---|---|
| `Image.fromBytes(model, bytes): Image` | Decode image bytes. `image.tokenCount()` / `image.grid()` expose the soft-token layout. |
| `Audio.fromBytes(model, bytes): Audio` | Decode an audio clip. |
| `Video.fromBytes(model, bytes, maxFrames): Video` | Demux and uniformly sample up to `maxFrames` frames. |
| `await ctx.appendImage(image)` | Run the vision encoder and commit soft-token KV pages. |
| `await ctx.appendImages(images)` | Splice adjacent images in one forward when delimiters allow it. |
| `await ctx.appendAudio(clip)` | Run the audio encoder and commit. |
| `await ctx.appendAudios(clips)` | Splice adjacent audio clips in one forward when possible. |
| `await ctx.appendVideo(video)` | Commit the sampled frames. |
| `await model.speak(text).speaker(id).maxDurationSeconds(s).generate()` | Text-to-speech. `speech.toWav()`, `sampleRate()`, `channels()`. |
