...
Pulls in the common types: `main`, `Context`, `Result`, `Schema`, `Model`, `runtime`, `session`, `Adapter`, `Forward`, `Output`, `SampleHandle`, `ProbeHandle`, `Sampler`, `Probe`, `Generator`, `GenStep`, `chat`, `reasoning`, `tools`, and `Speculator`.
...
| `session::receive().await` | Wait for the next text message. |
| `session::receive_file().await` | Wait for the next binary blob. |

Await `receive()` directly:

```rust
use inferlet::session;

let msg: Option<String> = session::receive().await;
```
