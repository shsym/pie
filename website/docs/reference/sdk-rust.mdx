...
| `.constrain_with(Schema::JsonSchema(s))?` | Apply a declarative schema. |
...
| `out.tokens` | `&[u32]` | All tokens the forward pass accepted. |
| `out.raw().slots` | underlying slot list | Advanced cases. |
...
ctx.generate(sampler).constrain_with(Schema::JsonSchema(s))?;
```

| Variant | Description |
|---|---|
| `Schema::Json` | Any valid JSON. |
| `Schema::JsonSchema(s: &str)` | JSON matching the JSON Schema. |
| `Schema::Regex(p: &str)` | Strings matching the regex. |
| `Schema::Ebnf(g: &str)` | Custom EBNF grammar (Lark format). |
| `Schema::Grammar(&grammar)` | Pre-compiled grammar object. |
...
Plug in with `g.speculator(spec)` on the generator builder.
...
| `tools::parse_call(&model, text) -> Option<(String, String)>` | One-shot extract from a finished string. |
