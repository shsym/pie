...
```rust
// First request: compute and cache the system prompt in a named context
let prefix = Context::create(&model, "system-prompt-v1", None)?;
prefix.system("You are a helpful assistant. [long system prompt...]");
prefix.flush().await?;
// Pages are now committed and cached under "system-prompt-v1"
```

### Reuse the cached prefix

```rust
// Subsequent requests: look up and fork the cached context
let prefix = Context::lookup(&model, "system-prompt-v1")
    .ok_or("Prefix not found")?;

let ctx = prefix.fork("request-123")?;
...
Contexts persist within an inferlet instance. Look one up by name to resume state:

```rust
let ctx = match Context::lookup(&model, "session-123") {
    Some(ctx) => ctx,                     // Resume existing session
    None => {
        let ctx = Context::create(&model, "session-123", None)?;
