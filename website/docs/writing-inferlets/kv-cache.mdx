...
let working = ctx.working_page_count();          // Working (not yet committed) pages
```
...
ctx.reserve_working_pages(pages_needed)?;
```
...
```rust
ctx.commit_working_pages(num_full_pages)?;
```
...
ctx.release_working_pages(num_pages);
```
