...
let untrusted_page_count = ctx.inner().committed_page_count();
...
// Release all working pages that contained the raw email
let to_release = ctx.inner().committed_page_count() - untrusted_page_count;
ctx.inner().release_working_pages(to_release);

// Continue with only the safe extraction
let mut safe_ctx = Context::new(&model)?;
...
let searcher = Context::new(&model)?;
...
for path in &related_paths {
    let summary_ctx = Context::new(&model)?;
