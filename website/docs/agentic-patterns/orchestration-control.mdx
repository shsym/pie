...
let worker = Context::new(&worker_model)?;
...
if result.contains("[ESCALATE]") {
    ...
    let oracle = Context::new(&oracle_model)?;
...
let quarantined = Context::new(&model)?;
quarantined.system(
...
let privileged = Context::new(&model)?;
...
```
 Session 1:  Initializer ──▶ Create ctx ──▶ Scaffold ──▶ [save → named ctx]
                                                              │
 Session N:  Maintainer  ──▶ open(ctx)   ──▶ Add Feature ──▶ [ctx updated]
```

```rust
// Initializer: runs once, creates and saves a named context
let init = Context::new(&model)?;
...
let scaffold = init.generate(sampler.clone())
    .with_max_tokens(1024)
    .collect_text().await?;
init.save("project-state")?;

// ... later, in a new session:

// Maintainer: resumes from named context
let state = Context::open(&model, "project-state")
...
:::tip[Why Pie]
`Context::open()` retrieves a named context with its full KV cache intact. The Maintainer resumes with complete memory of the initialization — no re-prompting or state serialization needed.
:::
...
let retriever = Context::new(&retrieval_model)?;
...
let generator = Context::new(&gen_model)?;
...
let research = Context::new(&model)?;
...
let planning = Context::new(&model)?;
...
let execution = Context::new(&model)?;
...
let perceive = Context::new(&model)?;
perceive.system(
