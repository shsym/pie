...
bool test_softmax_edge_cases() {
    ...
    // Test 3: Very large values (potential overflow)
    {
        ...
        if (result != 0) {
            ...
            // Use looser tolerance for large values due to numerical precision
            if (!has_finite || std::abs(sum - 1.0) > 0.1) {
    ...
    // Test 4: High temperature (approaching uniform)
    {
        ...
        if (result != 0) {
            ...
            // Check that distribution is reasonably balanced (not perfectly uniform)
            float min_val = *std::min_element(output.begin(), output.end());
            float max_val = *std::max_element(output.begin(), output.end());
            float range = max_val - min_val;
            
            // With high temperature, the range should be small (approaching uniform)
            if (range > 0.2f) { // Very lenient tolerance
                std::cerr << "❌ High temperature should approach uniform distribution, range=" << range << std::endl;
                all_passed = false;
            } else {
