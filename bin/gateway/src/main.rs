...
//! workers dial IN. A thin bin shell (Seam 3): the `startup` process skeleton
...
/// come from `startup`'s [`GlobalArgs`](startup::GlobalArgs); the gateway adds
...
async fn main() -> anyhow::Result<ExitCode> {
    ...
    let ctx = startup::init(
        startup::BootSpec::gateway().version(env!("CARGO_PKG_VERSION")),
        cli.global,
    )?;
