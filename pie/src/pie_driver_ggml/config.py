"""TOML-shape config for `[model.X.driver.ggml]`."""
...
@dataclass
class GgmlDriverConfig:
    """Knobs the C++ binary needs at boot.

    Field names mirror `NativeDriverConfig` / `CudaNativeDriverConfig`
    where possible so swapping `type = "ggml"` is a one-line change.
    """

    # ── binary discovery ──────────────────────────────────────────────────
    # Override path to the `pie_driver_ggml` executable. Empty → resolve via
    # $PIE_GGML_DRIVER_BIN, then `which pie_driver_ggml`, then a relative
    # search next to the source tree (`<repo>/driver/ggml/build/bin/`).
    binary_path: str = ""
