"""Worker entry point for the vllm backend.

Mirrors `pie_backend.worker.worker_main` so `pie/server.py`'s spawn call works
identically. The leader/follower RPC loops, distributed init, and process-group
setup are all reused directly from pie_backend — only the engine class changes.
"""
...
def worker_main(
    ...
    """Worker process entry point for `mp.spawn` — vllm variant.

    Same signature as `pie_backend.worker.worker_main`. Constructs a VllmEngine
    instead of pie_backend's Engine; otherwise the worker behavior (distributed
    init, RPC loop, broadcast to TP followers, telemetry) is identical and is
    reused directly from `pie_backend.worker`.
    """
    import torch
...
def _worker_body(
    ...
    engine = VllmEngine.load(config, compute_process_group=compute_pg)
