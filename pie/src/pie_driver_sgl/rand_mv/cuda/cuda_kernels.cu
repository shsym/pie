...
// Per-input-section variant of Sections. Used by
// batched_randn_matmul_sectioned_per_input_kernel. Each section has its
// own input slice (in_offsets / in_widths) AND its own output slice
// (out_widths). This is what LoRA UP needs: each output section's
// matmul uses a different input slice (q_down, k_down, v_down) of
// qkv_down, not a shared input.
struct SectionsPerInput {
    int  out_widths[4];      // per-section output column count (sum to O)
    int  in_widths[4];       // per-section input column count
    int  in_offsets[4];      // per-section input start offset within x
    int64_t seed_offsets[4]; // per-section seed offset added to seeds[b]
    int  n;
};
...
// ============================================================================
//  Matmul kernel, sectioned (Q/K/V-fused) for LoRA UP projection — variant
//  with per-section INPUT slicing. Each output section's matmul uses a
//  different per-section input range:
//
//     y[b, o ∈ section k] = alpha * Σ_{i=0..in_widths[k])
//                              x[b, in_offsets[k] + i]
//                              * (W_mean[i, o] + S[i, o] * N(seed_b + seed_off_k, ...))
//                          + beta * y[b, o]
//
//  S has shape (max_in_width, O); rows i ∈ [0, in_widths[k]) of column o ∈
//  section k are the only ones used for that section.
//
//  Constraint: each block (BN consecutive output cols) must lie entirely
//  within one section. Caller ensures this by making out_widths multiples
//  of BN. Adapter UP for Llama-3.2-1B: out_widths = (2048, 512, 512), all
//  divisible by 32/64/128. Asserted in the C++ entry point.
//
//  k_split is intentionally not supported here — UP-projection input dim
//  (in_widths[k] = rank, typically ≤ 8) is always tiny so single-pass
//  writes fully utilize SMs as long as B * tiles_n is large enough.

template<typename XT, typename ST, typename OT, int BN, bool HAS_MEAN>
__global__ void __launch_bounds__(BN, 4) batched_randn_matmul_sectioned_per_input_kernel(
    const XT* __restrict__ x,
    const int64_t* __restrict__ seeds,
    const ST* __restrict__ S,
    const ST* __restrict__ W_mean,
    ...
    // Section determined from the block's first output column. All
    // threads in the block must agree, so use n_tile * BN (block-uniform)
    // rather than o_local. Caller ensures sections align with BN.
    //
    // The `still_advancing` guard is load-bearing: without it, asymmetric
    // section widths like (2048, 512) mis-classify a block at o=512.
    // At s=1 the per-iter check `512 >= 0 + 512` is true even though
    // s=0's check (`512 >= 0 + 2048`) was false — so we'd erroneously
    // advance to section 2. Pragma-unrolled loops can't `break`, so we
    // mask the per-iter condition with a sticky flag.
    int block_first_o = n_tile * BN;
    int sec_idx = 0;
    int boundary = 0;
    bool still_advancing = true;
    #pragma unroll
    for (int s = 0; s < 4; s++) {
        bool advance = still_advancing && (s < sec.n)
                       && (block_first_o >= boundary + sec.out_widths[s]);
        if (advance) {
            boundary += sec.out_widths[s];
            sec_idx = s + 1;
        } else {
            still_advancing = false;
        }
    }
    int sec_out_w = (sec_idx < sec.n) ? sec.out_widths[sec_idx] : 0;
    int sec_in_w = sec.in_widths[sec_idx];
    int sec_in_off = sec.in_offsets[sec_idx];
    int64_t seed_off_for_sec = sec.seed_offsets[sec_idx];
    int o_in_section = o_local - boundary;
    ...
    uint32_t seed = (uint32_t)(seed_val + seed_off_for_sec);

    const int I_packs = (sec_in_w + 3) / 4;
    const int o_abs = o_in_section + col_offset;
    const uint32_t section_global_cols = (uint32_t)sec_out_w;
    ...
    float acc = 0.0f;
    // Per-section x base: skip in_offset columns into x.
    const int x_base = b * stride_xb + sec_in_off * stride_xi;
    const bool active = (o_local < O);

    int kp = 0;
    while (kp < I_packs) {
        int chunk = (I_packs - kp) < X_PACK ? (I_packs - kp) : X_PACK;
        int vals_to_load = chunk * 4;

        __syncthreads();
        for (int j = t; j < vals_to_load; j += BN) {
            int row = kp * 4 + j;  // local row within section
            float xv = 0.0f;
            if (row < sec_in_w) xv = (float)x[x_base + row * stride_xi];
            x_sh[j] = xv;
        }
        __syncthreads();
        ...
        if (active) {
            ...
            for (int pp = 0; pp < chunk; pp++) {
                int this_kp = kp + pp;
                int row0 = this_kp * 4;  // local row within section

                uint32_t offset = (uint32_t)this_kp * section_global_cols + (uint32_t)o_abs;
                Normals4 w = philox_4_normals(seed, offset ZIG_LUT_ARG);

                float s0 = 0.0f, s1 = 0.0f, s2 = 0.0f, s3 = 0.0f;
                if (row0     < sec_in_w) s0 = (float)S[(row0    ) * stride_Si + o_local * stride_So];
                if (row0 + 1 < sec_in_w) s1 = (float)S[(row0 + 1) * stride_Si + o_local * stride_So];
                if (row0 + 2 < sec_in_w) s2 = (float)S[(row0 + 2) * stride_Si + o_local * stride_So];
                if (row0 + 3 < sec_in_w) s3 = (float)S[(row0 + 3) * stride_Si + o_local * stride_So];

                int base = pp * 4;
                if constexpr (HAS_MEAN) {
                    float m0 = 0.0f, m1 = 0.0f, m2 = 0.0f, m3 = 0.0f;
                    if (row0     < sec_in_w) m0 = (float)W_mean[(row0    ) * stride_Wi + o_local * stride_Wo];
                    if (row0 + 1 < sec_in_w) m1 = (float)W_mean[(row0 + 1) * stride_Wi + o_local * stride_Wo];
                    if (row0 + 2 < sec_in_w) m2 = (float)W_mean[(row0 + 2) * stride_Wi + o_local * stride_Wo];
                    if (row0 + 3 < sec_in_w) m3 = (float)W_mean[(row0 + 3) * stride_Wi + o_local * stride_Wo];
                    acc += x_sh[base + 0] * (m0 + s0 * w.n0);
                    acc += x_sh[base + 1] * (m1 + s1 * w.n1);
                    acc += x_sh[base + 2] * (m2 + s2 * w.n2);
                    acc += x_sh[base + 3] * (m3 + s3 * w.n3);
...
template<typename XT, typename ST, typename OT, int BN>
void launch_matmul_sectioned_per_input(
    const torch::Tensor& x, const torch::Tensor& seeds, const torch::Tensor& S,
    torch::Tensor& y, int col_offset, int global_cols,
    SectionsPerInput sec, float alpha, float beta,
    const torch::Tensor* W_mean_opt
) {
    const int B = (int)x.size(0);
    const int O = (int)S.size(1);
    ...
    dim3 grid(tiles_n, B, 1);
    dim3 block(BN);

    auto stream = at::cuda::getCurrentCUDAStream().stream();
    if (W_mean_opt != nullptr) {
        const auto& W = *W_mean_opt;
        batched_randn_matmul_sectioned_per_input_kernel<XT, ST, OT, BN, true><<<grid, block, 0, stream>>>(
            x.data_ptr<XT>(),
            seeds.data_ptr<int64_t>(),
            S.data_ptr<ST>(),
            W.data_ptr<ST>(),
        ...
        batched_randn_matmul_sectioned_per_input_kernel<XT, ST, OT, BN, false><<<grid, block, 0, stream>>>(
            x.data_ptr<XT>(),
            seeds.data_ptr<int64_t>(),
            S.data_ptr<ST>(),
            nullptr, 0, 0,
...
torch::Tensor batched_randn_matmul_sectioned_per_input_cuda(
    torch::Tensor x, torch::Tensor seeds, torch::Tensor S,
    std::vector<int64_t> in_widths,
    std::vector<int64_t> in_offsets,
    std::vector<int64_t> out_widths,
    ...
    TORCH_CHECK(x.is_cuda() && S.is_cuda(), "x and S must be CUDA");
    TORCH_CHECK(x.dim() == 2 && S.dim() == 2, "x and S must be 2D");
    TORCH_CHECK(
        in_widths.size() == out_widths.size() &&
        in_widths.size() == in_offsets.size() &&
        in_widths.size() == seed_offsets.size(),
        "section vectors length mismatch"
    );
    TORCH_CHECK(in_widths.size() >= 1 && in_widths.size() <= 4,
                "up to 4 sections supported");

    const int64_t B = x.size(0);
    const int64_t O = S.size(1);

    int64_t total_out = 0;
    for (auto w : out_widths) total_out += w;
    TORCH_CHECK(total_out == O, "sum(out_widths) must equal S.shape[1]");

    SectionsPerInput sec{};
    sec.n = (int)in_widths.size();
    for (int i = 0; i < sec.n; i++) {
        sec.in_widths[i] = (int)in_widths[i];
        sec.in_offsets[i] = (int)in_offsets[i];
        sec.out_widths[i] = (int)out_widths[i];
        sec.seed_offsets[i] = seed_offsets[i];
    }
    ...
    // Block-alignment: each block (BN cols) must lie within one section.
    // We pick the largest BN whose tiles align with every section boundary.
    int chosen_BN = 0;
    auto aligns = [&](int BN) -> bool {
        int boundary = 0;
        for (int i = 0; i < sec.n; i++) {
            if ((boundary % BN) != 0) return false;
            if ((sec.out_widths[i] % BN) != 0 && i + 1 < sec.n) return false;
            boundary += sec.out_widths[i];
        }
        return true;
    };
    if (O >= 128 && aligns(128))      chosen_BN = 128;
    else if (O >= 64 && aligns(64))   chosen_BN = 64;
    else if (aligns(32))              chosen_BN = 32;
    TORCH_CHECK(chosen_BN > 0,
                "section out_widths must be aligned with BN ∈ {32, 64, 128}; "
                "got widths that don't align with any of them");

    auto dispatch_bn = [&](auto xt_tag, auto st_tag, auto ot_tag) {
        using XT = typename decltype(xt_tag)::type;
        using ST = typename decltype(st_tag)::type;
        using OT = typename decltype(ot_tag)::type;
        if (chosen_BN == 32)       launch_matmul_sectioned_per_input<XT, ST, OT, 32>(x, seeds, S, y, col_offset, global_cols, sec, kernel_alpha, kernel_beta, W_mean_ptr);
        else if (chosen_BN == 64)  launch_matmul_sectioned_per_input<XT, ST, OT, 64>(x, seeds, S, y, col_offset, global_cols, sec, kernel_alpha, kernel_beta, W_mean_ptr);
        else                       launch_matmul_sectioned_per_input<XT, ST, OT, 128>(x, seeds, S, y, col_offset, global_cols, sec, kernel_alpha, kernel_beta, W_mean_ptr);
    };
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("batched_randn_matmul_sectioned", &batched_randn_matmul_sectioned_cuda,
          ...
          py::arg("beta") = 0.0,
          py::arg("W_mean") = c10::nullopt);
    m.def("batched_randn_matmul_sectioned_per_input",
          &batched_randn_matmul_sectioned_per_input_cuda,
          "batched_randn_matmul with per-section input AND output slices (CUDA). "
          "Designed for LoRA UP fusion: each output section's matmul uses a "
          "different per-section input range of x.",
          py::arg("x"), py::arg("seeds"), py::arg("S"),
          py::arg("in_widths"), py::arg("in_offsets"),
          py::arg("out_widths"), py::arg("seed_offsets"),
          py::arg("col_offset") = 0, py::arg("global_cols") = 0,
          py::arg("out") = c10::nullopt,
          py::arg("alpha") = 1.0,
          py::arg("beta") = 0.0,
