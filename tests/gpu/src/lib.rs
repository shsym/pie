//! The runtime's on-device end-to-end suite. **There is no code here** — this
...
//! one crate that links an engine, not because they test a CLI: `cargo test -p
...
//! panics on a second boot; the engine takes a fixed POSIX shmem), so every
