...
# ---------------------------------------------------------------------------
# Configure Python path so we can import backend modules just like the server
# entrypoints do. We reuse the shared Python backend models/config plus the
# Metal-specific runtime that lives under backend/backend-metal/src.
# ---------------------------------------------------------------------------
BACKEND_ROOT = Path(__file__).resolve().parents[3] / "backend"
BACKEND_PYTHON_PATH = BACKEND_ROOT / "backend-python"
BACKEND_METAL_SRC = BACKEND_ROOT / "backend-metal" / "src"
INTEGRATION_TEST_DIR = Path(__file__).resolve().parent

for path in (BACKEND_METAL_SRC, BACKEND_PYTHON_PATH):
    path_str = str(path)
