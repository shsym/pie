...
async fn main(input: Input) -> Result<String> {
    ...
    // One recurrent working set per beam lane on a model that folds a
    // state; none on a pure-attention model, where the empty binding IS
    // the attention pass (the hybrid interface with `rs = []`).
    let mut rs_working_sets: Vec<RsWorkingSet> = match model::pass_kind() {
        model::ForwardKind::Attention => Vec::new(),
        model::ForwardKind::Hybrid => (0..B).map(|_| RsWorkingSet::new()).collect(),
