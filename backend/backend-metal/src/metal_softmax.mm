...
int metal_softmax_float(
    ...
    @autoreleasepool {
        ...
        if (vocab_size > 16384 && g_softmax_large_tiled_pipeline) {
            ...
            // Use standard kernel - optimize threadgroup size for large vocabularies
            const uint32_t max_threadgroup_size = 1024;
            threadgroup_size = std::min(static_cast<uint32_t>(vocab_size), max_threadgroup_size);

            // For very large vocabularies, ensure we have sufficient threads
            if (vocab_size > 32000) {
                threadgroup_size = max_threadgroup_size; // Use maximum threads for large vocabs
                std::cout << "Using optimized threadgroup_size=" << threadgroup_size
                         << " for large vocab_size=" << vocab_size << std::endl;
            }
