...
int main() {
    ...
    const std::string artifacts_dir = "../../metal-protocol-tests/tests/artifacts/silu_and_mul/production/";
    ...
    if (gate_data.empty() || up_data.empty() || expected_output.empty()) {
        ...
        std::cerr << "cd ../../metal-protocol-tests && make && CUDA_VISIBLE_DEVICES=0 ./build/metal_protocol_tests --op silu_and_mul --case production --num_tokens 128 --intermediate_size 11008" << std::endl;
        return 1;
    }
