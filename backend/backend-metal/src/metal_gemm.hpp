...
/**
 * @brief Pure f32 Metal implementation of GEMM operation without bfloat16 conversion
 *
 * Implements C = A × B^T + bias (optional) using Metal Performance Shaders
 * Performs all computation in f32 and stores f32 results directly
 *
 * @param device Metal device to run computation on
 * @param commandQueue Metal command queue for GPU commands
 * @param A Input matrix A [m, k] or [k, m] if transposed (float32)
 * @param B Input matrix B [k, n] or [n, k] if transposed (float32)
 * @param bias Optional bias vector [n] (float32), can be nullptr
 * @param C Output matrix C [m, n] (float32)
 * @param m Number of rows in A and C
 * @param n Number of columns in B and C
 * @param k Number of columns in A / rows in B
 * @param workspace Metal buffer for intermediate computations (unused for compatibility)
