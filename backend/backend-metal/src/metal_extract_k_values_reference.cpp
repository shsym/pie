...
bool test_extract_k_values_vs_cuda() {
    ...
    const std::string artifacts_dir = "../../metal-protocol-tests/tests/artifacts/extract_k_values/production_bf16/";
    ...
    if (input_data.empty() || expected_values.empty() || expected_indices.empty()) {
        ...
        std::cout << "To generate CUDA artifacts, run:" << std::endl;
        std::cout << "cd ../../metal-protocol-tests && make && CUDA_VISIBLE_DEVICES=0 ./build/metal_protocol_tests --op extract_k_values --case production_bf16" << std::endl;
